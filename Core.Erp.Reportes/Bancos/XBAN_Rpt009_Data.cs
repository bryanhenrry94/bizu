  ...
  public  class XBAN_Rpt009_Data
    {
        ...
        public List<XBAN_Rpt009_Info> get_ReporteSaldoBancos(int IdEmpresa, DateTime Fecha_ini, DateTime Fecha_fin)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Get_Data_Reporte", ex.Message), ex) { EntityType = typeof(XBAN_Rpt009_Data) };
            }
        }
        ...
        public List<XBAN_Rpt009_Info> get_ReporteSaldoBancos_Considerar_IngEgr_X_AnulacionContable(int IdEmpresa, DateTime Fecha_ini, DateTime Fecha_fin)
        {
            try
            {
                ...
                using (EntitiesBancos_Reporte_Ge listado = new EntitiesBancos_Reporte_Ge())
                {
                    var select = from q in listado.spBAN_Rpt009_1(IdEmpresa, Fecha_ini, Fecha_fin)
                                 select q;
