...
{
    public partial class XINV_Rpt031_frm : Form
    {
        ...
        void ucInv_MenuReportes1_event_tnConsultar_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                XINV_Rpt031_Rpt rpt = new XINV_Rpt031_Rpt();
                ...
                rpt.IdMarca_List = new List<Int32>();

                foreach (CheckedListBoxItem item in ucInv_MenuReportes1.cmb_Marca_Multiple.GetItems())
                {
                    if (item.CheckState == CheckState.Checked)
                    {
                        rpt.IdMarca_List.Add(Convert.ToInt32(item.Value));
                    }
                }
