...
{
    public partial class XINV_Rpt025_SinCosto_Rpt : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XINV_Rpt025_SinCosto_Rpt()
        {
            InitializeComponent();
        }
        ...
        public void Set_parameters(int IdSucursal, string nom_Sucursal, int IdBodega, string nom_Bodega, int IdMovi_inven_tipo,string nom_Movi_inven_tipo, decimal IdProducto, string nom_Producto, DateTime fecha_ini, DateTime fecha_fin, string signo)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Set_parameters", ex.Message), ex) { EntityType = typeof(XINV_Rpt025_SinCosto_Rpt) };
            }
        }

        private void XINV_Rpt025_SinCosto_Rpt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "XINV_Rpt025_SinCosto_Rpt_BeforePrint", ex.Message), ex) { EntityType = typeof(XINV_Rpt025_SinCosto_Rpt) };
            }
        }
