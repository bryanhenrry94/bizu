...
namespace Core.Erp.Reportes.Inventario
{
    public class XINV_Rpt010_Data
    {
        ...
        public List<XINV_Rpt010_Info> Get_List(DateTime Fecha_desde, DateTime Fecha_hasta, int IdEmpresa, int IdSucursal, List<int> lst_bodega, decimal IdProducto, string idUsuario, bool No_mostrar_valores_en_0, bool Mostrar_detallado, string IdCategoria, List<int> List_IdLinea)
        {
            try
            {
                ...
                using (Entities_Inventario_General context = new Entities_Inventario_General())
                {
                    ...
                    foreach (var item_bodega in lst_bodega)
                    {
                        ...
                        if (IdCategoria != "0")
                        {
                            ...
                            {
                                lst = lst.Where(q => q.IdCategoria == IdCategoria && List_IdLinea.Contains(q.IdLinea));
                            }
                            else
                            {
                        ...
                        foreach (var item in lst)
                        {
                            ...
                            if (item.IdProducto != IdProducto_tmp)
                            {
                                IdProducto_tmp = item.IdProducto;
                            ...
                            {
                                info.Saldo_fin_cant = 0;
                                info.Cost_prom_fin = 0;
                                info.Saldo_fin_cost = 0;
                            }
