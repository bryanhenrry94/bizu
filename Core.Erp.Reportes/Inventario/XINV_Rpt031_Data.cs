    ...
    public class XINV_Rpt031_Data
    {
        public List<XINV_Rpt031_Info> Get_list(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdProducto, int IdMarca, DateTime Fecha_ini, DateTime Fecha_fin)
        {
            try
            {
                ...
                List<XINV_Rpt031_Info> Lista = new List<XINV_Rpt031_Info>();
                ...
                using (Entities_Inventario_General Context = new Entities_Inventario_General())
                {
                    ...
                    {
                        XINV_Rpt031_Info prd = new XINV_Rpt031_Info();
            ...
            try
            {
                ...
                using (Entities_Inventario_General Context = new Entities_Inventario_General())
                {
                    var lst = from q in Context.vwINV_Rpt031
                              ...
                              && IdBodega_ini <= q.IdBodega && q.IdBodega <= IdBodega_fin                              
                              && Fecha_ini <= q.Fecha_Transac && q.Fecha_Transac <= Fecha_fin
                              && IdMarca_List.Contains(q.IdMarca)
                              select q;
            ...
            try
            {
                ...
                using (Entities_Inventario_General Context = new Entities_Inventario_General())
                {
                    ...
                    foreach (var item in lst)
                    {
                        ...
                        {
                            if (item.IdMarca == 1 || item.IdMarca == 8)
                            {
