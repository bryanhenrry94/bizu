    ...
    public class XCOMP_Rpt003_Data
    {
        ...
        public List<XCOMP_Rpt003_Info> consultar_data(int idempresa, int idsucursal, int IdMovi_inven_tipo, decimal IdNumMovi, int IdBodega, ref string mensaje)
        {
            try
            {
                ...
                using (EntitiesCompra_reporte_Ge EIngresoCompras = new EntitiesCompra_reporte_Ge())
                {
                    ...
                    foreach (var item in select)
                    {
                        ...
                        itemInfo.EsDocumentoAutorizado = item.EsDocumentoAutorizado;
                        itemInfo.NumGuia = item.NumGuia;
                        itemInfo.numDocumento = item.numDocumento;
                        itemInfo.Fecha_registro = item.Fecha_registro;
                        itemInfo.IdUsuario_registro = item.IdUsuario_registro;
