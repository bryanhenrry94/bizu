...
{
    public partial class XCOMP_Rpt010_Rpt : DevExpress.XtraReports.UI.XtraReport
    {
        cl_parametrosGenerales_Bus param = cl_parametrosGenerales_Bus.Instance;
        tb_sis_Log_Error_Vzen_Bus Log_Error_bus = new tb_sis_Log_Error_Vzen_Bus();
        XCOMP_Rpt010_Bus repbus = new XCOMP_Rpt010_Bus();

        public XCOMP_Rpt010_Rpt()
        {
            InitializeComponent();
        }
        ...
        private void XCOMP_Rpt010_Rpt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {                
                List<XCOMP_Rpt010_Info> ListDataRpt = new List<XCOMP_Rpt010_Info>();
                ...
                bool MostrarConSaldo = Convert.ToBoolean(Parameters["MostrarConSaldo"].Value);
                string IdCentroCosto = Convert.ToString(Parameters["IdCentroCosto"].Value);

                ListDataRpt = repbus.Cargar_data(param.IdEmpresa, IdSucursal, IdProveedor, IdCentroCosto, Fecha_Desde, Fecha_Hasta, MostrarConSaldo, param.IdUsuario);
                ...
                this.DataSource = ListDataRpt.OrderBy(q => q.Fecha_Documento);
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "XCOMP_Rpt010_Rpt_BeforePrint", ex.Message), ex) { EntityType = typeof(XCOMP_Rpt010_Rpt) };
            }
        }
    }
}