...
namespace Core.Erp.Reportes.CuentasxCobrar
{
    public class XCXC_Rpt006_Data
    {
        ...
        public List<XCXC_Rpt006_Info> get_RptCobros(int IdEmpresa, int IdSucursalIni, int IdSucursalFin, decimal IdClienteIni,
            decimal IdClienteFin, DateTime FechaIni, DateTime FechaHasta, List<string> ListIdTipoCobro, string TipoFecha, Boolean fechFilt)
        {
            try
            {
                ...
                using (Entities_CuentasxCobrar listado = new Entities_CuentasxCobrar())
                {
                    ...
                    if (TipoFecha == "x_fechacanc")
                        select = select.Where(q => q.cr_fechaCobro >= FechaIni && q.cr_fechaCobro <= FechaHasta).OrderBy(q => q.pe_nombreCompleto);
                    // rth requerimiento poligrup  .:  .OrderBy(q => q.pe_nombreCompleto);
                    else
                        select = select.Where(q => q.cr_fecha >= FechaIni && q.cr_fecha <= FechaHasta).OrderBy(q => q.pe_nombreCompleto);
                    // rth requerimiento poligrup  .:   .OrderBy(q => q.pe_nombreCompleto);    
                    //select = select.Where(q => q.cr_fecha >= FechaIni && q.cr_fecha <= FechaHasta || q.cr_fechaCobro >= FechaIni && q.cr_fechaCobro <= FechaIni && q.IdCobro_tipo == "CHQF");
                    ...
                    foreach (var item in select)
                    {
                        ...
                        infoRpt.Documento = Convert.ToString(item.IdCbte_vta_nota);
                ...
                if (fechFilt == true)
                {
                    return lstRpt.OrderBy(p => p.cr_fechaCobro).ToList();
                }
                else
                {
                    return lstRpt;
