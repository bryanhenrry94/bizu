...
namespace Core.Erp.Reportes.Controles
{
    public partial class UCCxp_MenuReportes : UserControl
    {
        ...
        sri_impuesto_Bus BusImpuesto = new sri_impuesto_Bus();
        cp_TipoDocumento_Bus BusTipoDocumento = new cp_TipoDocumento_Bus();
        ...
        List<vwtb_persona_beneficiario_Info> List_persona_beneficiario = new List<vwtb_persona_beneficiario_Info>();
        List<sri_impuesto_Info> ListImpuesto = new List<sri_impuesto_Info>();
        List<cp_TipoDocumento_Info> ListTipoDocumento = new List<cp_TipoDocumento_Info>();
        ...
        public Boolean VisibleGrupoBeneficiario
        {
            get { return this.GrupoBeneficiario.Visible; }
            set { this.GrupoBeneficiario.Visible = value; }
        }

        public Boolean VisibleGrupoImpuesto
        {
            get { return this.GrupoImpusto.Visible; }
            set { this.GrupoImpusto.Visible = value; }
        }
        ...
        public DevExpress.XtraBars.BarItemVisibility VisibleCmb_Sub_CentroCosto
        {
        ...
        private void UCCxp_MenuReportes_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                dtpDesde.EditValue = DateTime.Now; //DateTime.Now.AddMonths(-1);
                dtpHasta.EditValue = DateTime.Now; //DateTime.Now.AddMonths(1);
                ...
                List_Persona_Tipo = Bus_Persona_Tipo.Get_List_persona_tipo();
                cmb_persona_tipo.DataSource = List_Persona_Tipo;
                
                ListImpuesto = BusImpuesto.GetList();
                cmbImpuesto.DataSource = ListImpuesto;

                List<string> ListCodTipoDocumento = new List<string>();
                ListCodTipoDocumento.Add("01"); // Factura
                ListCodTipoDocumento.Add("02"); // Nota de Venta
                ListCodTipoDocumento.Add("03"); // Liquidacion Compra
                ListCodTipoDocumento.Add("04"); // Nota de Credito
                ListCodTipoDocumento.Add("15"); // Comprobante de vendta el Exterior


                ListTipoDocumento = BusTipoDocumento.Get_List_TipoDocumento(ListCodTipoDocumento);
                cmbCodTipoDocumento.DataSource = ListTipoDocumento;
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void beiPersona_Tipo_EditValueChanged(object sender, EventArgs e)
        {
            List_persona_beneficiario = Bus_Persona_Beneficiario.Get_List_PersonaBeneficiario(param.IdEmpresa, Convert.ToString(beiPersona_Tipo.EditValue));
            cmb_beneficiario.DataSource = List_persona_beneficiario;
        }
    }
}