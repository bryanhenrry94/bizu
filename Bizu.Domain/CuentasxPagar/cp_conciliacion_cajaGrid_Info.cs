using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Bizu.Domain.Contabilidad;
using Bizu.Domain.Caja;
using System.Drawing;

namespace Bizu.Domain.CuentasxPagar
{
    public class cp_conciliacion_cajaGrid_Info
    {
        
        //public string Tipo { get; set; }
        //public decimal Proveedor { get; set; }
        //public string NomProveedor { get; set; }
        //public decimal Sustento { get; set; }
        //public string CtaGasto { get; set; }
        //public decimal Autorizacion { get; set; }
        //public string seriAutorizacion { get; set; }
        //public string NDocumento { get; set; }
        //public DateTime FechaDocumento { get; set; }
        //public DateTime FechaCaducidad { get; set; }

        //public string co_serie { get; set; }

        //public double Total0 { get; set; }
        //public double Total12 { get; set; }
        //public double IVA { get; set; }
        //public double  Retencion { get; set; }

        //public double BaseImponible { get; set; }
        //public double Descuento { get; set; }
        //public double Total { get; set; }
        
        
        //public cp_retencion_Info CabRetencion { get; set; }

        //public ct_Cbtecble_Info CbteCble_Ret { get; set; }

        //public decimal IdPersona { get; set; }
        //public cp_proveedor_Info InfoProveedor { get; set; }
        //public caj_parametro_Info CajParametro { get; set; }

        //public decimal indice { get; set; }
        //public string Observacion { get; set; }
        //public ct_Cbtecble_Info CbteCble_OG { get; set; }
        //public cp_orden_giro_Info Info_Orden_Giro { get; set; }

        //public int IdEmpresa_OGiro { get; set; }
        //public int IdTipoCbte_Ogiro { get; set; }
        //public decimal IdCbteCble_Ogiro { get; set; }

        //public int IdEmpresa_movcaja { get; set; }
        //public decimal IdCbteCble_movcaja { get; set; }
        //public int IdTipocbte_movcaja { get; set; }

        //public ct_Cbtecble_Info CbteCble_Caja { get; set; }
        //public  caj_Caja_Movimiento_Info MoviCaja { get; set; }
        //public string CtaProveedor { get; set; }
        //public decimal? TipoFujo { get; set; }
        //public string IdCentroCosto { get; set; }
        //public string GeneraRetencion { get; set; }

        //public string EstaEnBase { get; set; }


        //public string IdCtaCble { get; set; }
        //public decimal IdConciliacionCaja { get; set; }

        //public Bitmap Ico { get; set; }

        //public Boolean Icono_Buscar { get; set; }

        //public string NomSubCentroCosto { get; set; }
        //public string IdCentroCosto_sub_centro_costo { get; set; }
        //public int? IdTipoMovi { get; set; }

        //public double Saldo_cont_al_periodo { get; set; }
        //public double Ingresos { get; set; }
        //public double Total_Ing { get; set; }
        //public double Total_fact_vale { get; set; }
        //public double Total_fondo { get; set; }
        //public double Dif_x_pagar_o_cobrar { get; set; }
        //public int IdPeriodo { get; set; }

        //public cp_conciliacion_cajaGrid_Info()       
        //{
           
        //}

    }
}

/// <summary>
/// Prog: Katiuska Franco
/// Ult Mod: 14/02/2014 10:45
/// </summary>