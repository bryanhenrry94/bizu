    ...
    public class com_solicitud_compra_Bus
    {
        ...
        public Boolean AnularDB(com_solicitud_compra_Info info, ref string msg)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "AnularDB", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public Boolean GuardarDB(com_solicitud_compra_Info info, ref string msg)
        {
            try
            {
                ...
                if (validarObjeto(info, ref msg))
                {
                    ...
                    // SI EXISTE DETALLE DE OC X SOLICITUD
                    if(info.list_compra_x_solicitud.Count() > 0)
                    {
                        foreach(var item in info.list_compra_x_solicitud)
                        {
                            item.IdEmpresa_sc = info.IdEmpresa;
                            item.IdSucursal_sc = info.IdSucursal;
                            item.IdSolicitudCompra_sc = info.IdSolicitudCompra;
                        }

                        com_ordencompra_local_x_com_solicitud_compra_Bus bus_oc_x_solicitud = new com_ordencompra_local_x_com_solicitud_compra_Bus();

                        if(!bus_oc_x_solicitud.Grabar(info.list_compra_x_solicitud, ref mensaje))
                        {
                            // MENSAJE DE ERROR
                        }
                    }

                    // SI EXISTE DETALLE DE OC_DET X SOLICITUD_DET
                    if (info.list_compra_det_x_solicitud_det.Count() > 0)
                    {
                        foreach (var item in info.list_compra_det_x_solicitud_det)
                        {
                            item.scd_IdEmpresa = info.IdEmpresa;
                            item.scd_IdSucursal = info.IdSucursal;
                            item.scd_IdSolicitudCompra = info.IdSolicitudCompra;
                        }

                        com_ordencompra_local_det_x_com_solicitud_compra_det_Bus bus_oc_det_x_solicitud_det = new com_ordencompra_local_det_x_com_solicitud_compra_det_Bus();

                        if (!bus_oc_det_x_solicitud_det.GrabarDB(info.list_compra_det_x_solicitud_det, ref mensaje))
                        {
                            // MENSAJE DE ERROR
                        }
                    }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GuardarDB", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public Boolean ModificarDB(com_solicitud_compra_Info info, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDB", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public List<com_solicitud_compra_Info> Get_List_Solicitud_x_OC(int IdEmpresa, int IdSucursal, string IdOrdenCompra)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "validarObjeto", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
            ...
            , string IdEstadoAprobacion = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_solicitud_compra", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public Boolean ModificarDBEstadoAprobacion(com_solicitud_compra_Info info, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDB_EstadoAprobacion", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public List<com_solicitud_compra_Info> Get_List_solicitud_compra_Aprobadas(int idempresa, int idsucursal)
        {
            try
            {
                return odata.Get_List_solicitud_compra_Aprobadas(idempresa, idsucursal);
            }
            catch (Exception ex)
            {

                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_solicitud_compra_Aprobadas", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }

        public Boolean ModificarDB_EstadoAprobacion(com_solicitud_compra_Info info, ref string msg)
        {
            try
            {
                return odata.ModificarDB_EstadoAprobacion(info);
        ...
        public Boolean ModificarDBEstadoAprobacion(List<com_solicitud_compra_Info> listInfo, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDBEstadoAprobacion", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_Bus) };
            }
        }
        ...
        public List<com_solicitud_compra_Info> Get_List_Solicitud_Apro(int IdEmpresa, int IdSucursal, int IdSolicitud)
        {
            try
            {
                return odata.Get_List_Solicitud_Apro(IdEmpresa, IdSucursal, IdSolicitud);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
