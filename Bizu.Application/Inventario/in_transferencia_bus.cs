...
namespace Bizu.Application.Inventario
{
    public class in_transferencia_bus
    {
        ...
        private void Generar_inventario(in_transferencia_Info info, ref decimal _idTransferencia)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Generar_inventario", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
            }   
        }
        ...
        private in_Ing_Egr_Inven_Info Get_Info_Ing_Egr_Inven(in_transferencia_Info info, string Signo)
        {
            try
            {
                ...
                switch (Signo)
                {
                    case "-":
                        ...
                        {
                            if (info.IdCentroCosto_Ing_Egr_Inven_Origen != null)
                                item.IdCentroCosto = info.IdCentroCosto_Ing_Egr_Inven_Origen;
                        }
                    ...
                    case "+":
                        ...
                        foreach (var item in info_IngEgr.listIng_Egr)
                        {
                            if (info.IdCentroCosto_Ing_Egr_Inven_Destino != null)
                                item.IdCentroCosto = info.IdCentroCosto_Ing_Egr_Inven_Destino;
                        }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "get_Ing_Egr_Inven", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
        ...
        private List<in_Ing_Egr_Inven_det_Info> GetDetalle_Ing_Egr_Inven(List<in_transferencia_det_Info> listDetalle, int IdSucursal, int IdBodega, string Signo, DateTime fecha)
        {
            try
            {
                ...
                foreach (var item in listDetalle)
                {
                    ...
                    {
                        case "-":
                            info.IdMotivo_Inv = item.IdMotivo_Inv_SucuOrig;
                            break;
                        case "+":
                            info.IdMotivo_Inv = item.IdMotivo_Inv_SucuDest;
                            break;
                    }

                    #region Control de inventario por Lote

                    in_Parametro_Bus Bus_Parametro_Inv = new in_Parametro_Bus();
                    in_Parametro_Info Info_Parametro_Inv = new in_Parametro_Info();

                    Info_Parametro_Inv = Bus_Parametro_Inv.Get_Info_Parametro(param.IdEmpresa);

                    if (Convert.ToBoolean(Info_Parametro_Inv.P_Maneja_Lote))
                    {
                        in_producto_Bus Bus_Producto = new in_producto_Bus();

                        if (Bus_Producto.Producto_maneja_lote(item.IdEmpresa, item.IdProducto))
                        {
                            info.Lote = item.Lote;
                        }
                    }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GetDetalle_Ing_Egr_Inven", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
            }
        }
        ...
        public in_transferencia_Info Get_Info_transferencia(in_transferencia_Info info, int idEmpresa)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerCabecera", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
        ...
        public in_transferencia_Info Get_Info(int idEmpresa, int IdSucursal, int IdBodega, decimal IdTransferencia)
        {
            try
            {
                return oData_Transferencia.Get_Info(idEmpresa, IdSucursal, IdBodega, IdTransferencia);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public List<in_transferencia_det_Info> ObtenerDetalle(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdTransferencia)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerDetalle", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
        ...
        public bool ModificarDB(in_transferencia_Info info)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDB", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
            }
        ...
        public List<in_transferencia_Info> ObtenerTransferencias(int idempresa, DateTime FechaIni, DateTime FechaFin, int idSucursal, int idBodega)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerTransferencias", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
        ...
        public Boolean Anular(in_transferencia_Info info,DateTime fecha_Anulacion,ref string mensaje)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Anular", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
            }
        }
        ...
        public decimal GetId(int IdEmpresa, int IdSucursal, int IdBodega)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GetId", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
        ...
        public List<in_transferencia_det_Info> Get_List_transferencia_det(in_transferencia_Info Info, int idEmpresa)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerTransferenciasDetalle", ex.Message), ex) { EntityType = typeof(in_transferencia_bus) };
