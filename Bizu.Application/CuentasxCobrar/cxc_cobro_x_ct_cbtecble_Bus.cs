...
namespace Bizu.Application.CuentasxCobrar
{
    public class cxc_cobro_x_ct_cbtecble_Bus
    {
        ...
        public cxc_cobro_x_ct_cbtecble_Info Get_List_cobro_x_ct_cbtecble_x_Reverso(int IdEmpresa, int IdSucursal, decimal IdCobro)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_cobro_x_ct_cbtecble_x_Reverso", ex.Message), ex) { EntityType = typeof(cxc_cobro_x_ct_cbtecble_Bus) };
            }
        }
