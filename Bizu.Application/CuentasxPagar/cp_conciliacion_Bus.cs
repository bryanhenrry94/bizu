    ...
    public class cp_conciliacion_Bus
    {
        ...
        public List<cp_conciliacion_Info> Get_List_conciliacion(int IdEmpresa, ref string mensaje)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_conciliacion", ex.Message), ex) { EntityType = typeof(cp_conciliacion_Bus) };
            }
        }

        public List<cp_conciliacion_Info> Get_List_conciliacion_General(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, ref string mensaje)
        {
            try
            {
                return Data.Get_List_conciliacion_General(IdEmpresa, FechaIni, FechaFin, ref mensaje);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public Boolean GrabarDB(ref cp_conciliacion_Info Info, ct_Cbtecble_Info CbteCble_I, ref string mensaje)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GrabarDB", ex.Message), ex) { EntityType = typeof(cp_conciliacion_Bus) };
            }
        }
        ...
        public Boolean ModificarDB(cp_conciliacion_Info Info, ref string mensaje)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDB", ex.Message), ex) { EntityType = typeof(cp_conciliacion_Bus) };
            }
        }
