...
namespace Bizu.Application.Facturacion
{
    public class fa_pedido_Bus
    {
        ...
        public List<fa_pedido_Info> Get_List_pedido2(int idempresa, DateTime fecha_ini, DateTime fecha_fin, int idsucursal, int idbodega)
        {
            try
            {

                List<fa_pedido_Info> lM = new List<fa_pedido_Info>();
                return data.Get_List_pedido2(idempresa, fecha_ini, fecha_fin, idsucursal, idbodega);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerPedido", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
            }
        }
        ...
        public List<fa_pedido_Info> Get_List_pedido_estado(int idempresa, DateTime fecha_ini, DateTime fecha_fin, int idsucursal, string idEstadoApro)
        {
            try
            {

                List<fa_pedido_Info> lM = new List<fa_pedido_Info>();
                return data.Get_List_pedido_estado(idempresa, fecha_ini, fecha_fin, idsucursal, idEstadoApro);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public fa_pedido_Info Get_Pedido_Venta(int idempresa, int idsucursal, int idbodega, decimal IdPedido)
        {
            try
            {             
                return data.Get_Pedido_Venta(idempresa, idsucursal, idbodega, IdPedido);          
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public List<fa_pedido_Info> Get_List_pedido(int IdEmpresa)
        {
            ...
            catch (Exception ex)
            {
                ...
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerPedidosNoDespachadosxClientes", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
            
            }
        }

        public List<fa_pedido_Info> Get_List_Ventas(int IdEmpresa)
        {

            try
            {
                return data.Get_List_pedido_Venta(IdEmpresa);
            }
            catch (Exception ex)
            {
        ...
        public Boolean VerificarCodigo(string Codigo)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "VerificarCodigo", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public Boolean ModificarDB(fa_pedido_Info info, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ModificarDB", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public Boolean validarObjeto(fa_pedido_Info info,ref string mensajeE)
        {
            try 
            {
                ...
                info.cp_fecha = Convert.ToDateTime(info.cp_fecha);
                info.cp_fechaVencimiento = Convert.ToDateTime(info.cp_fechaVencimiento);
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "validarObjeto", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public Boolean GrabarDB(fa_pedido_Info info, ref decimal id, ref string msg)
        {
            try
            {
                ...
                {                                    
                        ...
                        //val = true;
                        val = data.GrabarDB(info, ref id, ref msg);                                               
                }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GrabarDB", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        private Boolean verificarrepetidos(decimal idempleado, Boolean eliminar, int tipo, fa_pedido_Info Pedido_info)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "verificarrepetidos", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        private Boolean Validar_y_corregir_objeto_Pedido(ref fa_pedido_Info Pedido_info, ref string msg)
        {
            try
            {
                ...
                foreach (var item in Pedido_info.lista_detalle)
                {
                   if(item.IdProducto !=0)
                   {
                    ...
                    var cont = from C in Pedido_info.lista_detalle
                               where C.IdProducto == item.IdProducto && item.IdProducto > 0//C.dp_precio==item.dp_precio
                               select C;
                    if (cont.Count() > 1)
                    {                       
                        msg = "el producto : " + item.DesProduct + " ,Está Repetido 2 veces";
                        return false;
                    }
                    else
                    { }  
                ...
                Pedido_info.cp_fecha = Convert.ToDateTime(Pedido_info.cp_fecha);
                Pedido_info.cp_fechaVencimiento = Convert.ToDateTime(Pedido_info.cp_fechaVencimiento);
                ...
                foreach (var item in Pedido_info.DetformaPago_list)
                {
                    ...
                    item.Fecha_vtc = Convert.ToDateTime(item.Fecha_vtc);
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Validar_y_corregir_objeto_Pedido", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public Boolean AnularDB(fa_pedido_Info info, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Anular", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public List<fa_pedido_det_Info> ObtenerOrdenDespachoxCliente(fa_orden_Desp_Info info)
        {
            ...
            {
                  Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerOrdenDespachoxCliente", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
        ...
        public Boolean ActualizarEstadoApro(List<fa_pedido_Info> Lst, string IdEstadoAprobacion)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ActualizarEstadoApro", ex.Message), ex) { EntityType = typeof(fa_pedido_Bus) };
