...
namespace Bizu.Application.Facturacion
{
    public class fa_notaCredDeb_Bus
    {
        ...
        public List<fa_notaCreDeb_Info> Get_List_notaCreDeb(int idEmpresa, int idSucursalIni, int idSucursalFin, int idBodegaIni, int idBodegaFin, DateTime FechaIni, DateTime FechaFin, string Tipo)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ObtenerLista", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }

        public List<fa_notaCreDeb_Info> Get_List_notaCreDeb(int idEmpresa, int idSucursal, int IdBodega, decimal IdCliente)
        {
            try
            {
                return oData_NotaCredDeb.Get_List_notaCreDeb(idEmpresa, idSucursal, IdBodega, IdCliente);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public List<fa_notaCreDeb_Info> Get_List_notaCreDeb_Obra(int idEmpresa, int idSucursalIni, int idSucursalFin, int idBodegaIni, int idBodegaFin, DateTime FechaIni, DateTime FechaFin, string Tipo)
        {
            try
            {
                return oData_NotaCredDeb.Get_List_notaCreDeb_Obra(idEmpresa, idSucursalIni, idSucursalFin, idBodegaIni, idBodegaFin, FechaIni, FechaFin, Tipo);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public fa_notaCreDeb_Info Get_Info_notaCreDeb_x_ND(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "getNotaCreditoDebitoDet", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }

        public fa_notaCreDeb_Info Get_Info_notaCreDeb_x_ND_Tipo(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota, string CreDeb)
        {
            try
            {
                return oData_NotaCredDeb.Get_Info_notaCreDeb_x_ND_Tipo(IdEmpresa, IdSucursal, IdBodega, IdNota, CreDeb);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
        ...
        public Boolean AnularDB(fa_notaCreDeb_Info oNota, ref string mensajeError)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "AnularNota", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        ...
        public Boolean GuardarDB(fa_notaCreDeb_Info Info, ref decimal idnota, ref string mensajeDocumentoDupli, ref string mensaje, Boolean SeContabiliza = true)
        {
            try
            {
                ...
                in_producto_x_tb_bodega_Costo_Historico_Bus Bus_CostoHistorico = new in_producto_x_tb_bodega_Costo_Historico_Bus();
                in_producto_x_tb_bodega_Costo_Historico_Info Info_CostoHistorico = new in_producto_x_tb_bodega_Costo_Historico_Info();
                ...
                {
                    //BRYAN NAVARRETE 04-04-2018
                    //OBTENEMOS EL ULTIMO COSTO DEL PRODUCTO POR BODEGA PARA ACTUALIZAR EL COSTO DE LA N/C
                    foreach (var item in Info.ListaDetalles)
                    {
                        Info_CostoHistorico = Bus_CostoHistorico.get_UltimoCosto_x_Producto_Bodega(item.IdEmpresa, item.IdSucursal, item.IdBodega,
                            item.IdProducto, DateTime.Today);

                        if (Info_CostoHistorico != null)
                        {
                            item.sc_costo = (Info_CostoHistorico.costo != null) ? Info_CostoHistorico.costo : 0;
                        }

                        //OBTENEMOS EL ULTIMO PESO DEL PRODUCTO PARA ACTUALIZAR LA N/C
                        Info_Producto = Bus_Producto.Get_info_Product(item.IdEmpresa, item.IdProducto);

                        if (Info_Producto != null)
                        {
                            item.sc_Peso = (Info_Producto.pr_peso != null) ? Info_Producto.pr_peso : 0;
                        }
                    }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GuardarDB", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public List<fa_notaCreDeb_Info> ProcesarDataTable_ND_x_Saldo_inicial(DataTable ds, ref string MensajeError)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GetId", ex.Message), ex) { EntityType = typeof(fa_notaCreDeb_Info) };
            }
        ...
        public fa_notaCreDeb_x_cxc_cobro_Info Get_faNotaCreDeb_x_Cobro(cxc_cobro_Info info, fa_notaCreDeb_Info Item)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "get_faNotaCreDeb_x_Cobro", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        ...
        public Boolean VerificarCodigo(string Codigo)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "VerificarCodigo", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public Boolean ActualizarDB(fa_notaCreDeb_Info Info, ref string mensajeError, ref string mensajeDocumentoDupli)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ActualizarDB", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        ...
        public Boolean ContabilizarNCD(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNotaCD)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ContabilizarNCD", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public Boolean ContabilizarNCDREVERSO(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNotaCD)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ContabilizarNCDREVERSO", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public void GeneraCbteCtbl(fa_notaCreDeb_Info Info)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GeneraCbteCtbl", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public void GeneraCbteCtblREVERSO(fa_notaCreDeb_Info Info)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GeneraCbteCtblREVERSO", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public ct_Cbtecble_Info get_CbteCbleInfo(fa_notaCreDeb_Info Info)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "get_CbteCbleInfo", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public List<ct_Cbtecble_det_Info> get_CbteCble_det_x_Items(fa_notaCreDeb_Info Info)
        {
            ...
            try
            {
                ...
                List<vwfa_ContabilizacionNotaCredito_x_descuento_x_item_Info> lst_descuento = new List<vwfa_ContabilizacionNotaCredito_x_descuento_x_item_Info>();
                vwfa_ContabilizacionNotaCredito_x_descuento_x_item_Bus bus_descuento = new vwfa_ContabilizacionNotaCredito_x_descuento_x_item_Bus();
                ...
                {
                    if (ClienInfo.IdCentroCosto_CXC == null)
                    {
                        foreach (var item in DetalleNotaCred)
                        {
                ...
                var lista_agrupada = from q in DetalleNotaCred
                                     group q by new
                                     {
                                         ...
                                         q.IdBodega,
                                         q.IdNota,
                                         q.IdPunto_cargo_grupo,
                                         q.IdPunto_Cargo,
                                         q.IdCentroCosto,
                                         q.IdCtaCble_Vta
                                     }
                                         into grouping
                                         select new { grouping.Key, Subtotal = grouping.Sum(q => q.Sub_total), Desc_vta = grouping.Sum(q => q.Des_total) };
                ...
                foreach (var item in lista_agrupada)
                {
                    ...
                    if (Info.CreDeb.TrimEnd() == "C")
                    {
                        dte.dc_Valor = Math.Round(Convert.ToDouble(item.Subtotal), 2, MidpointRounding.AwayFromZero);
                    }
                    else
                    {
                        dte.dc_Valor = Math.Round(-1 * Convert.ToDouble(item.Subtotal), 2, MidpointRounding.AwayFromZero);
                    }
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "get_CbteCble_det_CATEGORIA", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        ...
        public Boolean GenerarXml_notaCreDeb(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota, string CreDeb, string RutaDescarga, ref string msg)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "GenerarXml_notaCreDeb", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public Boolean ValidarObjeto_XML_notaCreDeb(List<vwfa_notaCreDeb_sri_Info> lista, ref string MensajeError)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "ValidarObjeto_XML_notaCreDeb", ex.Message), ex) { EntityType = typeof(fa_notaCredDeb_Bus) };
            }
        }
        ...
        public Boolean Generar_Cobro_x_NC(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota_Cred, ref string MensajeError)
        {
            try
            {
                ...
                if (list_cbts_vtas_x_cobrar.Count() > 0)
                {
                    ...
                    if (listDet_Nota.Count > 0)
                    {
                        fa_notaCreDeb_det_Info Det_Nota = new fa_notaCreDeb_det_Info();
                        Det_Nota = listDet_Nota.FirstOrDefault();

                        if (Det_Nota.IdCentroCosto == null || Det_Nota.IdCentroCosto == "")
                        {
                            InfoCbro.IdCentroCosto = null;
                        }
                        else
                        {
                            InfoCbro.IdCentroCosto = (Det_Nota.IdCentroCosto == null) ? "" : Det_Nota.IdCentroCosto;
                        }
                    }
                    ...
                    InfoCbro.cr_fecha = Convert.ToDateTime(InfoNota_deb.no_fecha); //Convert.ToDateTime(DateTime.Now);
                    InfoCbro.cr_fechaDocu = Convert.ToDateTime(InfoNota_deb.no_fecha); //Convert.ToDateTime(DateTime.Now);
                    InfoCbro.cr_fechaCobro = Convert.ToDateTime(InfoNota_deb.no_fecha); //Convert.ToDateTime(DateTime.Now);
                    ...
                    foreach (var item in list_cbts_vtas_x_cobrar)
                    {
                        if (item.Valor_Aplicado != 0)
                        {
                            ...
                            Infodet.IdCentroCosto = InfoCbro.IdCentroCosto;
                ...
                if (consulta.ToList().Count != 0)
                {
                    ...
                    if (info_notCredet.Count != 0)
                    {
                        ...
                        foreach (var item in consulta)
                        {
                            ...
                            myObject.infoNotaCredito.contribuyenteEspecial = (item.ContribuyenteEspecial == null) ? null : item.ContribuyenteEspecial.Trim();
                            ...
                            myObject.infoNotaCredito.fechaEmisionDocSustento = Convert.ToDateTime(item.fechaEmisionDocSustento).ToString(format);

                            switch (param.IdCliente_Ven_x_Default)
                            {
                                case Cl_Enumeradores.eCliente_Vzen.EDEHSA:
                                    myObject.infoNotaCredito.motivo = item.nc_Motivo;
                            ...
                            foreach (var item2 in info_notCredSRI.lista_DetCreDeb)
                            {
                                ...
                                NCdetalle.descuento = Convert.ToDecimal(Math.Round(descuento_total, 2));
                                NCdetalle.precioTotalSinImpuesto = Math.Round(Convert.ToDecimal(subtotal), 2);

                                //Detalle adicional x item
                                if (string.IsNullOrEmpty(item2.DetallexItems) == false)
                                {
                                    NCdetalle.detallesAdicionales = new List<notaCreditoDetalleDetAdicional>();
                                    notaCreditoDetalleDetAdicional det_adicional = new notaCreditoDetalleDetAdicional();
                                    det_adicional.nombre = "Detalle";
                                    det_adicional.valor = item2.DetallexItems.Trim();
                                    NCdetalle.detallesAdicionales.Add(det_adicional);
                                }
                                ...
                                if (param.IdCliente_Ven_x_Default == Cl_Enumeradores.eCliente_Vzen.EDEHSA)
                                {
                                    ...
                                    Info_Producto = Bus_Producto.Get_info_Product(item2.IdEmpresa, item2.IdProducto);

                                    if (Info_Producto != null)
                                    {
                                        if (string.IsNullOrEmpty(Info_Producto.IdUnidadMedida) == false)
                                        {
                                ...
                                switch (item2.vt_por_iva)
                                {
                                    ...
                                    case 15:
                                        ...
                                        impue.codigoPorcentaje = "4";
                                        impue.tarifa = 15;
                                        break;
                                    case 5:
                                        impuesto.codigoPorcentaje = "5";

                                        impue.codigoPorcentaje = "5";
                                        impue.tarifa = 5;
                                        break;
                                }
                            ...
                            foreach (var item3 in resul_Impuesto)
                            {
                                ...
                                impuesto.baseImponible = Math.Round(item3.baseImponible, 2);
                ...
                if (consulta.ToList().Count != 0)
                {
                    ...
                    if (info_notCredet.Count != 0)
                    {
                        ...
                        foreach (var item in consulta)
                        {
                            ...
                            myObject.infoTributaria = new Domain.class_sri.FacturaV2.infoTributaria();
                            ...
                            myObject.infoNotaDebito.fechaEmisionDocSustento = Convert.ToDateTime(item.fechaEmisionDocSustento).ToShortDateString();
                            ...
                            myObject.infoNotaDebito.impuestos = new List<Domain.class_sri.FacturaV2.impuesto>();

                            List<notaDebitoMotivosMotivo> lista_motivo = new List<notaDebitoMotivosMotivo>();

                            foreach (var item2 in info_notCredSRI.lista_DetCreDeb)
                            {
                                double subtotal = item2.sc_cantidad * item2.sc_Precio;
                                sum_totsinImp = sum_totsinImp + subtotal;
        ...
        public byte[] Get_Comprobante_Efirm_PDF(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota, ref string IdComprobante, ref string mensaje_error)
        {
            try
            {
                tb_Comprobantes_Efirm_Bus Bus_Comprobante = new tb_Comprobantes_Efirm_Bus();
                fa_Cliente_Bus Bus_Cliente = new fa_Cliente_Bus();
                fa_Cliente_Info Info_Cliente;
                fa_notaCreDeb_Info Info_NotaDebCed;
                byte[] result = null;
                int IdEmpresa_Efirm = 0;

                Info_NotaDebCed = Get_Info_notaCreDeb_x_ND(IdEmpresa, IdSucursal, IdBodega, IdNota);

                if (Info_NotaDebCed != null)
                {
                    Info_Cliente = Bus_Cliente.Get_Info_Cliente(Info_NotaDebCed.IdEmpresa, Info_NotaDebCed.IdCliente);

                    IdComprobante = Cl_Enumeradores.eTipoComprobante_Alias.NC + "-" + Info_NotaDebCed.Serie1 + "-" + Info_NotaDebCed.Serie2 + "-" + Info_NotaDebCed.NumNota_Impresa;

                    IdEmpresa_Efirm = Bus_Comprobante.Get_IdEmpresa_x_Ruc(param.InfoEmpresa.em_ruc, ref mensaje_error);

                    result = Bus_Comprobante.Get_RidePdf(IdEmpresa_Efirm, Info_Cliente.Persona_Info.pe_cedulaRuc, IdComprobante, ref mensaje_error);
                }
                else
                {
                    mensaje = "No se encontraron registros para el comprobante #" + IdNota;
                }
            ...
            catch (Exception ex)
            {
                ...
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_factura", ex.Message), ex) { EntityType = typeof(fa_factura_Bus) };
            }
        }

        public string Get_Comprobante_Efirm_XML(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdNota, ref string IdComprobante, ref string mensaje_error)
        {
            try
            {
                tb_Comprobantes_Efirm_Bus Bus_Comprobante = new tb_Comprobantes_Efirm_Bus();
                ...
                if (Info_NotaDebCed != null)
                {
                    ...
                    result = Bus_Comprobante.Get_Xml(IdEmpresa_Efirm, Info_Cliente.Persona_Info.pe_cedulaRuc, IdComprobante, ref mensaje_error);
                }
                else
                {
                    mensaje = "No se encontraron registros para el comprobante #" + IdNota;
