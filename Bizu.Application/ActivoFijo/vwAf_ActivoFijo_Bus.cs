    ...
    public class vwAf_ActivoFijo_Bus
    {
        ...
        public List<vwAf_ActivoFijo_Info> Get_List_ActivoFijo_Depre(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, string IdUsuario)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_ActivoFijo_Depre", ex.Message), ex) { EntityType = typeof(vwAf_ActivoFijo_Bus) };
            }
        }

        public List<vwAf_ActivoFijo_Info> Get_List_ActivoFijo_Depre_Mes_Contable(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, string IdUsuario)
        {
            try
            {
                return dataAf.Get_List_ActivoFijo_Depre_Mes_Contable(IdEmpresa, FechaIni, FechaFin, IdUsuario);
            }
            catch (Exception ex)
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "Get_List_ActivoFijo_Depre_Mes_Contable", ex.Message), ex) { EntityType = typeof(vwAf_ActivoFijo_Bus) };
            }
        }
        ...
        private List<vwAf_ActivoFijo_Info> Get_List_ActivoFijo_Depre(List<vwAf_ActivoFijo_Info> lstInfo, DateTime FechaIni)
        {
            ...
            {
                Bizu.Domain.Log_Exception.LoggingManager.Logger.Log(Bizu.Domain.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Bizu.Domain.Log_Exception.DalException(string.Format("", "CalcularValoresDepre", ex.Message), ex) { EntityType = typeof(vwAf_ActivoFijo_Bus) };
            }
        }
