            ...
            try
            {
                ...
                foreach (var item in Info.ListaDetalles)
                {
                    using (EntitiesFacturacion Context = new EntitiesFacturacion()) 
                    {
                        ...
                        Address.sc_Peso = item.sc_Peso; //br 09-05-2018                        
                        ...
                        Address.IdCod_Impuesto_Iva = (item.IdCod_Impuesto_Iva == null) ? "IVA0" : item.IdCod_Impuesto_Iva;
        ...
        public List<fa_notaCreDeb_det_Info> Get_List_notaCreDeb_det(fa_notaCreDeb_Info Info)
        {
            try
            {
                ...
                foreach ( var item  in cons)
                {
                    ...
                    infogrid.sc_cantidad = item.sc_cantidad;// (float)item.sc_cantidad;
                    infogrid.sc_descUni = item.sc_descUni;// (float)item.sc_descUni;
                    infogrid.sc_observacion = item.sc_observacion;
                    infogrid.sc_Peso = item.sc_Peso; //br 09-05-2018
                    infogrid.sc_iva = item.sc_iva;
                    infogrid.sc_PordescUni = item.sc_PordescUni;// (float)item.sc_PordescUni;
                    infogrid.sc_Precio = item.sc_Precio; //(float)item.sc_Precio;
                    infogrid.sc_precioFinal = item.sc_precioFinal; //(float)item.sc_precioFinal;
