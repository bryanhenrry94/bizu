    ...
    public class tb_transportista_Data
    {
            ...
            try
            {
                using (EntitiesGeneral oEnti = new EntitiesGeneral())
                {
                    ...
                    if (consulta.ToList().Count == 0)
                    {
                        ...
                        registo.IdTipoDocumento = Info.IdTipoDocumento;
                        ...
                        registo.Fecha_Transaccion = Info.Fecha_Transaccion;
        ...
        public Boolean ModificarDB(tb_transportista_Info Info)
        {
            try
            {
                using (EntitiesGeneral oEnti = new EntitiesGeneral())
                {
                    ...
                    if (registro != null)
                    {
                        registro.IdTipoDocumento = Info.IdTipoDocumento;
            ...
            try
            {
                using (EntitiesGeneral conexion = new  EntitiesGeneral())
                {
                    ...
                    var item = (from q in conexion.tb_transportista
                                ...
                                select new tb_transportista_Info() { IdEmpresa = q.IdEmpresa, Nombre = q.Nombre, Cedula = q.Cedula, Estado = q.Estado , IdTransportista = q.IdTransportista, IdTipoDocumento = q.IdTipoDocumento, Placa = q.Placa  }).FirstOrDefault();
                    return item;
        ...
        public tb_transportista_Info Get_Info_Transportista(int IdEmpresa, decimal IdTransportista)
        {
            ...
            try
            {
                using (EntitiesGeneral conexion = new EntitiesGeneral())
                {
                    var Address = (from q in conexion.tb_transportista
                                where q.IdEmpresa == IdEmpresa 
                                && q.IdTransportista == IdTransportista
                                select q).First();

                    if (Address != null)
                    {
                        Info.IdEmpresa = Address.IdEmpresa;
                        Info.IdTransportista = Address.IdTransportista;
                        Info.IdTipoDocumento = Address.IdTipoDocumento;
                        Info.Cedula = Address.Cedula;
                        Info.Nombre = Address.Nombre;
                        Info.Estado = Address.Estado;
                        Info.IdUsuario = Address.IdUsuario;
                        Info.Fecha_Transaccion = Address.Fecha_Transaccion;
                        Info.IdUsuarioUltModi = Address.IdUsuarioUltModi;
                        Info.Fecha_UltMod = Address.Fecha_UltAnu;
                        Info.IdUsuarioUltAnu = Address.IdUsuarioUltAnu;
                        Info.Fecha_UltAnu = Address.Fecha_UltAnu;
                        Info.MotivoAnulacion = Address.MotivoAnulacion;
                        Info.Placa = Address.Placa;
                    }
                }
            }
            catch (Exception ex)
