    ...
    public class cp_orden_pago_Data
    {
        ...
        public bool Eliminar_OrdenPago(int IdEmpresa, decimal IdOrdenPago, ref string mensaje)
        {
            try
            {
                ...
                {
                    Context.Database.ExecuteSqlCommand("delete from cp_orden_pago_det where IdEmpresa = " + IdEmpresa + " and IdOrdenPago = " + IdOrdenPago);
                    Context.Database.ExecuteSqlCommand("delete from cp_orden_pago where IdEmpresa = " + IdEmpresa + " and IdOrdenPago = " + IdOrdenPago);
                }
        ...
        public cp_orden_pago_Info Get_Info_orden_pago(int IdEmpresa, decimal IdOrdenPago, ref string mensaje)
        {
            try
            {
                ...
                foreach (var _pago in Orden_Pago)
                {
                    info.IdEmpresa = _pago.IdEmpresa;
                    info.IdOrdenPago = _pago.IdOrdenPago;
                    info.Observacion = _pago.Observacion;
                    info.IdTipo_op = _pago.IdTipo_op;
                    info.Fecha = _pago.Fecha;
                    info.IdFormaPago = _pago.IdFormaPago;
                    info.Fecha_Pago = _pago.Fecha_Pago;
                    info.IdBanco = _pago.IdBanco;
                    info.IdEstadoAprobacion = _pago.IdEstadoAprobacion;
                    info.Estado = _pago.Estado;
                    info.IdPersona = _pago.IdPersona;
                    info.IdTipo_Persona = _pago.IdTipo_Persona;
                    info.IdEntidad = Convert.ToDecimal(_pago.IdEntidad);
                    info.IdUsuario = _pago.IdUsuario;
                    info.nom_pc = _pago.nom_pc;
                    info.ip = _pago.ip;
                    info.IdTipoFlujo = _pago.IdTipoFlujo;
                }
        ...
        public List<cp_orden_pago_Info> Get_List_orden_pago(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, ref string mensaje)
        {
            try
            {
                ...
                foreach (var _pago in Orden_Pago)
                {
                    cp_orden_pago_Info item = new cp_orden_pago_Info();
                    item.IdEmpresa = _pago.IdEmpresa;
                    item.IdOrdenPago = _pago.IdOrdenPago;
                    item.Observacion = _pago.Observacion;
                    item.IdTipo_op = _pago.IdTipo_op;
                    item.IdEntidad = Convert.ToDecimal(_pago.IdEntidad);
                    item.Fecha = _pago.Fecha;
                    item.IdEstadoAprobacion = _pago.IdEstadoAprobacion;
                    item.IdFormaPago = _pago.IdFormaPago;
                    item.Fecha_Pago = _pago.Fecha_Pago;
                    item.IdBanco = Convert.ToInt32(_pago.IdBanco);
                    item.Estado = _pago.Estado;
                    item.IdPersona = _pago.IdPersona;
                    item.IdTipo_Persona = _pago.IdTipo_Persona;
                    item.pe_nombreCompleto = _pago.pe_nombreCompleto;
                    item.Total_OP = Convert.ToDecimal(_pago.Total_OP);
                    item.Total_cancelado = Convert.ToDecimal(_pago.Total_cancelado);
                    item.Saldo = Convert.ToDecimal(_pago.Saldo);
                    item.IdTipoFlujo = _pago.IdTipoFlujo;
                    item.EstadoCancelacion = _pago.EstadoCancelacion;
                    item.Descripcion = _pago.Descripcion;
                    item.co_factura = _pago.co_factura;
                    item.co_FechaFactura = _pago.co_FechaFactura;
        ...
        public List<cp_orden_pago_Info> Get_List_orden_pago(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, string IdUsuario, ref string mensaje)
        {
            try
            {
                ...
                var query = from selec in ECXP.spCXP_orden_pago(IdEmpresa, FechaIni, FechaFin, IdUsuario)
                                 select selec;

                foreach (var _pago in query)
                {
                    cp_orden_pago_Info item = new cp_orden_pago_Info();
                    item.IdEmpresa = _pago.IdEmpresa;
                    item.IdOrdenPago = _pago.IdOrdenPago;
                    ...
                    item.co_factura = _pago.co_factura;
                    item.co_FechaFactura = _pago.co_FechaFactura;
                    item.IdUsuario = _pago.IdUsuario;
        ...
        public List<vwcp_Anticipos_x_NotaCred_Saldo_Info> Get_List_Orden_Pago_Anticipos_con_Saldos_Pagados(int IdEmpresa, decimal IdProveedor, ref string mensaje)
        {
            try
            {
                ...
                var query = from selec in ECXP.spCXP_Anticipos_x_NotaCred_Saldo(IdEmpresa, IdProveedor)
                                 where selec.valor_saldo_cbte > 0.004
                                 select selec;
        ...
        public decimal GetIdOrdenPago(int IdEmpresa)
        {
            try
            {
                ...
                {
                    Id = (from t in ECXP.cp_orden_pago
                                   where t.IdEmpresa == IdEmpresa
                                   select t.IdOrdenPago).Max() + 1;
                }
        ...
        public Boolean ModificarDB_FormaPago(cp_orden_pago_Info Info, ref string msg)
        {
            try
            {
                using (EntitiesCuentasxPagar context = new EntitiesCuentasxPagar())
