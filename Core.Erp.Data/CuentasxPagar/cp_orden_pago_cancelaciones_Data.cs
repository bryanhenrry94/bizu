    ...
    public class cp_orden_pago_cancelaciones_Data
    {
            ...
            try
            {
                ...
                using (EntitiesCuentasxPagar cxp = new EntitiesCuentasxPagar())
                {
                    ...
                    foreach (var item in consulta)
                    {
                        ...
                        info.FechaCancelacion = item.FechaCancelacion;
            ...
            try
            {
                ...
                using (EntitiesCuentasxPagar cxp = new EntitiesCuentasxPagar())
                {
                    ...
                    foreach (var item in consulta)
                    {
                        ...
                        info.IdOrdenPago_op = item.IdOrdenPago_op;
                        info.IdEmpresa_op = Convert.ToInt32(item.IdOrdenPago_op);
                        info.Secuencia_op = item.Secuencia_op;
            ...
            try
            {
                ...
                foreach (var Item in Lst)
                {
                    using (EntitiesCuentasxPagar Context = new EntitiesCuentasxPagar())
                    {
                        ...
                        Item.Idcancelacion = IdCancelacion;
                        ...
                        Cabe.FechaCancelacion = Item.FechaCancelacion;
            ...
            try
            {
                decimal IdCancelacion = GetIdcancelacion(Id);
                    ...
                    using (EntitiesCuentasxPagar Context = new EntitiesCuentasxPagar())
                    {
                        ...
                        Cabe.FechaCancelacion = Info.FechaCancelacion;
        ...
        public Boolean ModificarDB(List<cp_orden_pago_cancelaciones_Info> info)
        {
            try
            {
                foreach (var Item in info)
                {
                    using (EntitiesCuentasxPagar Context = new EntitiesCuentasxPagar())
                    {
                        string sql = "DELETE FROM cp_orden_pago_cancelaciones WHERE IdEmpresa = {0} AND Idcancelacion = {1} AND Secuencia = {2} AND IdEmpresa_op = {3} AND IdOrdenPago_op = {4} AND Secuencia_op = {5}";
                        Context.Database.ExecuteSqlCommand(sql, Item.IdEmpresa, Item.Idcancelacion, Item.Secuencia, Item.IdEmpresa_op, Item.IdOrdenPago_op, Item.Secuencia_op);
                    }
                }
                return true;
            }
        ...
        public bool Eliminar_OrdenPagoCancelaciones_List(List<cp_orden_pago_cancelaciones_Info> Lst, int Id, ref string mensaje)
        {
            try
            {
                ...
                foreach (var Item in Lst)
                {
                    using (EntitiesCuentasxPagar Context = new EntitiesCuentasxPagar())
                    {
                        Context.Database.ExecuteSqlCommand("delete from cp_orden_pago_cancelaciones where IdEmpresa = " + Item.IdEmpresa + " and Idcancelacion = " + Item.Idcancelacion);
                    }
                }
                return true;
            }
        ...
        public bool Eliminar_OrdenPagoCancelaciones(int IdEmpresa_pago, int IdTipoCbte_pago, decimal IdCbteCble_pago, ref string mensaje)
        {
            ...
            {               
                    using (EntitiesCuentasxPagar Context = new EntitiesCuentasxPagar())
                    {
                        Context.Database.ExecuteSqlCommand("delete from cp_orden_pago_cancelaciones where IdEmpresa_pago = " + IdEmpresa_pago + " and IdTipoCbte_pago = " + IdTipoCbte_pago + " and IdCbteCble_pago=" + IdCbteCble_pago);
                    }
