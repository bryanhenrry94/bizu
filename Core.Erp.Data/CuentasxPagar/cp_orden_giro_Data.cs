    ...
    public class cp_orden_giro_Data
    {
        ...
        public List<cp_orden_giro_Info> Get_List_orden_giro(int IdEmpresa, DateTime F_inicio, DateTime F_fin)
        {
            try
            {
                ...
                var select_ = (from T in Base.vwcp_orden_giro_x_Pagos_saldo
                               ...
                               join ctd in Base.cp_TipoDocumento on T.IdOrden_giro_Tipo equals ctd.CodTipoDocumento
                ...
                foreach (var item in select_)
                {
                    ...
                    dat.Tiene_ingresos = Convert.ToBoolean(item.Tiene_ingresos);
                    dat.TipoDocumento = item.Descripcion;

                    // Si es Liquidacion de Compra
                    if(dat.IdOrden_giro_Tipo == "03")
                    {
                        dat.Tiene_ride = true;
                        dat.Tiene_xml = true;
                    }
        ...
        public List<cp_orden_giro_Info> Get_List_orden_giro_SinIngresos(int IdEmpresa, DateTime F_inicio, DateTime F_fin)
        {
            try
            {
                ...
                var select_ = (from T in Base.vwcp_orden_giro_x_Pagos_saldo_SinIngresos
                               join P in Base.cp_proveedor on new { T.IdProveedor, T.IdEmpresa } equals new { P.IdProveedor, P.IdEmpresa }
                               join C in Base.cp_proveedor_clase on new { P.IdEmpresa, P.IdClaseProveedor } equals new { C.IdEmpresa, C.IdClaseProveedor }
                               where T.IdEmpresa == IdEmpresa
                               && T.co_fechaOg >= F_inicio && T.co_fechaOg <= F_fin
        ...
        public cp_orden_giro_Info Get_Info_orden_giro_Saldo(int IdEmpresa, int IdTipoCbte_Ogiro, decimal IdCbteCble_Ogiro)
        {
            try
            {
                ...
                var select_ = (from T in Base.vwcp_orden_giro_x_Pagos_saldo
                               ...
                               join C in Base.cp_proveedor_clase on new { P.IdEmpresa, P.IdClaseProveedor } equals new { C.IdEmpresa, C.IdClaseProveedor }
                               where T.IdEmpresa == IdEmpresa
                               && T.IdTipoCbte_Ogiro == IdTipoCbte_Ogiro
                               && T.IdCbteCble_Ogiro == IdCbteCble_Ogiro
                               orderby T.IdCbteCble_Ogiro descending
                               select new
                               {
                                   T.em_nombre,
            ...
            try
            {
                ...
                if (!Mostrar_fact_conci_caja)
                {
                    select_ = from T in Base.vwcp_orden_giro_x_pagar
                              ...
                              ///SE COMENTA PARA Q APAREZCA EN APROB DE FACT X PAGAR PERO PUEDE UN ERROR DE DUBLICACION 
                              //POR QUE PUEDE DARSE Q EL USUARIO APESAR DE HABER CONCILIADO VAYA Y VUELVA A PAGAR ES FACTURA POR ESTE FILTRO NO APLICA
                              //DEBIDO A Q LA FACTURA ESTA CONCILIADA Y NO CERRADA
                              orderby T.IdCbteCble_Ogiro descending
                              select T;
                }
                else
        ...
        public List<cp_orden_giro_Info> Get_List_orden_giro_SRI(int IdEmpresa, int anio, int mes)
        {
            try
            {
                ...
                var select_ = from T in Base.vwcp_orden_giro_SRI
                              ...
                              && T.co_vaCoa == "S"
                              //&& T.cp_es_comprobante_electronico != true
                              select T;
        ...
        public List<cp_orden_giro_Info> Get_List_orden_giro_SRI_Factura_X_Proveedor(int IdEmpresa, string CedRuc)
        {
            try
            {
                ...
                foreach (var item in select_)
                {
                    ...
                    dat.FactComp = item.co_serie.Trim() + "-" + item.co_factura.Trim();
        ...
        public List<cp_orden_giro_Info> Get_List_orden_giro_NumComprobante(int IdEmpresa)
        {
            try
            {
                ...
                var select_ = from selec in Base.vwcp_orden_giro_SRI
                              where selec.IdEmpresa == IdEmpresa
                              select new { selec.IdEmpresa, selec.IdProveedor, selec.pe_cedulaRuc, selec.co_serie, selec.co_factura };
                ...
                foreach (var item in select_)
                {
                    ...
                    dat.serie1 = Convert.ToString(item.co_serie.Split('-')[0]);
                    dat.serie2 = Convert.ToString(item.co_serie.Split('-')[1]);
                    dat.co_factura = item.co_factura;
        ...
        public Boolean GrabarDB(cp_orden_giro_Info item, ref string mensaje)
        {
            try
            {
                using (EntitiesCuentasxPagar context = new EntitiesCuentasxPagar())
                {
                    ...
                    dat.Archivo_PDF = item.Archivo_PDF;
        ...
        public Boolean ModificarDB(cp_orden_giro_Info info, ref string mensaje)
        {
            try
            {
                using (EntitiesCuentasxPagar context = new EntitiesCuentasxPagar())
                {
                    ...
                    contact.IdCentroCosto = (info.IdCentroCosto == "") ? null : info.IdCentroCosto;
                    ...
                    contact.Archivo_PDF = info.Archivo_PDF;
        ...
        public Boolean ModificarDB_PDF(cp_orden_giro_Info info, ref string mensaje)
        {
            try
            {
                using (EntitiesCuentasxPagar context = new EntitiesCuentasxPagar())
                {
                    ...
                    var select_ = from T in context.cp_orden_giro
                                  where T.IdEmpresa == info.IdEmpresa
                                   && T.IdCbteCble_Ogiro == info.IdCbteCble_Ogiro
                                   && T.IdTipoCbte_Ogiro == info.IdTipoCbte_Ogiro
                                  select T;
                    ...
                    if (select_.Count() >= 1)
                    {
                        ...
                        //Modificar el codigo binario del pdf de la factura
                        contact.Archivo_PDF = info.Archivo_PDF;
                        context.SaveChanges();
                    }
                    else
        ...
        public Boolean ExisteFacturaPorProveedor(int IdEmpresa, string cedulaRuc, String co_serie, String co_Factura)
        {
            try
            {
                using (EntitiesCuentasxPagar Entity = new EntitiesCuentasxPagar())
                {
                    ...
                    co_serie = co_serie.Trim();
                    co_Factura = co_Factura.Trim();
                    var result = from q in Entity.vwcp_orden_giro_SRI
                                 where q.IdEmpresa == IdEmpresa
                                 && q.pe_cedulaRuc == cedulaRuc
                                 && co_Factura == q.co_factura
                                 && q.co_serie == co_serie
                                 && q.Estado == "A"
                                 select q;
        ...
        public bool Verifica_EstaAplicadaRetencion(int IdEmpresa, decimal IdCbteCble_Ogiro, int IdTipoCbte_Ogiro)
        {
            try
            {
                EntitiesCuentasxPagar pagar = new EntitiesCuentasxPagar();

                if (IdTipoCbte_Ogiro != 7)
                    return true;

                var query = from q in pagar.cp_retencion
                            where q.IdEmpresa == IdEmpresa
                ...
                cp_retencion Address = query.FirstOrDefault<cp_retencion>();

                var num = (from q in pagar.cp_retencion_det
                           where q.IdEmpresa == Address.IdEmpresa
                           && q.IdRetencion == Address.IdRetencion
                           select q.re_valor_retencion).Sum();

                if (num == 0.0 || num <= 0.0)
                    return true;

                var _giro = (from q in pagar.cp_orden_giro
                             where q.IdEmpresa == IdEmpresa
                             && q.IdTipoCbte_Ogiro == IdTipoCbte_Ogiro
                             && q.IdCbteCble_Ogiro == IdCbteCble_Ogiro
                             select q).FirstOrDefault();
                
                return Math.Round(_giro.co_valorpagar, 2) == Math.Round(_giro.co_total - num, 2);
            }
            catch (Exception exception)
            {
                string str = this.ToString();
                new Core.Erp.Data.General.tb_sis_Log_Error_Vzen_Data().Guardar_Log_Error(new tb_sis_Log_Error_Vzen_Info(exception.ToString(), "", str, "", "", "", "", "", DateTime.Now), ref this.mensaje);
                this.mensaje = exception.ToString() + " " + exception.Message;
        ...
        public cp_orden_giro_Info Get_OrdenGiro_Por_SerieComprobante(int IdEmpresa, decimal IdProveedor, string SerieComprobante)
        {
            ...
            {                
                ...
                string[] partes = SerieComprobante.Split('-');
                ...
                var query = from q in dbContext.cp_orden_giro
                            where q.IdEmpresa == IdEmpresa
                            && q.IdProveedor == IdProveedor
                            && q.co_serie == Serie
                            && q.co_factura == NumeroDocumento
                            select q;
