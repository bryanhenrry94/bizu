        ...
        public Boolean EliminarRegistro(int IdEmpresa, int IdProductoPadre, ref string mensaje)
        {
            try
            {
                using (EntitiesInventario entity = new EntitiesInventario())
                {
                    ...
                    string qry = string.Format("delete from in_producto_x_cp_proveedor where IdEmpresa_prod = " + IdEmpresa + " and IdProducto = " + IdProductoPadre + " and IdEmpresa_prov  = " + IdEmpresa);
                    var s = entity.Database.ExecuteSqlCommand(qry);

                    qry = "delete from in_Producto_Composicion where IdEmpresa = " + IdEmpresa + " and IdProductoPadre = " + IdProductoPadre;
                    entity.Database.ExecuteSqlCommand(qry);
                }
                return true;
            }
            catch (Exception ex)
