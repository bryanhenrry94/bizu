...
namespace Core.Erp.Data.Inventario
{
    public class in_producto_x_tb_bodega_Data
    {
        ...
        public List<in_producto_x_tb_bodega_Info> Get_List_Producto_x_Bodega_InicioFin(int IdEmpresa, int IdBodega_Inicio, int IdBodega_Fin, 
        ...
        public Boolean GrabaDB(List<in_producto_x_tb_bodega_Info> ls, int idempresa, ref string mensaje)
        {
            try
            {
                using (EntitiesInventario context = new EntitiesInventario())
                {
                    ...
                    {
                        int iCount = EDB.in_producto_x_tb_bodega.Count(v => v.IdEmpresa == item.IdEmpresa && v.IdSucursal == item.IdSucursal && v.IdBodega == item.IdBodega && v.IdProducto == item.IdProducto);

                        if (iCount == 0)
                        {
                            in_Producto_Info Info_Producto = new in_Producto_Info();
                            in_Producto_data Data_Producto = new in_Producto_data();
                            in_subgrupo_info Info_Subgrupo = new in_subgrupo_info();
                            in_subgrupo_data Data_Subgrupo = new in_subgrupo_data();

                            Info_Producto = Data_Producto.Get_info_Producto(item.IdEmpresa, item.IdProducto);

                            if (Info_Producto != null)
                            {
                                Info_Subgrupo = Data_Subgrupo.Get_Info_in_subgrupo(Info_Producto.IdEmpresa, Info_Producto.IdCategoria, Info_Producto.IdLinea, Info_Producto.IdGrupo, Info_Producto.IdSubGrupo);
                            }
                            ...
                            // Si tiene asignado una cuenta contable en la parametrizacion de subgrupo de producto se asigna las cuentas contables en la configuracion de producto por bodega de forma predeterminada
                            if (Info_Subgrupo.IdCtaCtble_Inve != null)
                                address.IdCtaCble_Inven = Info_Subgrupo.IdCtaCtble_Inve;
                            else
                                address.IdCtaCble_Inven = item.IdCtaCble_Inven;

                            if (Info_Subgrupo.IdCtaCtble_Costo != null)
                                address.IdCtaCble_Costo = Info_Subgrupo.IdCtaCtble_Costo; 
                            else
        ...
        public Boolean EliminarDB(int IdEmpresa, decimal IdProducto)
        {
            try
            {
                ...
                {
                    entity.Database.ExecuteSqlCommand("delete from in_producto_x_tb_bodega where IdEmpresa = " + IdEmpresa + " and IdProducto = " + IdProducto);
                }
        ...
        public Boolean EliminarDB(int IdEmpresa)
        {
            try
            {
                using (EntitiesInventario entity = new EntitiesInventario())
                {
                    entity.Database.ExecuteSqlCommand("delete from in_producto_x_tb_bodega where IdEmpresa = " + IdEmpresa );
                }
