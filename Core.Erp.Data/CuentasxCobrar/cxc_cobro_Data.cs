...
namespace Core.Erp.Data.CuentasxCobrar
{
    public class cxc_cobro_Data
    {
        ...
        public List<vwcxc_cartera_x_cobrar_Info> NotasCreditoConFacturaXCobrar(int IdEmpresa, int IdSucursal, decimal IdCliente)
        {
            try
            {
                ...
                foreach(var item in sele)
                {
                    ...
                    Info.SaldoAUX = Math.Round(Convert.ToDouble(item.Saldo), 2);
                    ...
                    Info.Total = Convert.ToDouble((item.vt_total == null) ? 0 : Math.Round(Convert.ToDouble(item.vt_total), 2));
                    ...
                    Info.vt_iva = Math.Round(Convert.ToDouble(item.vt_iva), 2);
                    //Info.vt_Subtotal = Convert.ToDouble(item.vt_Subtotal);
                    Info.vt_Subtotal = Math.Round(Convert.ToDouble(item.vt_Subtotal), 2);
                    //Info.vt_total = Convert.ToDouble(item.vt_total);
                    Info.vt_total = Math.Round(Convert.ToDouble(item.vt_total), 2);
        ...
        //public List<cxc_cobro_Info> Get_List_Cobros_x_depositar_detalle(int IdEmpresa)
        //{
                ...
                if (TipoMovCaj=="INGRESOS")
               {
                ...
                if (TipoMovCaj == "EGRESOS")
                {
                    ...
                    var select_ = from T in Base.vwcaj_Movi_x_Despositar
                                ...
                                && T.SeDeposita == true
                                group T by new {
                                    T.IdEmpresa, T.IdSucursal, T.IdCobro, T.IdCobro_tipo,T.IdCliente,T.cr_TotalCobro,T.cr_fecha,T.cr_fechaCobro,T.cr_observacion,
                                    T.cr_Banco,T.cr_cuenta,T.cr_Tarjeta,T.cr_NumDocumento, T.cr_estado,T.cr_recibo,  T.nSucursal,   T.nCliente,
                                    T.cr_fechaDocu,  T.IdCaja, T.ca_Descripcion, T.IdTipoMovi,T.tm_descripcion,T.tm_Signo,  T.IdUsuario,
                                    T.IdCbteCble_MoviCaja,  T.IdTipocbte_MoviCaja,        T.IdCtaCble,   T.SeDeposita
                                
                                }
                                into grouping
                                  let count = grouping.Count()
                                  select new { grouping.Key, cr_TotalCobro = grouping.Average(p => p.cr_TotalCobro)};
                ...
                foreach (var item in select_)
                {
                    ...
                    dat.IdEmpresa = item.Key.IdEmpresa;
                    dat.IdSucursal = item.Key.IdSucursal;
                    dat.IdCobro = item.Key.IdCobro;
                    dat.IdCobro_tipo = item.Key.IdCobro_tipo;
                    dat.IdCliente = item.Key.IdCliente;
                    dat.cr_TotalCobro = (item.Key.tm_Signo == "-") ? -1 * item.cr_TotalCobro : item.cr_TotalCobro;
                    dat.cr_fecha = item.Key.cr_fecha;
                    dat.cr_fechaCobro = Convert.ToDateTime((item.Key.cr_fechaCobro == null) ? DateTime.Now : item.Key.cr_fechaCobro);
                    dat.cr_observacion = item.Key.cr_observacion;
                    dat.cr_Banco = item.Key.cr_Banco;
                    dat.cr_cuenta = item.Key.cr_cuenta;
                    dat.cr_NumDocumento = item.Key.cr_NumDocumento;
                    dat.cr_estado = item.Key.cr_estado;
                    dat.cr_recibo = item.Key.cr_recibo;
                    dat.nSucursal = item.Key.nSucursal;
                    dat.nCliente = item.Key.nCliente;
                    dat.Caja = item.Key.ca_Descripcion;
                    dat.IdCbteCble_MoviCaja = item.Key.IdCbteCble_MoviCaja;
                    dat.IdTipocbte_MoviCaja = item.Key.IdTipocbte_MoviCaja;
                    dat.Tipo = (item.Key.tm_Signo == "-") ? "Egreso" : "Ingreso";
                    dat.Secuencia_MoviCaja = 0;


                    dat.chek = false;

                    dat.IdCtaCble = item.Key.IdCtaCble;
        ...
        public List<cxc_cobro_Info> Get_List_Cobros_x_depositar_Agrupado_detallado(int IdEmpresa,int mcj_IdTipocbte,decimal mcj_IdCbteCble )
        {
            try
            {
                ...
                var select_ = from T in Base.vwcaj_Movi_x_Despositar
                              where T.IdEmpresa == IdEmpresa
                              && T.IdCbteCble_MoviCaja == mcj_IdCbteCble
                              && T.IdTipocbte_MoviCaja == mcj_IdTipocbte
                             select T;
