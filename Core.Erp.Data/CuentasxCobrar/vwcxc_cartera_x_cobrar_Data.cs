...
namespace Core.Erp.Data.CuentasxCobrar
{
    public class vwcxc_cartera_x_cobrar_Data
    {
        ...
        public List<vwcxc_cartera_x_cobrar_Info> Get_List_cartera_x_cobrar_Todos(int IdEmpresa, int IdSucursal, decimal IdCliente)
        {
            try
            {
                int IdSucursal_Ini = (IdSucursal == 0) ? 1 : IdSucursal;
                ...
                foreach (var item in sele)
                {
                    ...
                    Obj.IdCliente = item.IdCliente;
                    Obj.vt_fecha = item.vt_fecha;
                    Obj.vt_total = Math.Round(Convert.ToDouble(item.vt_total), 2);
                    Obj.Saldo = Math.Round(Convert.ToDouble(item.Saldo), 2); ;
                    Obj.TotalxCobrado = Math.Round(Convert.ToDouble(item.TotalxCobrado), 2);
                    Obj.Bodega = item.Bodega;
                    Obj.vt_Subtotal = Math.Round(Convert.ToDouble(item.vt_Subtotal), 2); ;
                    Obj.vt_iva = Math.Round(Convert.ToDouble(item.vt_iva), 2); ;
                    Obj.vt_fech_venc = Convert.ToDateTime(item.vt_fech_venc);
                    Obj.SaldoAUX = Math.Round(Convert.ToDouble(item.Saldo), 2); ;
                    Obj.NomCliente = item.NomCliente;
                    Obj.pe_nombreCompleto = "[" + Obj.IdCliente + "] : " + item.NomCliente;
                    Obj.dc_ValorRetFu = item.dc_ValorRetFu;
                    Obj.dc_ValorRetIva = item.dc_ValorRetIva;
