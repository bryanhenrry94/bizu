  ...
  public  class cxc_conciliacion_det_Data
    {
          ...
          try
          {
              foreach (var Item in Lst)
              {
                  using (EntitiesCuentas_x_Cobrar Context = new EntitiesCuentas_x_Cobrar())
                  {
                      ...
                      Deta.secuencia_cbr = Item.secuencia_cbr;
                      Deta.IdEmpresa = Item.IdEmpresa;
                      Deta.IdSucursal = Item.IdSucursal;
                      Item.IdConciliacion = Id;
                      Deta.IdConciliacion = Item.IdConciliacion;
                      Item.Secuencia = GetSecuencia(Item.IdEmpresa, Item.IdSucursal, Id, ref mensaje);
      ...
      public Boolean Modificar_Secuencia_Cobro_DB(List<cxc_conciliacion_det_Info> Lst, ref string mensaje)
      {
          try
          {
              cxc_conciliacion_det_Info Info = Lst.FirstOrDefault();

              using (EntitiesCuentas_x_Cobrar Context = new EntitiesCuentas_x_Cobrar())
              {
                  var Query = from q in Context.cxc_conciliacion_det
                              where q.IdEmpresa == Info.IdEmpresa
                                  && q.IdSucursal == Info.IdSucursal
                                  && q.IdConciliacion == Info.IdConciliacion
                              select q;
          ...
          try
          {
              ...
              using (EntitiesCuentas_x_Cobrar cxc = new EntitiesCuentas_x_Cobrar())
              {
                  ...
                  if (data != null)
                  {
                      ...
                      data.secuencia_cbr = info.secuencia_cbr;
                      cxc.SaveChanges();
                      res = true;
                  }
              }
