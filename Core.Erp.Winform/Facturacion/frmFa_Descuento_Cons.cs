...
{
    public partial class frmFa_Descuento_Cons : DevExpress.XtraEditors.XtraForm
    {
        public frmFa_Descuento_Cons()
        {
            InitializeComponent();
        }
        ...
        List<fa_descuento_Info> Lista;
        fa_descuento_Bus Bus;
        ...
        private void load_Data()
        {
            try
            {
                Bus = new fa_descuento_Bus();
                Lista = new List<fa_descuento_Info>();
                Lista = Bus.Get_Lista_Descuento(param.IdEmpresa, ref MensajError);
                gridControlDescuento.DataSource = Lista;
        ...
        private void llama_formulario(Cl_Enumeradores.eTipo_action Accion)
        {
            try
            {
                ...
                frmFa_Descuento_Mant frm = new frmFa_Descuento_Mant();
                frm.MdiParent = this.MdiParent;
                frm.event_frmFa_Descuento_Mant_FormClosing += frm_event_frmFa_Descuento_Mant_FormClosing;                
        ...
        #region " Eventos "
        void frm_event_frmFa_Descuento_Mant_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                load_Data();
        ...
        private void frmFa_Descuento_Cons_Load(object sender, EventArgs e)
        {
            try
            {
                //Control de Usuario
                Infoseg = Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(param.IdEmpresa, param.IdUsuario, "Facturacion.frmFa_Descuento_Cons");
                foreach (var item in Infoseg)
                {
                    ...
                    {
                        ucGe_Menu_Mantenimiento_x_usuario1.Enabled = false;
                    }
                }
        ...
        private void ucGe_Menu_Mantenimiento_x_usuario1_event_btnImprimir_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                this.gridControlDescuento.ShowPrintPreview();
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private void gridViewDescuento_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                var data = gridViewDescuento.GetRow(e.RowHandle) as fa_descuento_Info;
                if (data == null)
                    return;
