...
{
    public partial class frmCxc_CatalogoTipo_Mantenimiento : DevExpress.XtraEditors.XtraForm
    {
            ...
            try
            {
                if (validar())
                {
                    if (Bus.ValidarCodigoExiste(_Info.IdCatalogo_tipo) == false)
                    {
                            ...
                            {
                                BloquearDatos();
                                string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El registro ", _Info.IdCatalogo_tipo);
            ...
            try
            {
                if (validar())
                {
                    if (Bus.ModificarDB(_Info))
                    {
                        ...
                        string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El registro ", _Info.IdCatalogo_tipo);
                        MessageBox.Show(smensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
