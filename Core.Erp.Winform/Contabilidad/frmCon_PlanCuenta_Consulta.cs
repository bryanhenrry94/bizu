...
{
    public partial class frmCon_PlanCuenta_Consulta : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frmCon_PlanCta_Load(object sender, EventArgs e)
        {
            ...
            {
                //Control de Usuario
                Infoseg = Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(param.IdEmpresa, param.IdUsuario, "Contabilidad.frmCon_PlanCuenta_Consulta");
        ...
        public void Cargar_PlanCuenta()
        {
            try
            {
                ...
                this.gridControlPlancta.DataSource = _ListPlanCtaInfo.OrderBy(q => q.IdCtaCble);
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void gridViewPlancta_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                var data = gridViewPlancta.GetRow(e.RowHandle) as ct_Plancta_Info;
                if (data == null)
                    return;
                if (data.pc_Estado == "I")
                    e.Appearance.ForeColor = Color.Red;
