    ...
    public partial class UCIn_Sucursal_Bodega : UserControl
    {
        ...
        public void UCIn_Sucursal_Bodega_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //cargar_bodega_en_Combo(1, 1);
                cargar_sucursal(param.IdEmpresa);

                switch (param.IdCliente_Ven_x_Default)
                {
                    case Cl_Enumeradores.eCliente_Vzen.CG:
                        cargar_bodega_CG(param.IdEmpresa, param.IdSucursal);
                        break;
                }                                                               
            }
            catch (Exception ex)
        ...
        public void cargar_sucursal(int idEmpresa)
        {
            try
            {
               ...
               // cargar_bodega_en_Combo(param.IdEmpresa, param.IdSucursal);
        ...
        public void cargar_bodega(int IdEmpresa)
        {
            try
            {
                ...
                listBodega.ForEach(v => { v.bo_Descripcion = "[" + v.IdBodega + "]-" + v.bo_Descripcion; });
                this.cmb_bodega.Properties.DataSource = listBodega;
                cmb_bodega.Properties.DisplayMember = "bo_Descripcion";
                cmb_bodega.Properties.ValueMember = "IdBodega";                
            }
            catch (Exception ex)
        ...
        public void cargar_sucursal_CG(int idEmpresa)
        {
            try
            {
                if (TipoCarga == Cl_Enumeradores.eTipoFiltro.Normal)
        ...
        public void cargar_bodega_CG(int IdEmpresa, int IdSucursal)
        {
            try
            {
                if (TipoCarga == Cl_Enumeradores.eTipoFiltro.Normal)
                {
                    listBodega = bodeB.Get_List_Bodega(IdEmpresa, IdSucursal);
                }
                if (TipoCarga == Cl_Enumeradores.eTipoFiltro.todos)
                {
                    listBodega = bodeB.Get_List_Bodega(IdEmpresa, TipoCarga);
                ...
                cmb_bodega.EditValue = listBodega.FirstOrDefault().IdBodega;
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private void cmb_sucursal1_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
              ...
              //  _SucursalInfo = (tb_Sucursal_Info) (listSucursal.FirstOrDefault(v => v.IdSucursal == 1));
        ...
        private void cmb_bodega1_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
               // _BodegaInfo = (tb_Bodega_Info)(listBodega.FirstOrDefault(v =>v.IdSucursal == Convert.ToInt32(cmb_sucursal.EditValue) && v.IdBodega == Convert.ToInt32(cmb_bodega.EditValue)));               
                ...
                //J.A 08-06-2018 cuando es bodega 0 la sucursal tambien debe ser 0, se está callendo porque está filtrando bodega 0 y sucursal 1
                if (Convert.ToInt32(cmb_bodega.EditValue) == 0) 
                {
                    _BodegaInfo = (tb_Bodega_Info)(listBodega.FirstOrDefault(v => v.IdSucursal == 0 && v.IdBodega == Convert.ToInt32(cmb_bodega.EditValue))); 
                }
                else
                {
                    _BodegaInfo = (tb_Bodega_Info)(listBodega.FirstOrDefault(v => v.IdSucursal == param.IdSucursal && v.IdBodega == Convert.ToInt32(cmb_bodega.EditValue)));  
                }
