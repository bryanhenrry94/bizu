...
namespace Core.Erp.Winform.Controles
{
    public partial class UCCp_Retencion : UserControl
    {
        ...
        // rth  sunchodesa validar dos decimales
        int decimalParametrizado = 0;
        tb_parametro_Bus bus_parametros = new tb_parametro_Bus();
        ...
        public void set_FechaEmisionRetencion(DateTime fechaEmision)
        {            
            dtp_fechaEmisionRetencion.Value = fechaEmision;
        }
        ...
        private IEnumerable<object> listaAux_codigoSRI_grid;
        ...
        private void UCCp_Retencion_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(Info_Retencion.NumRetencion) == false)
                {
                    ...
                    UC_numRetencion.rbt_doc_Electronico.Checked = (bool)Info_Retencion.EsDocumentoElectronico;//true;//(bool)Info_Retencion.EsDocumentoElectronico;

                    UC_numRetencion.rbt_pre_impresa.Checked = (bool)Info_Retencion.EsDocumentoElectronico;//false;//!(bool)Info_Retencion.EsDocumentoElectronico;
                    ...
                    UC_numRetencion.Set_Check_electronico(talonario_Info.es_Documento_electronico);
        ...
        public void Set_TipoDocumento(bool Check){
            UC_numRetencion.Set_TipoDocumento(Check);
        }

        public void Set_Check_doc_elec(bool Check)
        { 
            try
            {
                UC_numRetencion.Set_Check_electronico(Check);
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
                        ...
                        if (item.MontoRetencion > 0)
                        {
                            switch (param.IdCliente_Ven_x_Default)
                            {
                                case Cl_Enumeradores.eCliente_Vzen.EDEHSA:
                                    ...
                                    valor = Convert.ToDouble(Math.Round(valor, 2));
                                    break;
        ...
        public void Get_Documento_usado(string IdEstablecimiento, string IdPuntoVenta, string Num_Documento)
        {
            try
            {
                UC_numRetencion.IdEstablecimiento = IdEstablecimiento;
                //UC_numRetencion.IdPuntoEmision = IdPuntoEmision;
                UC_numRetencion.Set_Serie_Num_Documento(IdEstablecimiento, IdPuntoVenta, Num_Documento);
        ...
        private void gridView_SRI_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
                ...
                if (e.Column.Name == "colIdCodigo_SRI")
                {
                    ...
                    gridView_SRI.SetFocusedRowCellValue(colMontoRetencion, Math.Round(Convert.ToDecimal(prueba), 2));
            ...
            try
            {
               ...
               int i= Info_Retencion.IdEmpresa;
                ...
                if (UC_numRetencion.rbt_doc_Electronico.Checked == true)
                {
                    ...
                    lista_Retencion_sri = busRet.Get_Lis_reten(param.IdEmpresa, Info_Retencion.IdRetencion, ref MensajeError);
        ...
        public void FormateaNumDecimales()
        {
            string error = "";
            decimalParametrizado = Convert.ToInt32(bus_parametros.Get_valor_parametro("decimales", ref error));
            //this.MontoRetencion.Properties.Mask.EditMask = "c" + decimalParametrizado;
            //this.MontoRetencion.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric;
            this.colMontoRetencion.DisplayFormat.FormatString = "{0:c"+decimalParametrizado+"}";
            this.colMontoRetencion.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            //this.MontoRetencion.Properties.DisplayFormat.FormatString = "{0:c" + decimalParametrizado + "}";           

        }

        public void FormateaNumDecimalesVariables(List<cp_codigo_SRI_Info> listaAux_codigoSRI)
        {
            try
            {
                foreach (var item in listaAux_codigoSRI)
                {
                    item.MontoRetencion = Math.Round(item.MontoRetencion, decimalParametrizado);                    
                }
            }
            catch (Exception ex)
            {
