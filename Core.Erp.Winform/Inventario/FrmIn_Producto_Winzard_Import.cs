...
{
    public partial class FrmIn_Producto_Winzard_Import : DevExpress.XtraEditors.XtraForm
    {
        ...
        bool Cargar_ArchivoExcel_x_Producto_A_DataTable()
        {
            try
            {
                ...
                //  DataTable_Excel_producto = DataTable_Excel_producto.Rows.Cast<DataRow>().Where(row => !row.ItemArray.All(field => field is System.DBNull || string.Compare((field as string).Trim(), string.Empty) == 0)).CopyToDataTable();
                //DataTable_Excel_producto = DataTable_Excel_producto.Columns.Cast<DataColumn>().Where(row => !row.colu.ItemArray.All(field => field is System.DBNull || string.Compare((field as string).Trim(), string.Empty) == 0)).CopyToDataTable();
                ...
                if (gridControlProductos.DataSource != null)
                {
                    ...
                    if (MessageBox.Show(MensajeWarning, "SISTEMAS", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                    {
                        ...
                        if (B_Proceso_anulacion == true)
                        {
                           ...
                           List<in_subgrupo_info> list_SubGrupo = new List<in_subgrupo_info>();
                           list_SubGrupo = SubGrupo_bus.ConsultaGeneralSubGrupo(param.IdEmpresa);
                            ...
                            foreach (in_Producto_Info item in _ListProductoInfo)
                            {
                                ...
                                try
                                {
                                   ...
                                   // item.IdLinea = listLinea.First(v => v.IdCategoria == item.IdCategoria && v.cod_linea.ToString().Trim().ToLower() == item.nom_Linea.ToString().Trim().ToLower()).IdLinea;
                                }
                                catch (Exception)
                                {
                                    item.IdLinea = 1;
                                ...
                                try
                                {
                                    ...
                                    //item.IdGrupo = listgrupo.First(v => v.IdCategoria == item.IdCategoria && v.IdLinea == item.IdLinea && v.cod_grupo.ToString().Trim().ToLower() == item.nom_Grupo.ToString().Trim().ToLower()).IdGrupo;
                                ...
                                try
                                {
                                    item.IdSubGrupo = list_SubGrupo.First(v => v.IdCategoria == item.IdCategoria && v.IdLinea == item.IdLinea && v.IdGrupo == item.IdGrupo && v.nom_subgrupo.Trim().ToLower() == item.nom_SubGrupo.Trim().ToLower()).IdSubgrupo;
                                   // item.IdSubGrupo = list_SubGrupo.First(v => v.IdCategoria == item.IdCategoria && v.IdLinea == item.IdLinea && v.IdGrupo == item.IdGrupo && v.cod_subgrupo.ToString().Trim().ToLower() == item.nom_SubGrupo.ToString().Trim().ToLower()).IdSubgrupo;
                                ...
                                try
                                {
                                    item.IdPresentacion = list_Presentacion.First(v => v.nom_presentacion.Trim().ToLower() == item.nom_Presentacion.Trim().ToLower()).IdPresentacion;
                                    //item.IdPresentacion = list_Presentacion.First(v => v.nom_presentacion.Trim().ToLower() == item.nom_Presentacion.Trim().ToLower()).IdPresentacion;

                                }
                                catch (Exception)
                                {
                                    item.IdPresentacion = "01";
                                }


                                try
                                {
                                    item.IdMarca = list_Marca.First(v => v.Descripcion.Trim().ToLower() == item.nom_Marca.Trim().ToLower()).IdMarca;
                                   // item.IdMarca = list_Marca.First(v => v.IdMarca.ToString().Trim().ToLower() == item.nom_Marca.Trim().ToLower()).IdMarca;
                                ...
                                ListEstadoGrabacion.Add(new cl_estado_grabacion(c, item.IdProducto.ToString(), item.pr_codigo, item.pr_descripcion, "OK" + MensajeError, "Migrado " + respuesta));
                                progressBar.Value = c;
