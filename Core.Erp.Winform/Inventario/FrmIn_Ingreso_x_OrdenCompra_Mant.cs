...
{
    public partial class FrmIn_Ingreso_x_OrdenCompra_Mant : DevExpress.XtraEditors.XtraForm
    {
        ...
        void Imprimir()
        {
            try
            {
                ...
                string msg = "";
                if (ucIn_GuiaRemision_Recepcion.chk_NEDV.Checked)
                    msg = "NEDV #" + ucIn_GuiaRemision_Recepcion.txt_numero_nedv.Text;
                else
                    msg = "Guía #" + ucIn_GuiaRemision_Recepcion.txt_numGuia.Text;

                if (MessageBox.Show("¿Desea Imprimir etiquetado de los produtos registrados a la Bodega x OC. " + msg + "\n" + "Imprimir", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    Imprimir_Etiquetado();
                }
            }
        ...
        void Imprimir_Etiquetado()
        {
            try
            {
                ...
                Lista = Bus_IngEgrDet.Get_List_Ing_Egr_Inven_det(param.IdEmpresa, Convert.ToInt32(ucIn_Sucursal_Bodega1.cmb_sucursal.EditValue),
                    Convert.ToInt32(info_Comparam.IdMovi_inven_tipo_OC), Convert.ToDecimal(txtNumIngreso.Text));
        ...
        void Get()
        {
            try
            {
                ...
                info_IngComp.numGuia = Convert.ToString(ucIn_GuiaRemision_Recepcion.txt_numGuia.EditValue);
                info_IngComp.EsDocumentoAutorizado = ucIn_GuiaRemision_Recepcion.chk_NEDV.Checked;
                info_IngComp.numDocumento = Convert.ToString(ucIn_GuiaRemision_Recepcion.txt_numero_nedv.EditValue);
        ...
        private void setInfo()
        {
            try
            {
                ...
                ucIn_GuiaRemision_Recepcion.txt_numGuia.EditValue = SETINFO_.numGuia;
        ...
        public Boolean Validar()
        {
            try
            {
                ...
                if (!ucIn_GuiaRemision_Recepcion.validar_Guia())
                {
                    return false;
                }
                ...
                switch (Accion)
                {
                    case Cl_Enumeradores.eTipo_action.grabar:
                        ...
                        if (Bus_IngEgr.GuardarDB(info_IngComp, ref IdIngreso_oc, ref mensaje))
                        {
                            ...
                            string smensaje = "";
                            if (mensaje == "")
                                smensaje = string.Format("{0} # {1} se guardó con éxito.", "El registro del Ingreso a Bodega ", IdIngreso_oc.ToString());
                            else
                                smensaje = mensaje;
        ...
        private void Buscar_Orden_Compra_()
        {
            try
            {
                ...
                foreach (var item in list)
                {
                    ...
                    info.detalle_x_tiem = item.detalle_x_tiem;
                    lista_AUX.Add(info);
                }
