...
{
    public partial class FrmIn_responsable_Mant : DevExpress.XtraEditors.XtraForm
    {
        #region Variables
        ...
        void Guardar()
        {
            try
            {
                ...
                if (Bus_comprador.GuardarDB(Info, ref MensajeError))
                {
                    ...
                    string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El comprador ", Info.IdResponsable);
        ...
        void Actualizar()
        {
            try
            {
                ...
                {
                    string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El comprador", Info.IdResponsable);
                    MessageBox.Show(smensaje, param.Nombre_sistema);
                    //ucGe_Menu_Superior_Mant1.Visible_bntGuardar_y_Salir = DevExpress.XtraBars.BarItemVisibility.Never;
                    //ucGe_Menu_Superior_Mant1.Visible_btnGuardar = DevExpress.XtraBars.BarItemVisibility.Never;    
                    LimpiarDatos();
                ...
                if (Info.IdResponsable != 0)
                {
                    if (MessageBox.Show("¿Está seguro que desea anular eL Comprador #: " + Info.IdResponsable + " ?", "Anulación de Comprador ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (Info.Estado == true)
                        {
                            ...
                            {
                                string smensaje = string.Format("{0} # {1} se anuló con éxito.", "El comprador", Info.IdResponsable);
                                MessageBox.Show(smensaje, param.Nombre_sistema);
