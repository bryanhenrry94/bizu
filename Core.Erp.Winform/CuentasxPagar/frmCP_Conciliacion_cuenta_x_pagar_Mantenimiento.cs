...
{
    public partial class frmCP_Conciliacion_cuenta_x_pagar_Mantenimiento : DevExpress.XtraEditors.XtraForm
    {
        #region Variables
        int IdEmpresa_ant = 0;
        ...
        void Cargar_Grids()
        {
            try
            {
                ...
                List_OPxCancelar = bus_OPxCancelar.Get_List_orden_pago_con_cancelacion_todos_Mayor_a_cero_Lite(param.IdEmpresa, "APRO");
                ...
                if (tabControl.SelectedTab == tabPage5)
                {
                    foreach (var item in this.BinList_OPxCancelacion)
                    {
                        if (item.Check == true)
                        {
                            ...
                            opcsinfo.FechaCancelacion = Convert.ToDateTime(dtpFecha.EditValue);
                            ...
                            if (radio == "ANTPROV")
                            {
                                ...
                                info.IdTipoCbte = (paramCP_I.pa_TipoCbte_para_conci_x_antcipo != null) ? paramCP_I.pa_TipoCbte_para_conci_x_antcipo : info_cbtcble.IdTipoCbte;
                                ...
                                foreach (var item2 in lst_cbte)
                                {
                                    ...
                                    if (item2.dc_Valor < 0)
                                    {
                                        ...
                                        det_Info.IdTipoCbte = (paramCP_I.pa_TipoCbte_para_conci_x_antcipo != null) ? paramCP_I.pa_TipoCbte_para_conci_x_antcipo : item2.IdTipoCbte;
                                ...
                                {
                                    MessageBox.Show(msgError, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);

                                    MessageBox.Show("No se pudo grabar el diario contable del anticipo, por favor verifique que las cuentas del diario generado por la OP sean diferentes!", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                    return false;
                                }
                            }
                            else
            ...
            try
            {
                ...
                if (lista_Cance.Count != 0)
                {
                    foreach (var item in lista_Cance)
                    {
                        ...
                        List_OPxCancelar = bus_OPxCancelar.Get_List_orden_pago_con_cancelacion_todos_Mayor_a_cero_Lite(Convert.ToInt32(item.IdEmpresa_op), Convert.ToDecimal(item.IdOrdenPago_op), Convert.ToInt32(item.Secuencia_op));
        ...
        public void CargarGridCompDis()
        {
            try
            {
                ...
                if (radio == "ANTPROV")
                {
                    ...
                    List<vwcp_Anticipos_para_Conciliar_Info> lista_AntxConciliar = bus_antxCon.Get_list_Anticipos_para_Conciliar(param.IdEmpresa, "", Convert.ToDateTime(dtpDesde.EditValue).Date, Convert.ToDateTime(dtpHasta.EditValue).Date, ref mensaje);
                ...
                {
                    List<vwct_cbtecble_con_saldo_cxp_Info> list_cbtecble_con_saldo_cxp = CCScxpBus.Get_list_cbtecble_con_saldo_cxp(param.IdEmpresa, radio, Convert.ToDateTime(dtpDesde.EditValue).Date, Convert.ToDateTime(dtpHasta.EditValue).Date, ref mensaje);
                    BinList_Anticipo_Diarios = new BindingList<vwct_cbtecble_con_saldo_cxp_Info>(list_cbtecble_con_saldo_cxp);
                }
        ...
        public Boolean Validar()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Ingrese Observación!", "Sistemas", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return false;
                }
        ...
        public Boolean Grabar()
        {
            try
            {
                ...
                if (Get())
                {
                    ...
                    {
                        ConciliacionInfo.IdEmpresa_cbtecble = param.IdEmpresa;
                        ConciliacionInfo.IdTipoCbte_cbtecble = paramCP_I.pa_TipoCbte_para_conci_x_antcipo;
                        ConciliacionInfo.IdCbteCble_cbtecble = 0;
                    }
                ...
                if (regresa == true)
                {
                    ...
                    string smensaje = string.Format("{0} # {1} se guardó con éxito.", "La Conciliación", txtIdConciliacion.EditValue);
                ...
                {
                    string smensaje = string.Format("Error al Grabar:  {0}", mensaje);
        ...
        public void Anular()
        {
            try
            {
                ...
                //J.A se corregió la anulacion de la conciliacion el proceso estaba mal

                if (MessageBox.Show("¿Está seguro que desea anular la Conciliación", "Anulación de Comprobante", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        string smensaje = string.Format("Error al Anular:  {0}");
                        MessageBox.Show(smensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                //foreach (var item in OrdenPagoCancesInfo)
        ...
        void GeneraDiario()
        {
            try
            {
                ...
                foreach (var item in this.BinList_OPxCancelacion)
                {
                    if (item.Check == true)
                    {
                        vwct_cbtecble_con_ctacble_acreedora_Info Info_Cta_Acreedora = new vwct_cbtecble_con_ctacble_acreedora_Info();

                        Info_Cta_Acreedora = Bus_CtaCble_Acreedora.Get_Info(item.IdEmpresa, Convert.ToInt16(item.IdTipoCbte_cxp), Convert.ToDecimal(item.IdCbteCble_cxp));

                        ct_Cbtecble_det_Info Debe = new ct_Cbtecble_det_Info();
                        Debe.IdEmpresa = param.IdEmpresa;
                        
                        if (Info_Cta_Acreedora != null)
                        {
                            Debe.IdCtaCble = Info_Cta_Acreedora.IdCtaCble_Acreedora;
                        }
