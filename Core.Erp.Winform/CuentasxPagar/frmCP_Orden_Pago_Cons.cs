...
{
    public partial class frmCP_Orden_Pago_Cons : DevExpress.XtraEditors.XtraForm
    {
        string mensajeFrm = "";
        string mensaje = "";
        ...
        private void CargarGrid()
        {
            try
            {
                ...
                list = this.Bus_OrdenPago.Get_List_orden_pago(this.param.IdEmpresa, Convert.ToDateTime(this.ucGe_Menu.fecha_desde), Convert.ToDateTime(this.ucGe_Menu.fecha_hasta), this.param.IdUsuario, ref mensaje);
                foreach (cp_orden_pago_Info info in list)
                {
                    if (info.Estado == "I")
                    {
                        info.EstadoCancelacion = "Anulado";
                        info.Descripcion = "Anulado";
                    }
                }
        ...
        private void frmCP_Orden_Pago_Cons_Load(object sender, EventArgs e)
        {
            try
            {
                this.Infoseg = this.Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(this.param.IdEmpresa, this.param.IdUsuario, "CuentasxPagar.frmCP_Orden_Pago_Cons");

                foreach (seg_Menu_info _info in this.Infoseg)
                {
                    if (!_info.Eliminacion)
                    {
                        this.ucGe_Menu.btnAnular.Enabled = false;
                    }
                    if (!_info.Escritura)
                    {
                        this.ucGe_Menu.btnModificar.Enabled = false;
                    }
                }

                Cl_Enumeradores.eCliente_Vzen vzen = this.param.IdCliente_Ven_x_Default;
        ...
        private void gridViewConsulta_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (data.EstadoCancelacion.ToString() == "PROCESADA")
        ...
        private void Cargar_combos()
        {
            try
            {
                this.lst_tipo_flujo = this.bus_tipo_flujo.Get_List_TipoFlujo(this.param.IdEmpresa);
                this.cmb_tipo_flujo.DataSource = this.lst_tipo_flujo;
            }
            catch (Exception exception)
            {
                this.Log_Error_bus.Log_Error(exception.ToString());
