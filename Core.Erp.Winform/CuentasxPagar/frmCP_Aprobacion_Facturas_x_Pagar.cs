...
{
    public partial class frmCP_Aprobacion_Facturas_x_Pagar : DevExpress.XtraEditors.XtraForm
    {
        ...
        void carga_grid()
        {
            try
            {
                ...
                if (lista.Count != 0)
                {
                    ...
                    {
                        cp_orden_pago_formapago_Info Info_FormaPago = lista_FormaPago.Find(q => q.IdFormaPago == "NTDEB_BAN");

                        if (Info_FormaPago != null)
                        {
                            item.IdFormaPago = Info_FormaPago.IdFormaPago;
                        }
                        else
                        {
                            item.IdFormaPago = null;
                        }
            ...
            try
            {
                ...
                foreach (var item in factura_x_pagar)
                {
                    if (item.check == true)
                    {
                        ...
                        {
                            MessageBox.Show("Ingrese la Forma de Pago a la Factura:  " + item.Referencia, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            res = false;
                        }
                        if (String.IsNullOrEmpty(item.IdEstadoAprobacion))
                        {
                            MessageBox.Show("Ingrese el Estado de Aprobación a la Factura:  " + item.Referencia, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            res = false;
                        }

                        if(item.Valor_a_Pagar <= 0)
                        {
                            MessageBox.Show("Ingrese el valor a pagar a la Factura:  " + item.Referencia, param.Nombre_sistema, MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
                            res = false;
                        }
                    }
                }

                if (!param.Validar_periodo_cerrado_x_modulo(param.IdEmpresa, Cl_Enumeradores.eModulos.CXP, Convert.ToDateTime(dte_Fecha_op.EditValue)))
                    return false;
        ...
        private void gridView_Factura_x_Pagar_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            try
            {
                cp_orden_giro_Info row = new cp_orden_giro_Info();
                row = (cp_orden_giro_Info)gridView_Factura_x_Pagar.GetRow(e.RowHandle);

                if (e.Column.Name == "colcheck")
                {
                    if (row.Saldo_OG_AUX == 0)
                    {
                    ...
                    if ((bool)gridView_Factura_x_Pagar.GetFocusedRowCellValue(colcheck))
                    {
                        ...
                        gridView_Factura_x_Pagar.SetFocusedRowCellValue(colIdEstadoAprobacion, row.IdEstadoAprobacion_aux);
                        gridView_Factura_x_Pagar.SetFocusedRowCellValue(colSaldo_OG, row.Saldo_OG_AUX);
                        gridView_Factura_x_Pagar.SetFocusedRowCellValue(colValor_a_Pagar, 0);
                        row.Info_cuotas_x_doc.Cancela_cuota = false;
                    }
                    else
                    {
                        ...
                        if (row.Info_cuotas_x_doc.Valor_cuota != null)
                        {
                            ...
                            {
                                gridView_Factura_x_Pagar.SetFocusedRowCellValue(colValor_a_Pagar, row.Info_cuotas_x_doc.Valor_cuota);
                                gridView_Factura_x_Pagar.SetFocusedRowCellValue(colSaldo_OG, 0);
                                row.Info_cuotas_x_doc.Cancela_cuota = true;

                            }
                            else
                            {
                                gridView_Factura_x_Pagar.SetFocusedRowCellValue(colValor_a_Pagar, row.Saldo_OG_AUX);
                                gridView_Factura_x_Pagar.SetFocusedRowCellValue(colSaldo_OG, 0);
                            }
                        }
                        else
                        ...
                        ofrm.IdEmpresa = row.IdEmpresa;
                        ofrm.IProveedor = row.IdProveedor;
                        ofrm.carga_Grid();
            ...
            try
            {
                ...
                if (e.Column.Name == "colValor_a_Pagar")
                {
                    ...
                    else
                    {
                        ...
                        if (Convert.ToDouble(gridView_Factura_x_Pagar.GetFocusedRowCellValue(colValor_a_Pagar)) > row.Saldo_OG_AUX)
                        {
                        ...
                        {
                            double total = 0;
                            total = row.Saldo_OG_AUX - Convert.ToDouble(gridView_Factura_x_Pagar.GetFocusedRowCellValue(colValor_a_Pagar));
        ...
        private void chkAprobar_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                cp_orden_giro_Info row = new cp_orden_giro_Info();
                row = (cp_orden_giro_Info)gridView_Factura_x_Pagar.GetRow(RowHandle);
                ...
                {
                    this.info_parametros_cp = this.bus_parametros_cp.Get_Info_parametros(this.param.IdEmpresa);
                    
                    // Valida si esta en el parametro la opcion valida existe retencion y el tipo documento es 01 "Factura"
                    if (Convert.ToBoolean(this.info_parametros_cp.pa_valida_existe_retencion) &&  row.IdOrden_giro_Tipo == "01")
                    {
                        // Si no esta aplicada la retencion, muestra un mensaje al usuario
                        if (!this.bus_ogiro.Verifica_EstaAplicadaRetencion(row.IdEmpresa, row.IdCbteCble_Ogiro, row.IdTipoCbte_Ogiro))
                        {
                            MessageBox.Show("La factura a cancelar no tiene aplicada la retencion o el valor a pagar incorrecto, comuniquese con sistemas", this.param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            this.gridView_Factura_x_Pagar.SetFocusedRowCellValue(this.colcheck, false);
                        }
                    }
                    else
                    {
