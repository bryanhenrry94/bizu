...
{
    public partial class FrmBA_Talonario_cheques_x_bancoMant : DevExpress.XtraEditors.XtraForm
    {
             ...
             try
             {
                 ...
                 if (Validar())
                 {
                     ...
                     if (ba_Talonario_cheques_x_banco_B.Get_Info_Cheq_existe(Info.IdEmpresa, Info.IdBanco, Info.Num_cheque, ref Mensaje) != false)
                     {
                         ...
                         {
                             MessageBox.Show("El registro ha sido guardado con éxito", param.Nombre_sistema);
                         }
                     }
                     else
                     {
             ...
             try
             {
                 ...
                 if (Info.Estado == "A")
                 {
                     if (MessageBox.Show("¿Está seguro que desea anular ?", "Anulación  " + param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                     {
                         ...
                         {
                             string smensaje = string.Format("{0} # {1} se anuló con éxito.", "El cheque", Info.Num_cheque);
                         ...
                         {
                             MessageBox.Show("Error al Anular:  {0}" + mensaje, "Sistemas", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                             resultB = false;
                         }
                     }
                 }
