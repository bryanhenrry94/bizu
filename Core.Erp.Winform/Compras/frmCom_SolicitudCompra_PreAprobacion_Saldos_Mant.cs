...
{
    public partial class frmCom_SolicitudCompra_PreAprobacion_Saldos_Mant : DevExpress.XtraEditors.XtraForm
    {
        ...
        vwcom_solicitud_compra_x_items_con_saldos_Info solicitudCompraInfo =
           new vwcom_solicitud_compra_x_items_con_saldos_Info();

        vwcom_solicitud_compra_x_items_con_saldos_Bus saldosBus = new vwcom_solicitud_compra_x_items_con_saldos_Bus();
        List<vwcom_solicitud_compra_x_items_con_saldos_Info> listaSaldos = new List<vwcom_solicitud_compra_x_items_con_saldos_Info>();

        public frmCom_SolicitudCompra_PreAprobacion_Saldos_Mant()
        {
            InitializeComponent();
        ...
        private void frmCom_SolicitudCompra_PreAprobacion_Saldos_Mant_Load(object sender, EventArgs e)
        {
            try
            {
                BindList_DetSolCom = new BindingList<vwcom_solicitud_compra_x_items_con_saldos_Info>();
                ...
                if (e.HitInfo.Column != null)
                {
                    ...
                    if (e.HitInfo.Column.FieldName == "Checked")
                    {
                        if ((Boolean)gridViewSolicitud_det_pre_Aprobacion.GetFocusedRowCellValue(colChecked)) //verdadero
                        {
                            ...
                            gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(coldo_Cantidad, 0);
                            gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(col_saldo, 0); return;
                        }
                        else //false
                        {
                            ...
                            gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(coldo_Cantidad, row.cant_solicitada);
                            row.cant_ing_SolCom = Convert.ToDouble(gridViewSolicitud_det_pre_Aprobacion.GetFocusedRowCellValue(coldo_Cantidad));
                            gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(col_saldo, (row.cant_solicitada - row.cant_ing_SolCom));
        ...
        private void gridViewSolicitud_det_pre_Aprobacion_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                solicitudCompraInfo = (vwcom_solicitud_compra_x_items_con_saldos_Info)gridViewSolicitud_det_pre_Aprobacion.GetFocusedRow();
                if (e.Column.Name == "coldo_Cantidad")
                {
                    ...
                    {
                        //  gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(colChecked, false);
                        //solicitudCompraInfo.Checked = false;
                        gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(coldo_Cantidad, 0);
                        gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(col_saldo, 0);
                    }
                    else
                    {
                        //gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(colChecked, true);
                        //solicitudCompraInfo.Checked = true;
                        solicitudCompraInfo.cant_ing_SolCom =
                            Convert.ToDouble(gridViewSolicitud_det_pre_Aprobacion.GetFocusedRowCellValue(coldo_Cantidad));
                        gridViewSolicitud_det_pre_Aprobacion.SetFocusedRowCellValue(col_saldo, (solicitudCompraInfo.cant_solicitada - solicitudCompraInfo.cant_ing_SolCom));
                    }
                }
            }
            catch (Exception ex)
