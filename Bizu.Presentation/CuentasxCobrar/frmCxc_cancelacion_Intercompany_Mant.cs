...
{
    public partial class frmCxc_cancelacion_Intercompany_Mant : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void Get()
        {
            try
            {
                ...
                _InfoCobro.cr_Banco = (cmbBancos.Text == "Bizu.Domain.General.tb_banco_InfoCobro" || cmbBancos.Text == "") ? null : (cmbBancos.Text.ToString()).Trim();
                _InfoCobro.IdBanco = Convert.ToInt32(cmbBancoCta.SelectedValue);
                _InfoCobro.cr_cuenta = txtNumCuenta.Text.ToString();
                //_InfoCobro.cr_Tarjeta = (cmbTarjetas.Text == "Bizu.Domain.General.tb_tarjeta_InfoCobro") ? " " : (cmbTarjetas.Text.ToString()).Trim();
        ...
        public Boolean GETINFO() {
            try
            {
                ...
                _Info.cr_Banco = (cmbBancos.Text == "Bizu.Domain.General.tb_banco_Info" || cmbBancos.Text == "") ? null : (cmbBancos.Text.ToString()).Trim();
                //_Info.IdBanco = Convert.ToInt32(cmbBancoCta.SelectedValue);
                _Info.cr_cuenta = txtNumCuenta.Text.ToString();
                _Info.cr_Tarjeta = (cmbTarjetas.Text == "Bizu.Domain.General.tb_tarjeta_Info" || cmbTarjetas.Text=="") ? null : (cmbTarjetas.Text.ToString()).Trim();
                ...
                switch (_Accion)
                {
                    case Cl_Enumeradores.eTipo_action.grabar:
                        if (Guardar() == true)
                        {
                            ...
                            string smensaje = string.Format("{0} # {1} se guardó con éxito.", "La Cancelación ", _Info.IdCancelacion);
                            ...
                            foreach (var item1 in G_ListadoDet_Cobros_Inter)
                            {
                                ...
                                {
                                    string mensaje = string.Format("{0} # {1} se guardó con éxito.", "La Cuenta por Cobrar", _InfoCobro.IdCobro);
                                ...
                                {
                                    MessageBox.Show("Error al Grabar:  {0}", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
                    ...
                    case Cl_Enumeradores.eTipo_action.actualizar:
                        ...
                        if (CancInterBus.GuardarDB(_Info)==true)
                        {
                            string smensaje = string.Format("{0} # {1} se modificó con éxito.", "La Cancelación", _Info.IdCancelacion);
                            ...
                            foreach (var item1 in G_ListadoDet_Cobros_Inter)
                            {
                                ...
                                {
                                    string mensaje2 = string.Format("{0} # {1} se modificó con éxito.", "La Cuenta por Cobrar ", _InfoCobro.IdCobro);
                        ...
                        {
                            MessageBox.Show("Error al Modificar:   {0}",param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }   
                        //Actualizar detalle de cancelacion intercompany Derek Mejía 22/02/2014
