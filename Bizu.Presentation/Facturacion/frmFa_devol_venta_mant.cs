...
{
    public partial class frmFa_devol_venta_mant : DevExpress.XtraEditors.XtraForm
    {
       ...
       // UCFa_Cliente_Facturacion UCCliente = new UCFa_Cliente_Facturacion();
        ...
        Boolean guardarDatos()
        {
            try
            {
                ...
                if (validarDatos())
                {
                    ...
                    {
                        bolResult = true;
                        string smensaje = string.Format("{0} # {1} se guardó con éxito.", "La Devolución", infoDev.IdDevolucion);
            ...
            try
            {
                ...
                if (MessageBox.Show("La Devolución " + txtidDev.Text + "\n" + "¿Desea Continuar?", "Anular", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                        ...
                        param.Fecha_Transac, frm.motivoAnulacion, ref msg))
                    {
                        ...
                        string smensaje = string.Format("{0} # {1} se anuló con éxito.", "La Devolución", txtidDev.Text);
                        MessageBox.Show(smensaje, param.Nombre_sistema);  
        ...
        private void frmFA_devol_venta_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                UCSucursal.TipoCarga = Bizu.Domain.General.Cl_Enumeradores.eTipoFiltro.Normal;
            ...
            try
            {
                ...
                if(rbNumDoc.Checked==true)
                    ...
                    else
                    {
                        gridDevolucion.DataSource = listDetTabla;
                        txtNumDoc.Enabled = false;
                        this.ucGe_Menu.Visible_bntGuardar_y_Salir = this.ucGe_Menu.Visible_btnGuardar = DevExpress.XtraBars.BarItemVisibility.Always;
