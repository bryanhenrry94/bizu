...
{
    public partial class frmFa_Devolucion_x_NotaCredito_mant : Form
    {
        ...
        fa_notaCreDeb_det_bus Bus_fa_notaCredDeb_det = new fa_notaCreDeb_det_bus();
        ...
        BindingList<vwfa_Nota_Credito_x_Devolver_Info> ListBindig_Info_Doc_rel = new BindingList<vwfa_Nota_Credito_x_Devolver_Info>();
        BindingList<fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info> ListBinding_Ing_Egr_Inven_det = new BindingList<fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info>();
        List<fa_notaCreDeb_det_Info> Info_NotaCred_det = new List<fa_notaCreDeb_det_Info>();
        ...
        fa_devol_venta_Info Info_DevolucionVenta { get; set; }                
        fa_notaCreDeb_Info NDCInfo = new fa_notaCreDeb_Info();
        vwfa_Nota_Credito_x_Devolver_Info Info_Doc_rel = new vwfa_Nota_Credito_x_Devolver_Info();
        in_Ing_Egr_Inven_det_Info InfoDet = new in_Ing_Egr_Inven_det_Info();
        fa_notaCreDeb_x_in_Ing_Egr_Inven_Info Info_Documento_Relacionado = new fa_notaCreDeb_x_in_Ing_Egr_Inven_Info();
        ...
        public frmFa_Devolucion_x_NotaCredito_mant()
        {
            try
            {
                InitializeComponent();
        ...
        private void frmFA_devol_venta_Load(object sender, EventArgs e)
        {
            try
            {
              ...
              //  pnlNumDoc.Controls.Add(UCNumDoc);
                ...
                gridControlDocumentosRelacionados.DataSource = ListBindig_Info_Doc_rel;
        ...
        public void load_Producto()
        {
            try
            {
                ...
                cmbProducto_grid.DataSource = Lista_producto;
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        void carga_Combos_Productos_y_Tipo_Movi()
        {
            try
            {
                if (UCSucursal.get_IdBodega() != 0)
                {

                    cmbTipoMovi.cargar_TipoMotivo(Convert.ToInt32(UCSucursal.get_IdSucursal()), Convert.ToInt32(UCSucursal.get_IdBodega()), "+", "");

                    listProducto = Bus_Producto.Get_list_Producto_modulo_x_inventario(param.IdEmpresa,
                        Convert.ToInt32(UCSucursal.get_IdSucursal()), Convert.ToInt32(UCSucursal.get_IdBodega()));
        ...
        public void importarDatos()
        {
            try
            {
               ...
               //         limpiar();
               //         this.ucGe_Menu.Visible_bntGuardar_y_Salir = this.ucGe_Menu.Visible_btnGuardar =  false;
               //     }
               //     else
               //     {
               //         gridDevolucion.DataSource = listDetTabla;
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal IdCliente = Convert.ToDecimal(UCCliente.cmb_cliente.EditValue);

                if (IdCliente != 0)
                {
                    frmFa_NotaCred_a_cruzar frm = new frmFa_NotaCred_a_cruzar();
                    frm.Cargar_combos(IdEmpresa, IdCliente);
                    frm.IdCliente = IdCliente;

                    frm.ShowDialog();
                    Info_Doc_rel = frm.Get_Info();

                    int cont = ListBindig_Info_Doc_rel.Where(q => q.IdEmpresa == Info_Doc_rel.IdEmpresa
                        && q.IdSucursal == Info_Doc_rel.IdSucursal && q.IdBodega == Info_Doc_rel.IdBodega
                        && q.IdNota == Info_Doc_rel.IdNota).Count();

                    if (cont == 0)
                    {
                        if (Info_Doc_rel != null)
                        {
                            ListBindig_Info_Doc_rel.Add(Info_Doc_rel);

                            Info_NotaCred_det = Bus_fa_notaCredDeb_det.Get_List_notaCreDeb_det(Info_Doc_rel.IdEmpresa,
                                Info_Doc_rel.IdSucursal, Info_Doc_rel.IdBodega, Info_Doc_rel.IdNota);

                            foreach (var item in Info_NotaCred_det)
                            {
                                fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info info = new fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info();
                                info.no_IdEmpresa = item.IdEmpresa;
                                info.no_IdSucursal = item.IdSucursal;
                                info.no_IdBodega = item.IdBodega;
                                info.no_IdNota = item.IdNota;
                                info.no_Secuencia = item.Secuencia;

                                info.IdProducto = item.IdProducto;
                                info.dm_cantidad_sinConversion = item.sc_cantidad;
                                info.dm_cantidad = item.sc_cantidad;
                                info.mv_costo_sinConversion = 0;
                                info.mv_costo = 0;
                                info.dm_observacion = item.sc_observacion;
                                //info.IdCentroCosto = item.IdCentroCosto;
                                //info.IdCentroCosto_sub_centro_costo = item.IdCentroCosto_sub_centro_costo;
                                info.IdPunto_cargo = 0;
                                info.IdRegistro = 0;
                                info.IdUnidadMedida = 0;
                                info.IdUnidadMedida_sinConversion = 0;                                
                                //info.IdPunto_cargo_grupo = item.IdPunto_cargo_grupo;

                                ListBinding_Ing_Egr_Inven_det.Add(info);
                                gridControlProductos.Refresh();
                            }
                        }
                    }
                    else
        ...
        private void gridViewDocumentosRelacionados_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Delete)
                {
                    ...
                    {
                        List<fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info> List = new List<fa_notaCreDeb_det_x_in_Ing_Egr_Inven_det_Info>(ListBinding_Ing_Egr_Inven_det);

                        foreach (var item in List)
                        {
                            if (item.no_IdEmpresa == Info_Doc_rel.IdEmpresa && Info_Doc_rel.IdSucursal == item.no_IdSucursal
                                && Info_Doc_rel.IdBodega == item.no_IdBodega && item.no_IdNota == Info_Doc_rel.IdNota)
                                ListBinding_Ing_Egr_Inven_det.Remove(item);
                        }

                        gridViewDocumentosRelacionados.DeleteRow(gridViewDocumentosRelacionados.FocusedRowHandle);
                        gridControlDocumentosRelacionados.RefreshDataSource();
                    }
                }             
            }
            catch (Exception ex)
        ...
        private void gridViewProductos_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Delete)
