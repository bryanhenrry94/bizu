...
{
    public partial class frmFa_motivo_venta_Consulta : DevExpress.XtraEditors.XtraForm
    {
        #region "Variables"
        ...
        private void frmFa_motivo_venta_Consulta_Load(object sender, EventArgs e)
        {
            ...
            {
                //Control de Usuario
                Infoseg = Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(param.IdEmpresa, param.IdUsuario, "Facturacion.frmFa_motivo_venta_Consulta");
                foreach (var item in Infoseg)
                {
                    if (item.Eliminacion == false)
                    {
                        ucGe_Menu_Mantenimiento_MotivoVenta.btnAnular.Enabled = false;
                    }
                    if (item.Escritura == false)
                    {
                        ucGe_Menu_Mantenimiento_MotivoVenta.btnModificar.Enabled = false;
                    }
                }
                LLenar_Grid();
            }
