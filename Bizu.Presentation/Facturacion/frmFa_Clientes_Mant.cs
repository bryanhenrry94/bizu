...
{
    public partial class frmFa_Clientes_Mant : DevExpress.XtraEditors.XtraForm
    {
        ...
        List<fa_catalogo_Info> List_EstadoObra = new List<fa_catalogo_Info>();
        fa_cliente_Obra_Bus Bus_Cliente_Obra = new fa_cliente_Obra_Bus();
        List<fa_cliente_Obra_Info> List_Cliente_Obra = new List<fa_cliente_Obra_Info>();
        BindingList<fa_cliente_Obra_Info> BindingList_Cliente_Obra = new BindingList<fa_cliente_Obra_Info>();
        ...
        public fa_Cliente_Info get_Cliente()
        {
            try
            {
                ...
                //Actualizado J.A 11-07-2018
                if (txt_apellidos.Text.Trim() != "" && txt_nombres.Text.Trim() != "")
                {
                ...
                {
                    string nomCom = "";
                    nomCom = this.txt_apellidos.Text + " " + this.txt_nombres.Text;
                    info_persona.pe_nombreCompleto = nomCom.Trim();
                }
                ...
                info_persona.pe_correo = info.Mail_Principal;
                info_persona.pe_correo_secundario1 = info.Mail_Secundario1;
        ...
        private Boolean guardar(ref string MensajeError)
        {
            try
            {
                ...
                if (get_Cliente() != null)
                {
                    List_Cliente_Obra = new List<fa_cliente_Obra_Info>(BindingList_Cliente_Obra);
                    ...
                    switch (_Accion)
                    {
                        case Cl_Enumeradores.eTipo_action.grabar:
                            ...
                            if (IdCliente_Registrado == true)
                            {
                                ...
                                if (Res)
                                {
                                    //ELIMINAMOS LOS REGISTROS DE LAS OBRAS EN RELACION AL CLIENTE
                                    Bus_Cliente_Obra.EliminarDB(info.IdEmpresa, info.IdCliente);

                                    //VOLVEMOS A REGISTRAR LAS OBRAS X CLIENTE
                                    if (List_Cliente_Obra.Count() > 0)
                                    {
                                        foreach (var item in List_Cliente_Obra)
                                        {
                                            item.IdEmpresa = param.IdEmpresa;
                                            item.IdCliente = info.IdCliente;
                                        }

                                        if (Bus_Cliente_Obra.GrabarDB(List_Cliente_Obra, ref MensajeError))
                                        {
                                        }
                                    }
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    //ELIMINAMOS LOS REGISTROS DE LAS OBRAS EN RELACION AL CLIENTE
                                    Bus_Cliente_Obra.EliminarDB(info.IdEmpresa, id_cliente);
                            ...
                            {
                                string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El Cliente", info.IdCliente);
                        ...
                        case Cl_Enumeradores.eTipo_action.actualizar:
                            ...
                            {
                                string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El Cliente", info.IdCliente);
                                MessageBox.Show(smensaje, param.Nombre_sistema);
                                LimpiarDatos();
                            }
                            break;
                ...
                else
                {
                    ...
                    if (MessageBox.Show("¿Está seguro que desea anular el Cliente: " + "[" + info.IdCliente + "] -" + info.Persona_Info.pe_nombre + " " + info.Persona_Info.pe_apellido + " ?", "Anulación de Clientes", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        if (info.Estado == "A")
                        {
                            ...
                            {
                                string smensaje = string.Format("{0} # {1} se anuló con éxito.", "El Cliente", info.IdCliente);
                                MessageBox.Show(smensaje, param.Nombre_sistema);
                                ucGe_Menu.Visible_bntAnular = DevExpress.XtraBars.BarItemVisibility.Never;
        ...
        private Boolean validaciones()
        {
            try
            {
                if (_Accion == Cl_Enumeradores.eTipo_action.grabar)
                {
                    if (param.IdCliente_Ven_x_Default == Cl_Enumeradores.eCliente_Vzen.EDEHSA)
                    {
                        //NO VALIDAMOS EL NUMERO DE CEDULA POR EL CLIENTE EDEHSA
                    }
                    else
                    {
                        if (!ValidarCedula())
                ...
                if (cmb_plancta.cmbPlanCta.EditValue == null)
                {
                ...
                if (cmb_PlanCtaAnti.cmbPlanCta.EditValue == null)
                {
                ...
                if (cmb_PlanCta_Credito.cmbPlanCta.EditValue == null)
                {
                ...
                if (txtMailPrincipal.EditValue == null)
                {
