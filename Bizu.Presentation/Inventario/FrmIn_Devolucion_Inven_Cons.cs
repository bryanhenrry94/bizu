...
{
    public partial class FrmIn_Devolucion_Inven_Cons : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void FrmIn_Devolucion_Inven_Cons_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //Control de Usuario
                Infoseg = Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(param.IdEmpresa, param.IdUsuario, "Inventario.FrmIn_Devolucion_Inven_Cons");
                foreach (var item in Infoseg)
                {
                    if (item.Eliminacion == false)
                    {
                        UCMenu.btnAnular.Enabled = false;
                    }
                    if (item.Escritura == false)
                    {
                        UCMenu.btnModificar.Enabled = false;
                    }
                }
                cargar_grid();
            }
