...
{
    public partial class FrmIn_Ajuste_Inventario_Mant : DevExpress.XtraEditors.XtraForm
    {
       ...
       #region DECLARACION DE VARIABLES
            ...
            try
            {
                ...
                foreach (var item in BindList_Ing_egr_inve_det)
                {
                    if (item.IdEstadoAproba == null || item.IdEstadoAproba == Cl_Enumeradores.eEstadoAprobacion_Ing_Egr.PEND.ToString())
                    {
                        ...
                        info.mv_costo = item.mv_costo_sinConversion;
        ...
        private void limpiar()
        {
            ...
            try
            {
                ...
                _sucursalInfo = new tb_Sucursal_Info();
                _bodegaInfo = new tb_Bodega_Info();
        ...
        private void cargar_motivo(string eTipo_Ing_Egr)
        {
            try
            {
                cmbMotivoInv.cargar_MotivoInv(eTipo_Ing_Egr);             
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void opt_egreso_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {                
                cargar_tipoMovi_Inven(_sucursalInfo.IdSucursal, _bodegaInfo.IdBodega, "-", "");    
                cargar_motivo(ein_Tipo_Ing_Egr.EGR.ToString());
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void opt_ingreso_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cargar_motivo(ein_Tipo_Ing_Egr.ING.ToString());
            }
            catch (Exception ex)
            { Log_Error_bus.Log_Error(ex.ToString());
            MessageBox.Show(ex.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
       ...
       string ClipboardData
       {
        ...
        private void btn_Buscar_OC_Click(object sender, EventArgs e)
        {
            try
            {
                if (txt_IdOrdenCompra.EditValue == null || Convert.ToDecimal(txt_IdOrdenCompra.EditValue) == 0)
                {
                    MessageBox.Show("Ingrese el # de la Orden de Compra", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                lista_IngEgrInv = Bus_IngEgrDet.Get_List_Ing_Egr_Inven_det_x_OrdenCompra(param.IdEmpresa, Convert.ToInt16(UCSucurBod.cmb_sucursal.EditValue), Convert.ToDecimal(txt_IdOrdenCompra.EditValue));

                foreach (var item in lista_IngEgrInv)
                {
                    item.IdEstadoAproba = "PEND";
                }
