...
{
    public partial class frmCon_CentroCostos_Nivel_Mant : DevExpress.XtraEditors.XtraForm
    {
            ...
            try
            {
                if (validar())
                {
                    ...
                    if (_CentroCostoNivelBus.GrabarDB(_CentroCostoNivelInfo))
                    {
                        ...
                        string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El nivel ", _CentroCostoNivelInfo.IdNivel.ToString());
        ...
        private Boolean Modificar()
        {
            ...
            try
            {
                if (validar())
                {
                    ...
                    {
                        string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El nivel ", _CentroCostoNivelInfo.IdNivel.ToString());
                        MessageBox.Show(smensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        //MessageBox.Show("Grabado ok");
                        LimpiarDatos();
                    }
                    ...
                    if (_CentroCostoNivelInfo.IdNivel != 0)
                    {
                        ...
                        else
                        {
                            if (MessageBox.Show("¿Está seguro que desea anular el Nivel #: " + _CentroCostoNivelInfo.IdNivel + " ?", "ANULACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                {
                                    string smensaje = string.Format("{0} # {1} se anuló con éxito.", "El Nivel ", _CentroCostoNivelInfo.IdNivel.ToString());
