                ...
                if (validar())
                {
                    ...
                    if (_Accion == Cl_Enumeradores.eTipo_action.grabar)
                    {
                        if (BusTipoCble.GrabarDB(InfoTipoCble, ref MensajeError))
                        {
                            ...
                            string smensaje = string.Format("{0} # {1} se guardó con éxito.", "Tipo de Comprobante Contable ", InfoTipoCble.IdTipoCbte);
                    ...
                    if (_Accion == Cl_Enumeradores.eTipo_action.actualizar)
                    {
                        ...
                        {
                            string smensaje = string.Format("{0} # {1} se modificó con éxito.", "Tipo de Comprobante Contable ", txtidtipocbte.Text);
                            MessageBox.Show(smensaje, param.Nombre_sistema);
                            //ucGe_Menu.Visible_bntGuardar_y_Salir = DevExpress.XtraBars.BarItemVisibility.Never;
                            //ucGe_Menu.Visible_btnGuardar = DevExpress.XtraBars.BarItemVisibility.Never;
                            LimpiarDatos();
                    ...
                    if (_Accion == Cl_Enumeradores.eTipo_action.Anular)
                    {
                        if (InfoTipoCble.tc_Estado != "I")
                        {
                            if (MessageBox.Show("¿Está seguro que desea anular el Tipo de Comprobante Contable #: " + txtidtipocbte.Text + " ?", "Anulación de Tipo de Comprobante Contable", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                {
                                    string smensaje = string.Format("{0} # {1} se anuló con éxito.", "Tipo de Comprobante Contable ", txtidtipocbte.Text);
                                    MessageBox.Show(smensaje, param.Nombre_sistema);
