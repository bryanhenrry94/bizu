...
{
    public partial class frmCon_cbtecble_Plantilla : DevExpress.XtraEditors.XtraForm
    {
        ...
        private Boolean Grabar()
        {
            try
            {
                ...
                if (bolResult)
                {
                    ...
                    string smensaje = string.Format("{0} # {1} se guardó con éxito.", "Plantilla ", IdPlantilla);
        ...
        Boolean Actualizar()
        {
            try
            {
                ...
                if (bolResult)
                {
                    string smensaje = string.Format("{0} # {1} se modificó con éxito.", "La Plantilla", Info_Plantilla.IdPlantilla);
                    MessageBox.Show(smensaje, param.Nombre_sistema);
                    bolResult = true;
                    LimpiarDatos();
                }
                    ...
                    else
                    {
                        if (MessageBox.Show("¿Está seguro que desea anular La plantilla #: " + Info_Plantilla.IdPlantilla + " ?", "Anulación de Plantilla ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            if (Info_Plantilla.cb_Estado == "A")
                            {
                                ...
                                {
                                    string smensaje = string.Format("{0} # {1} se anuló con éxito.", "La Plantilla", Info_Plantilla.IdPlantilla);
                                    MessageBox.Show(smensaje, param.Nombre_sistema);
                                    ucGe_Menu.Visible_bntAnular = DevExpress.XtraBars.BarItemVisibility.Never;
