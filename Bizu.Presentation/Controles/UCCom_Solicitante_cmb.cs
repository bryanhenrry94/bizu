...
namespace Bizu.Presentation.Controles
{
    public partial class UCCom_Solicitante_cmb : UserControl
    {
        ...
        public decimal get_SolicitanteInfoId(string nombre)
        {
            decimal id = 0;
            try
            {
               
                InfoSolicitante = listSolicitante.FirstOrDefault(v => v.nom_solicitante == nombre.ToString());
                if (InfoSolicitante.IdSolicitante != 0)
                {
                    id = InfoSolicitante.IdSolicitante;
                }
                return id;
            }
            catch (Exception ex)
