...
using Bizu.Presentation.CuentasxCobrar;

using DevExpress.XtraReports.UI;

namespace Bizu.Presentation.CuentasxPagar
{
    public partial class frmCP_anticipo_Proveedores_Mant : DevExpress.XtraEditors.XtraForm
    {
        #region Variables
        ...
        public frmCP_anticipo_Proveedores_Mant()
        {
            try
            {
                InitializeComponent();                
        ...
        void ucGe_Menu_event_btnImprimirSoporte_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (cobroAntiinfo.listCobros.First().IdCobro_tipo == "DEPO")
                //{
                //    Bizu.Presentation.Bancos.FrmBA_DepositoBancario_Mant frm = new Bizu.Presentation.Bancos.FrmBA_DepositoBancario_Mant();

                //    ba_Cbte_Ban_Info CbteBan_I = new ba_Cbte_Ban_Info();
                //    CbteBan_I.IdEmpresa = param.IdEmpresa;
                //    CbteBan_I.IdTipocbte = 3;
                //    CbteBan_I.IdCbteCble = 3;
                   
                //    frm.Imprimir_Deposito(CbteBan_I);
                //}
            }
            catch (Exception ex)
            {
        ...
        public Boolean Grabar()
        {
            try
            {
                ...
                foreach(var item in cxccinfoLst)
                {
                if(item.IdCobro_tipo == null)
                {
                    MessageBox.Show("Seleccione un tipo de cobro ", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                ...
                string mensaje = string.Format("El registro ha sido guardado con éxito","El anticipo ",cobroAntiinfo.IdAnticipo);
