...
{
    public partial class frmCP_NotaDebito_Mant : DevExpress.XtraEditors.XtraForm
    {
        #region Variables
        ...
        cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Bus Bus_Aprob = new cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Bus();
        ...
        List<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info> list_Aprob = new List<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info>();
        BindingList<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info> lstBind = new BindingList<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info>();
        ...
        void cargar_Combos()
        {
            try
            {
                ...
                list_centroCosto = _CentroCostoBus.Get_list_Centro_Costo_cuentas_de_movimiento(param.IdEmpresa, ref MensajeError);
        ...
        public void get_notaCr()
        {
            try
            {
                ...
                if (lstBind.Count > 0)
                {
                    // Asignamos todos los registros de la lista
                    List<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info> ListAprobacionDet = new List<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info>(lstBind);
                    // Filtramos solo los registros con check
                    Info_notaCredDeb.ListAprobacionIngBodDet = ListAprobacionDet.Where(q => q.Checked == true).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        private void SetAprobacionXOC()
        {
            try
            {
                // Si el objeto Info_notaCredDeb es null sale de la funcion
                if (this.Info_notaCredDeb == null) return;

                // Consulta registro en la base de datos por los datos de la nota de debito
                list_Aprob = Bus_Aprob.GetList(this.Info_notaCredDeb.IdEmpresa, this.Info_notaCredDeb.IdCbteCble_Nota, this.Info_notaCredDeb.IdTipoCbte_Nota);
                
                // Convierte la lista a un bindinglist y asigna el datasource al gridcontrol
                lstBind = new BindingList<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info>(list_Aprob);
                gridControlAproIngEgrxOC.DataSource = lstBind;

                // Calcula totales
                calculos(lstBind);

                // Desactiva boton de busqueda y establece el gridcontrol en solo lectura
                btn_buscar_ingresos.Enabled = false;
                gridControlAproIngEgrxOC.Enabled = false; 
                //gridViewAproIngEgrxOC.OptionsBehavior.ReadOnly = true;
            }
            catch (Exception ex)
            {
        ...
        private Boolean Grabar()
        {
            ...
            try
            {
                ...
                if (res)
                {
                    #region graba aprobacion

                    // Si existe registro de aprobacion
                    if (Info_notaCredDeb.ListAprobacionIngBodDet.Count() > 0)
                    {
                        cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_Info InfoAprobacion = new cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_Info();
                        InfoAprobacion.IdEmpresa = param.IdEmpresa;
                        InfoAprobacion.IdAprobacion = 0;
                        InfoAprobacion.FechaAprobacion = Info_notaCredDeb.cn_fecha;
                        InfoAprobacion.IdEmpresa_Nota = Info_notaCredDeb.IdEmpresa;
                        InfoAprobacion.IdCbteCble_Nota = Info_notaCredDeb.IdCbteCble_Nota;
                        InfoAprobacion.IdTipoCbte_Nota = Info_notaCredDeb.IdTipoCbte_Nota;
                        InfoAprobacion.Subtotal_iva = Info_notaCredDeb.cn_subtotal_iva;
                        InfoAprobacion.Subtotal_siniva = Info_notaCredDeb.cn_subtotal_siniva;
                        InfoAprobacion.Descuento = 0;
                        InfoAprobacion.BaseImponible = Info_notaCredDeb.cn_baseImponible;
                        InfoAprobacion.Por_iva = Info_notaCredDeb.cn_Por_iva;
                        InfoAprobacion.Valor_iva = Info_notaCredDeb.cn_valoriva;
                        InfoAprobacion.Total = Convert.ToDouble(Info_notaCredDeb.cn_total);

                        int iSecuencia = 0;

                        foreach (var item in Info_notaCredDeb.ListAprobacionIngBodDet)
                        {
                            iSecuencia++;

                            cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info InfoAprobacionDet = new cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info();
                            InfoAprobacionDet.IdEmpresa = InfoAprobacion.IdEmpresa;
                            InfoAprobacionDet.IdAprobacion = InfoAprobacion.IdAprobacion;
                            ...
                            InfoAprobacion.ListAprobacionDet.Add(InfoAprobacionDet);
                        }

                        cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_Bus BusAprobaIngBodxOC = new cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_Bus();
                        cp_nota_DebCre_x_com_ordencompra_local_Bus BusNotaDebXOC = new cp_nota_DebCre_x_com_ordencompra_local_Bus();
                        BusAprobaIngBodxOC.GrabarBD(InfoAprobacion);

                        // Agrupa el detalle de las aprobaciones por Orden de Compra
                        var query_agrupado_ing_x_oc = from T in Info_notaCredDeb.ListAprobacionIngBodDet
                                      group T by new
                                      {
                                          T.IdEmpresa,
                                          T.IdSucursal_OC,
                        ...
                        foreach (var item in query_agrupado_ing_x_oc)
                        {
                            cp_nota_DebCre_x_com_ordencompra_local_Info InfoNotaXOC = new cp_nota_DebCre_x_com_ordencompra_local_Info();
                            InfoNotaXOC.ndc_IdEmpresa = Info_notaCredDeb.IdEmpresa;
                            InfoNotaXOC.ndc_IdCbteCble_Nota = Info_notaCredDeb.IdCbteCble_Nota;
                            InfoNotaXOC.ndc_IdTipoCbte_Nota = Info_notaCredDeb.IdTipoCbte_Nota;
                            InfoNotaXOC.com_IdEmpresa = Info_notaCredDeb.IdEmpresa;
                            InfoNotaXOC.com_IdSucursal = item.Key.IdSucursal_OC;
                            InfoNotaXOC.com_IdOrdenCompraLocal = item.Key.IdOrdenCompra;
                            InfoNotaXOC.og_Observacion = "";

                            BusNotaDebXOC.GrabarBD(InfoNotaXOC);
                        }

                        #endregion
                    }

                    string smensaje = string.Format("{0} # {1} se guardó con éxito.", "Nota de Débito ", Info_notaCredDeb.IdCbteCble_Nota);
        ...
        private Boolean Modificar()
        {
            ...
            try
            {
                ...
                if (res)
                {
                    string smensaje = string.Format("{0} # {1} se modificó con éxito.", "Nota de Débito ", txE_NotaCre.EditValue);
                    MessageBox.Show(smensaje, param.Nombre_sistema);
                    Limpiar();
                }
                else
            ...
            try
            {
                ...
                if (Info_notaCredDeb != null)
                {
                    ...
                    if (Info_notaCredDeb.Estado == "A")
                    {
                        ...
                        {
                            string smensaje = string.Format("Error al Anular:  {0}", mensaje);
                            MessageBox.Show(smensaje, param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
        ...
        private void GeneraDiario()
        {
            try
            {
                ...
                var detalle_aprobacion = lstBind.Where(q => q.Checked).ToList();

                var lstDet_OG = from ing_ in lstBind
                                where ing_.Checked == true
                                group ing_ by new
                                {
                ...
                foreach (var item in lstDet_OG)
                {
                    ...
                    InfoGasto.IdEmpresa = param.IdEmpresa;
                    //InfoGasto.IdTipoCbte = info_cpParam.pa_TipoCbte_OG;
                    //InfoGasto.secuencia = iSecuencia;
                    InfoGasto.IdCtaCble = item.Key.IdCtaCble_Gasto;
                    InfoGasto.IdCentroCosto = item.Key.IdCentro_Costo_x_Gasto_x_cxp;
                    InfoGasto.IdCentroCosto_sub_centro_costo = item.Key.IdCentroCosto_sub_centro_costo_cxp;
                    InfoGasto.IdPunto_cargo_grupo = item.Key.IdPunto_cargo_grupo;
                    InfoGasto.IdPunto_cargo = item.Key.IdPunto_cargo;
                    InfoGasto.dc_Valor = item.Subtotal;
                    InfoGasto.dc_Observacion = Convert.ToString(txt_observacion.EditValue).Trim();
                    InfoGasto.dc_Numconciliacion = null;
                    InfoGasto.dc_EstaConciliado = null;
                    InfoGasto.dc_para_baseImponible = true;

                    ListCbteDet.Add(InfoGasto);
                }

                // Asigna lista de diarios al control
        ...
        private void cmbTipoNota_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Convert.ToString(cmbTipoNota.EditValue) == "T_TIP_NOTA_TARJETA")
        ...
        private void txt_observacion_EditValueChanged(object sender, EventArgs e)
        {

        }

        private void group_aut_Enter(object sender, EventArgs e)
        {
        ...
        private void dtE_Fecha_vencimiento_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private void dtE_Fecha_Contabilizacion_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private void buscar()
        {
            try
            {
                if (_Accion == Cl_Enumeradores.eTipo_action.consultar) return;

                list_Aprob = new List<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info>();
        ...
        private void gridViewAproIngEgrxOC_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info fila_OC_det = (cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info)gridViewAproIngEgrxOC.GetFocusedRow();
        ...
        private void chk_aprobacion_ing_x_oc_CheckedChanged(object sender, EventArgs e)
        {
        }
        ...
        private void chk_selec_op_visible_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    gridViewAproIngEgrxOC.SetRowCellValue(i, colChecked, chk_selec_op_visible.Checked);
                }

                calculos(lstBind);

                //if (MessageBox.Show("Desea actualizar diario contable?", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                //GeneraDiarioxAprobIngxOC();
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void calculos(BindingList<cp_nota_DebCre_Aprobacion_Ing_Bod_x_OC_det_Info> ListaBind)
        {
            try
            {
                ...
                cp_proveedor_Info Info_Proveedor = ucCp_Proveedor1.get_ProveedorInfo();

                dias_plazo = Info_Proveedor == null ? 0 : Convert.ToInt32(Info_Proveedor.pr_plazo);
                txt_observacion.Text = "";
                ...
                foreach (var item in ListaBind)
                {
                    if (item.Checked == true)
                    {
                        if (string.IsNullOrEmpty(txt_observacion.Text))
                            txt_observacion.Text = Info_Proveedor.pr_nombre + " FACT. " + txeNumDocum.EditValue + " - O/C " + item.IdOrdenCompra + " - P/I" + item.IdNumMovi_Ing_Egr_Inv + " - OBRA " + item.IdCentro_Costo + " - " + item.nom_producto;
                        else
                            txt_observacion.Text = (txt_observacion.Text + " | P/I " + item.IdNumMovi_Ing_Egr_Inv + " - OBRA " + item.IdCentro_Costo + " - " + item.nom_producto);
        ...
        private void gridViewAproIngEgrxOC_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            try
            {
        ...
        private void txE_SumatoriaOG_EditValueChanged(object sender, EventArgs e)
        {
