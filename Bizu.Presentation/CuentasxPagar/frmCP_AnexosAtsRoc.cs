...
using Bizu.Domain.CuentasxPagar.xmlREOC;
using Bizu.Application.Facturacion;
using Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4;

namespace Bizu.Presentation.CuentasxPagar
{
    public partial class frmCP_AnexosAtsRoc : DevExpress.XtraEditors.XtraForm
    {
        ...
        private iva CargaDatos()
        {
            ...
            try
            {
                ...
                List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras> lstDetalleCompras = new List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras>();
                ...
                //facturas de proveedores mas retenciones fuente e iva
                #region "Facturas cxp de Proveedor"
                foreach (cp_orden_giro_Info item in listaOG)
                {
                    try
                    {
                        contad = contad + 1;
                        Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras itemDetalleCompras = new Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras();
                        ...
                        List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleAir> LstdetalleAir = new List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleAir>();
                        ...
                        if (info_Retencion_x_RF.ListDetalle.Count != 0)
                        {
                            ...
                            {
                                Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleAir detalleAir = new Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleAir();
                        ...
                        if (item.PagoLocExt == "EXT")
                        {
                            itemDetalleCompras.denoProv = item.InfoProveedor.Persona_Info.pe_razonSocial;
                        }

                        itemDetalleCompras.parteRel = item.InfoProveedor.es_empresa_relacionada ? parteRelType.SI : parteRelType.NO;
                        ...
                        if (itemDetalleCompras.tpIdProv == "03")
                        {
                            ...
                            itemDetalleCompras.parteRel = !item.InfoProveedor.es_empresa_relacionada ? parteRelType.NO : parteRelType.SI;
                        }
                        ...
                        {
                            item.ConvenioTributacion = "NA";
                            item.PagoSujetoRetencion = "NA";
                        }
                        else
                        {
                            cp_orden_giro_Info Info_Ogiro_TMP = OGB.Get_Info_orden_giro(item.IdEmpresa, item.IdTipoCbte_Ogiro, item.IdCbteCble_Ogiro);
                            item.ConvenioTributacion = Info_Ogiro_TMP.ConvenioTributacion;
                            item.PagoSujetoRetencion = Info_Ogiro_TMP.PagoSujetoRetencion;
                        }

                        Item_pagoExterior.paisEfecPagoGen = (Item_pagoExterior.pagoLocExt == pagoLocExtType.Item01) ? "NA" : (item.PaisPago != null || item.PaisPago != "") ? item.PaisPago : "NA";
                        Item_pagoExterior.aplicConvDobTrib = (item.ConvenioTributacion == "SI") ? aplicConvDobTribType.SI : (item.ConvenioTributacion == "NO") ? aplicConvDobTribType.NO : aplicConvDobTribType.NA;

                        if (Item_pagoExterior.aplicConvDobTrib == aplicConvDobTribType.SI)
                        {
                            Item_pagoExterior.pagExtSujRetNorLeg = aplicConvDobTribType.NA;
                        }
                        else
                        {
                            Item_pagoExterior.pagExtSujRetNorLeg = (item.PagoSujetoRetencion == "SI") ? aplicConvDobTribType.NO : (item.PagoSujetoRetencion == "NO") ? aplicConvDobTribType.SI : aplicConvDobTribType.NA;
                        }
                ...
                //*************************************
                #region "notas de debito y credito cxp"
                ...
                foreach (cp_nota_DebCre_Info item in listaNotaDC)
                {
                    ...
                    itemDetalleCompras.valRetServ50Specified = true;
                    itemDetalleCompras.totbasesImpReembSpecified = true;
                    if (LstdetalleAir.Count > 0) itemDetalleCompras.air = LstdetalleAir;
                    ...
                    else
                    {
                        ...
                        cp_nota_DebCre_Info Info_Nota_CreDeb_TMP = nota_B.Get_Info_nota_DebCre(item.IdEmpresa, item.IdCbteCble_Nota, item.IdTipoCbte_Nota);
                        item.ConvenioTributacion = Info_Nota_CreDeb_TMP.ConvenioTributacion;
                        item.PagoSujetoRetencion = Info_Nota_CreDeb_TMP.PagoSujetoRetencion;
                    }
                ...
                List<detalleExportacionesType> LstDVExport = new List<detalleExportacionesType>();
                ...
                if (LstDVExport.Count() > 0)
                    iva.exportaciones = LstDVExport;
        ...
        private void btn_procesar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.DS_Iva_REOC_XML = new iva();
                this.DS_Iva_REOC_XML = this.CargaDatos();
                string ruta = "";
                this.saveFileDialog1.FileName = "AT-" + param.InfoEmpresa.codigo + "-" + ((cmb_periodo.SelectedValue.ToString().Length < 2) ? "0" + cmb_periodo.SelectedValue.ToString() : cmb_periodo.SelectedValue.ToString()) + cmb_anio.Text + ".xml";
                this.saveFileDialog1.Filter = "All files (*.xlm*)|*.xlm*";
                this.saveFileDialog1.InitialDirectory = @direcDoc;
                this.saveFileDialog1.ShowDialog();
                ruta = this.saveFileDialog1.FileName;

                if (SerializeToXML(this.DS_Iva_REOC_XML, ruta))
                {
                    this.progressBar1.Value = 100;
                    string msg = "";

                    if (this.validaXmlConXsd(ruta, Domain.Properties.Resources.ats_xsd_V_1_1_4, ref msg))
                    {
                ...
                List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras> List_Compras = new List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleCompras>();
                List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas> List_Ventas = new List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas>();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (!gridControlRT.IsPrintingAvailable)
                {
                    MessageBox.Show("No se encuentra libreria 'DevExpress.XtraPrinting'", "Error");
                    return;
                }
                else
                {
                    gridControlRT.ShowPrintPreview();
                }

                if (!gridControlCompras.IsPrintingAvailable)
                {
                    MessageBox.Show("No se encuentra libreria 'DevExpress.XtraPrinting'", "Error");
                    return;
                }
                ...
                {
                    gridControlCompras.ShowPrintPreview();
                }

                if (!gridControlVentas.IsPrintingAvailable)
                {
                    MessageBox.Show("No se encuentra libreria 'DevExpress.XtraPrinting'", "Error");
                    return;
                }
                ...
                {
                    gridControlVentas.ShowPrintPreview();
                }
            }
            catch (Exception ex)
            {
