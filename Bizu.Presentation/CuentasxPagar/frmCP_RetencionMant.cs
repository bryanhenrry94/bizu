...
{
    public partial class frmCP_RetencionMant : DevExpress.XtraEditors.XtraForm
    {
        decimal IdProveedor;
        ...
        void ucGe_Menu_event_btn_Generar_XML_Click(object sender, EventArgs e)
        {
            try
            {
                try
                {
                    ...
                    cp_retencion_Bus busRet = new cp_retencion_Bus();
                    List<vwcp_Retencion_sri_Info> lista_Retencion_sri = new List<vwcp_Retencion_sri_Info>();
                    lista_Retencion_sri = busRet.Get_Lis_reten(param.IdEmpresa, Convert.ToDecimal(ucCp_Retencion1.txtIdRetencion.EditValue), ref  mensaje);
                    foreach (var item in lista_Retencion_sri)
                    {
                        Cl_ValidarEmail_Info datosAdc = new Cl_ValidarEmail_Info();

                        if (datosAdc.email_bien_escrito(item.pe_correo.Trim()) == true) { }
                        else
                        {
                            if (MessageBox.Show("El correo del proveedor: '' " + item.pe_nombreCompleto + " '', No está bien escrito,  '' " + item.pe_correo + " '', Desea generar el xml sin adjuntar el correo", "Mensaje de sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) { }
                            else
                            {
                                return;
                            }
                            ...
                            switch (param.IdCliente_Ven_x_Default)
                            {
                                case Cl_Enumeradores.eCliente_Vzen.EDEHSA:
                                    if (item2.Tipo == "IVA")
                                    {
                                        ...
                                        info.MontoRetencion = Math.Round(Convert.ToDouble(info.BaseImponible * (item2.co_porRetencion / 100)), 2);
                                    }
                                    else
                                    {
            ...
            try
            {
                switch (Accion)
                {
                    case Cl_Enumeradores.eTipo_action.grabar:
                        ...
                        //ucCp_Retencion1.Get_Documento_Tipo_Talonario();
                    ...
                    case Cl_Enumeradores.eTipo_action.Anular:
                        ...
                        //btnGrabar.Image=Bizu.Presentation.Properties.Resources.eliminar;
        ...
        void set_Info_In_Controls()
        {
            try
            {
                ...
                //ucCp_Retencion1.Set_Check_doc_elec(Info_Retencion.EsDocumentoElectronico);
        ...
        private void frmCP_RetencionMant_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Accion == 0) { Accion = Bizu.Domain.General.Cl_Enumeradores.eTipo_action.grabar; }
        ...
        void Get_Modi()
        {
            ...
            {
                infoRetencion.IdEmpresa = param.IdEmpresa;
                infoRetencion.IdRetencion = Convert.ToDecimal(ucCp_Retencion1.txtIdRetencion.Text);
                ...
                foreach (var item in ucCp_Retencion1.Get_BindingList())
                {
                    ...
                    switch (param.IdCliente_Ven_x_Default)
                    {
                        case Cl_Enumeradores.eCliente_Vzen.EDEHSA:
                            if (item.Tipo == "IVA")
                            {
                                infoDetReten.re_baseRetencion = item.BaseImponible; //Convert.ToDouble(txtValorIva.EditValue);
                            }

                            if (item.Tipo == "RTF")
                            {
                                infoDetReten.re_baseRetencion = item.BaseImponible;//Convert.ToDouble(txtBaseImponible.EditValue);
                            }
        ...
        public Boolean getAsientoContable()
        {
            ...
            try
            {
                ...
                //29-06-2018 J.A Eliminar el detalle de retencion con valor cero
                List<ct_Cbtecble_det_Info> listCbteRetenDet = new List<ct_Cbtecble_det_Info>();
                ct_Cbtecble_det_Info InfoCbteRetenDet = new ct_Cbtecble_det_Info();

                foreach (var itemN in InfoCbteCble._cbteCble_det_lista_info)
                {
                    if (itemN.dc_Valor == 0) { }
                    else
                    {
                        InfoCbteRetenDet = itemN;
                        listCbteRetenDet.Add(InfoCbteRetenDet);
                    }
                }
                InfoCbteCble._cbteCble_det_lista_info = new List<ct_Cbtecble_det_Info>();
                InfoCbteCble._cbteCble_det_lista_info = new List<ct_Cbtecble_det_Info>(listCbteRetenDet);
        ...
        private Boolean Modificar_Retencion()
        {
            try
            {
                ...
                if (busRetencion.ActualizarDB(infoRetencion, ref msg))
                {
                    ...
                    //infoRetCbteCble = busRetencion.Get_Info_retencion_x_ct_cbtecble(Info_Retencion.IdEmpresa, Info_Retencion.IdRetencion);
                    //ucCp_Retencion1.setInfo_DiarioRetencion(infoRetCbteCble.ct_IdEmpresa, infoRetCbteCble.ct_IdTipoCbte, infoRetCbteCble.ct_IdCbteCble);
                    //if (infoRetCbteCble.ct_IdEmpresa == 0)
                    //{
                    //    if (MessageBox.Show("Se procederá a generar el diario desea continuar", "Diario Contable", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    //    {
                    //        ucCp_Retencion1.GeneraDiarioRetencion();
        ...
        void Anular_Retencion()
        {
            try
            {
                ...
                if (MessageBox.Show("¿Está realmente seguro que desea anular la Retención #: " + numRetencion + " ?. Este proceso afectará a la Orden de Giro #: " + txtNumCbte.EditValue + "", "Anulación de Retención", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        //MODIFICAMOS EL VALOR A PAGAR DE LA FACTURA
                        //BR 27-04-2018 INICIO
                        cp_orden_giro_Bus data_ordengiro = new cp_orden_giro_Bus();

                        double valor_ret = Convert.ToDouble(infoRetencion.ListDetalle.Sum(q => q.re_valor_retencion));

                        data_ordengiro.Modificar_ValorRetencion(Convert.ToInt32(Info_Retencion.IdEmpresa_Ogiro), Convert.ToDecimal(Info_Retencion.IdCbteCble_Ogiro),
                            Convert.ToInt32(Info_Retencion.IdTipoCbte_Ogiro), valor_ret * -1, ref MensajeError);
                        //BR 27-04-2018 FIN
        ...
        public Boolean ValidaBaseIvaRtf()
        {
            try
            {
                ...
                String ser1, ser2, NumReten;
                ser1 = ucCp_Retencion1.Get_Info_Talonario().Establecimiento;
                ser2 = ucCp_Retencion1.Get_Info_Talonario().PuntoEmision;
                NumReten = ucCp_Retencion1.Get_Info_Talonario().NumDocumento;

                if (ser1.Trim() == "" || ser2.Trim() == "" || NumReten.Trim() == "")
                {
                    MessageBox.Show("Ingrese un numero de documento", "Sistemas");
                    return false;
                }
                ...
                if (Convert.ToDouble(Math.Round(Convert.ToDouble(sumBaseIVA), 2)) > Convert.ToDouble(txtValorIva.EditValue))
                {
                ...
                decimal valor;
                valor = Math.Round(Convert.ToDecimal(sumBaseRTF), 2);

                if (Convert.ToDouble(valor) > Convert.ToDouble(txtBaseImponible.EditValue))
                {
                ...
                DateTime fechaFactura = Convert.ToDateTime(dteFechaFact.DateTime).Date;
                DateTime fechaEmisionRetencion = Convert.ToDateTime(Convert.ToDateTime(ucCp_Retencion1.dtp_fechaEmisionRetencion.Value)).Date;
                if (fechaEmisionRetencion < fechaFactura)
                {
                    MessageBox.Show("La fecha de emision de retención es menor a la fecha de factura", "Sistemas");
                    return false;
                }
        ...
        private void dteFechaOg_EditValueChanged(object sender, EventArgs e)
        {
            ucCp_Retencion1.set_FechaEmisionRetencion(Convert.ToDateTime(dteFechaOg.EditValue));
