...
{
    public partial class frmCP_Proveedor_Mant : DevExpress.XtraEditors.XtraForm
    {
        #region  'Declaracion Variables Generales Param LogError'
        cl_parametrosGenerales_Bus param = cl_parametrosGenerales_Bus.Instance;
        tb_sis_Log_Error_Vzen_Bus Log_Error_bus = new tb_sis_Log_Error_Vzen_Bus();
        cp_proveedor_Calificacion_Bus Bus_Calificacion = new cp_proveedor_Calificacion_Bus();
        ...
        tb_persona_Info InfoPersona = new Bizu.Domain.General.tb_persona_Info();
        ...
        // Datos Bancarios
        cp_proveedor_datos_acreditacion_Bus BusProveedorDatosAcreditacion = new cp_proveedor_datos_acreditacion_Bus();
        BindingList<cp_proveedor_datos_acreditacion_Info> BindingListProveedorDatosBancarios = new BindingList<cp_proveedor_datos_acreditacion_Info>();
        #endregion
        ...
        public Boolean validaciones()
        {
            try
            {
                ...
                {
                    MessageBox.Show(param.Get_Mensaje_sys(Bizu.Domain.General.enum_Mensajes_sys.Ingrese_la) + " clase del proveedor", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }

                if (txt_nomProveedor.Text == "")
                {
                    MessageBox.Show(param.Get_Mensaje_sys(Bizu.Domain.General.enum_Mensajes_sys.Ingrese_el) + " nombre del proveedor", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txt_nomProveedor.Focus();
                    return false;
                }
                ...
                {
                    MessageBox.Show(param.Get_Mensaje_sys(Bizu.Domain.General.enum_Mensajes_sys.Ingrese_CI_Ruc_Pasaporte), param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                //{
                //    MessageBox.Show(param.Get_Mensaje_sys(Bizu.Domain.General.enum_Mensajes_sys.Seleccione_la) + " Cuenta por pagar del Proveedor", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                //    return false;
                //}


                if (cmb_ctacbleCxp.cmbPlanCta.EditValue == null)
                {
                    MessageBox.Show("Seleccione la cuenta por pagar del Proveedor", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
        ...
        public void Limpiar()
        {
            try
            {
                ...
                txtCorreoPrincipal.EditValue = null;
                txtCorreoSecundario.EditValue = null;
                ...
                ucCp_TerminoPago1.cmbTermPago.EditValue = null;
            ...
            try
            {
                switch (_Accion)
                {
                    ...
                    case Cl_Enumeradores.eTipo_action.Anular:
                        ...
                        gridViewDatosBancarios.OptionsBehavior.ReadOnly = true;
        ...
        private void set_ProveedorInfo_in_controls()
        {
            try
            {
                ...
                this.chk_contExentoDeIVA.Checked = this.InfoProveedor.pr_contribuyenteExentoDeIVA == "S";
                ...
                cmbCtaCbleTarjeta.set_PlanCtarInfo(InfoProveedor.IdCtaCble_TarjetaPago);
                ucCp_TerminoPago1.set_Info((InfoProveedor.IdTerminoPago != null) ? InfoProveedor.IdTerminoPago.Trim() : null);
                ...
                txtCorreoPrincipal.Text = InfoProveedor.CorreoPrincipal;
                txtCorreoSecundario.EditValue = InfoProveedor.CorreoSecundario;
                txtCorreoAlterno.EditValue = InfoProveedor.CorreoAlterno;
                ...
                chkGranContribuyente.Checked = InfoProveedor.pr_GranContribuyente == "S" ? true : false;
                cmbRegimen.EditValue = InfoProveedor.IdRegimen;

                List<cp_proveedor_x_tb_ciiu_Info> ListProveedorXciiu = BusProveedorXtbciiu.GetList(InfoProveedor.IdEmpresa, InfoProveedor.IdProveedor);
                BindingListProveedorXciiu = new BindingList<cp_proveedor_x_tb_ciiu_Info>(ListProveedorXciiu);

                gridControlProveedorXciiu.DataSource = BindingListProveedorXciiu;
        ...
        private void LoadDatosBancarios()
        {
            try
            {
                // Ejecutar funcion para cargar los datos de acreditacion en la tabla gridControlDatosBancarios                
                List<cp_proveedor_datos_acreditacion_Info> ListaDatosAcreditacion = BusProveedorDatosAcreditacion.GetList(InfoProveedor.IdEmpresa, InfoProveedor.IdProveedor);
                BindingListProveedorDatosBancarios = new BindingList<cp_proveedor_datos_acreditacion_Info>(ListaDatosAcreditacion);
        ...
        public cp_proveedor_Info GetInfo_Proveedor()
        {
            try
            {
                ...
                InfoProveedor.pr_contribuyenteExentoDeIVA = this.chk_contExentoDeIVA.Checked ? "S" : "N";
                InfoProveedor.pr_plazo = Convert.ToInt32(txt_Plazo.Text);
                InfoProveedor.pr_estado = (chk_estado.Checked == true) ? "A" : "I";
                InfoProveedor.pr_nacionalidad = "NAC"; // NAC o EXT
                ...
                InfoProveedor.IdCiudad = null;

                //Se agrega termino de pago para proveedor 
                //InfoProveedor.IdTerminoPago = ucCp_TerminoPago1.get_Info().IdTerminoPago;
                if (String.IsNullOrEmpty(ucCp_TerminoPago1.get_Info().IdTerminoPago))
                    InfoProveedor.IdTerminoPago = null;
                else
                    InfoProveedor.IdTerminoPago = ucCp_TerminoPago1.get_Info().IdTerminoPago.Trim();
                ...
                if (string.IsNullOrEmpty(cmbCtaCbleTarjeta.get_PlanCtaInfo().IdCtaCble))
                    InfoProveedor.IdCtaCble_TarjetaPago = null;
                else
                    InfoProveedor.IdCtaCble_TarjetaPago = cmbCtaCbleTarjeta.get_PlanCtaInfo().IdCtaCble.Trim();
                ...
                foreach(var item in BindingListProveedorDatosBancarios)
                {
                    ...
                    if(item.Predefinida == true)
                    {
                        ...
                        InfoProveedor.num_cta_acreditacion = item.NumeroCuenta;
                        InfoProveedor.IdTipoDocumento_acreditacion = item.IdTipoIdentificacion;
                        InfoProveedor.cedulaRuc_acreditacion = item.Identificacion;
                        InfoProveedor.beneficiario_acreditacion = item.Beneficiario;
                ...
                InfoProveedor.IdRegimen = cmbRegimen.EditValue == null ? null : Convert.ToString(cmbRegimen.EditValue);

                InfoProveedor.CorreoPrincipal = (txtCorreoPrincipal.Text.Length == 0) ? "" : txtCorreoPrincipal.Text.Trim();
                InfoProveedor.CorreoSecundario = Convert.ToString(txtCorreoSecundario.EditValue);
                InfoProveedor.CorreoAlterno = Convert.ToString(txtCorreoAlterno.EditValue);
        ...
        private tb_persona_Info GetInfo_Persona()
        {
            try
            {
                ...
                InfoPersona.pe_correo = (txtCorreoPrincipal.Text.Length == 0) ? "" : txtCorreoPrincipal.Text.Trim();
                InfoPersona.pe_correo_secundario1 = Convert.ToString(txtCorreoSecundario.EditValue);
                ...
                InfoPersona.pe_codigoPostar = (this.txtCodPostal != null) ? this.txtCodPostal.Text.Trim() : " ";  // rth
        ...
        private void txe_cedRucPas_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                string TipoDocumento = UCDocumento.cmb_Docum_perso.SelectedValue.ToString();

                string men = "";
                if (TipoDocumento == "CED" || TipoDocumento == "RUC")
                {
            ...
            try
            {
                ...
                if (_Accion == Cl_Enumeradores.eTipo_action.grabar || _Accion == Cl_Enumeradores.eTipo_action.actualizar)
                {
                    if (BusPersona.VericarCedulaExiste(Convert.ToString(txe_cedRucPas.EditValue).Trim(), ref mensaje) == true)
                    {
                        ...
                        txtCorreoPrincipal.Text = !String.IsNullOrEmpty(InfoPersona.pe_correo) ? InfoPersona.pe_correo.Trim() : txtCorreoPrincipal.Text;
                        txtCorreoSecundario.Text = !String.IsNullOrEmpty(InfoPersona.pe_correo_secundario1) ? InfoPersona.pe_correo_secundario1.Trim() : txtCorreoSecundario.Text;
            ...
            try
            {
                ...
                if (Respuesta_Grabar_Persona)// si grabo o actualizo la persona procede a grabar el proveedor
                {
                    ...
                    if (Respuesta_Grabar_Provee)
                    {
                        ...
                        string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El Proveedor", IdProveedor);
        ...
        private Boolean Actualizar()
        {
            ...
            try
            {
                ...
                string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El Proveedor", InfoProveedor.IdProveedor);
                MessageBox.Show(smensaje, param.Nombre_sistema);
        ...
        private void frmCP_MantProveedor_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                BindingListProveedorDatosBancarios = new BindingList<cp_proveedor_datos_acreditacion_Info>();
                gridControlDatosBancarios.DataSource = BindingListProveedorDatosBancarios;
        ...
        private void Cargar_Combos()
        {
            try
            {
                ...
                List<tb_Catalogo_Info> ListTipoIdentificacion = new List<tb_Catalogo_Info>();
                ListTipoIdentificacion = CatBus.Get_CatalogoPorTipo(3);
                cmbTipoIdentificacionGrid.DataSource = ListTipoIdentificacion;

                // Cargo el combo de tipo de cuenta
                List<tb_Catalogo_Info> ListTipoCuenta = new List<tb_Catalogo_Info>();
                ListTipoCuenta = CatBus.Get_CatalogoPorTipo(27);
                cmbTipoCuentaAcreditacionGrid.DataSource = ListTipoCuenta;

                // Cargo el combo de banco
        ...
        private void ucCp_TerminoPago1_event_cmbTermPago_EditValueChanged(object sender, EventArgs e)
        {
            ucCp_TerminoPago1.cargar();
        }

        private void btn_CalificacionNuevo_Click(object sender, EventArgs e)
        {
            try
            {
                frmCp_Proveedor_Calificacion_Mant Frm = new frmCp_Proveedor_Calificacion_Mant();
                Frm.event_frmCp_Proveedor_Calificacion_Mant_FormClosing += Frm_event_frmCp_Proveedor_Calificacion_Mant_FormClosing;
                Frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            if (cmbActividadEconomica.EditValue == null)
            {
            ...
            try
            {
                // Setea los atributos a mi objeto Info para el registro en la base de datos
                cp_proveedor_x_tb_ciiu_Info infoProveedorXtbciiu = new cp_proveedor_x_tb_ciiu_Info();
                infoProveedorXtbciiu.IdEmpresa = param.IdEmpresa;
                infoProveedorXtbciiu.IdProveedor = Convert.ToDecimal(txt_IdProveedor.Text);
                infoProveedorXtbciiu.Id = Convert.ToInt32(cmbActividadEconomica.EditValue);

                bool existe = BusProveedorXtbciiu.ExisteDB(infoProveedorXtbciiu.IdEmpresa, infoProveedorXtbciiu.IdProveedor, infoProveedorXtbciiu.Id);

                if (existe == true)
                {

                    MessageBox.Show("La Actividad Economica, Ya esta Registrada", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                ...
                BusProveedorXtbciiu.GrabarActividadEconomica(infoProveedorXtbciiu);

                // mensaje de registro guardado con exito

                MessageBox.Show("La Actividad Economica, Se registro correctamente", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Question);
                cmbActividadEconomica.EditValue = null;

                // Consulta registro y lo asigna a la lista
                List<cp_proveedor_x_tb_ciiu_Info> ListaProveedorXtb_ciiu = BusProveedorXtbciiu.GetList(param.IdEmpresa, Convert.ToDecimal(txt_IdProveedor.Text));
                BindingListProveedorXciiu = new BindingList<cp_proveedor_x_tb_ciiu_Info>(ListaProveedorXtb_ciiu);

                // Setea lista al gridControl
                gridControlProveedorXciiu.DataSource = BindingListProveedorXciiu;
                gridControlProveedorXciiu.RefreshDataSource();
            }
            catch (Exception ex)
            {
        ...
        private void gridViewProveedorXciiu_KeyDown(object sender, KeyEventArgs e)
        {
            cp_proveedor_x_tb_ciiu_Info InfoSelected = gridViewProveedorXciiu.GetFocusedRow() as cp_proveedor_x_tb_ciiu_Info;
            ...
            if (e.KeyCode == Keys.Delete)
            {
                try
                {
                    ...
                    if (MessageBox.Show("Estas seguro de eliminar el registro seleccionado?", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                        return;

                    BusProveedorXtbciiu.EliminarBD(InfoSelected.IdEmpresa, InfoSelected.IdProveedor, InfoSelected.Id);
                    MessageBox.Show("La actividad económica seleccionada fue eliminada!", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {            
            ...
            if(e.KeyCode == Keys.Delete)
            {
                ...
                if (MessageBox.Show("Deseas eliminar el registro seleccionado?", param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    return;

                try
                {
                    if (RowSelected.Predefinida == true)
                    {
                        MessageBox.Show("No es posible eliminar una cuenta predeterminada!", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }

                    BusProveedorDatosAcreditacion.EliminarBD(RowSelected.IdEmpresa, RowSelected.IdProveedor, RowSelected.Secuencia);
        ...
        private void gridViewDatosBancarios_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                cp_proveedor_datos_acreditacion_Info RowSelected = gridViewDatosBancarios.GetFocusedRow() as cp_proveedor_datos_acreditacion_Info;
                ...
                if (e.Column == colBanco)
                {
                    SaveRow(RowSelected);
                }

                if (e.Column == colTipoCuenta)
                {
                    SaveRow(RowSelected);
                }

                if (e.Column == colNumCuenta)
                {
                    SaveRow(RowSelected);
                }

                if (e.Column == colTipoIdentifi)
                {
                    SaveRow(RowSelected);
                }

                if (e.Column == colNumIdentifi)
                {
                    string Cedula = Convert.ToString(e.Value);
                    tb_persona_Info InfoPersonaAcreditacion = BusPersona.Get_Info_Persona(Cedula);
                ...
                if (e.Column == colBeneficiario || e.Column == colCorreo)
                {
                    SaveRow(RowSelected);
                }
                ...
                {
                    if (RowSelected.IdEmpresa == 0 && RowSelected.IdProveedor == 0 && RowSelected.Secuencia == 0)
                        return;

                    if (RowSelected.Predefinida == true)
                    {
                        cp_proveedor_Info InfoProveedor = new cp_proveedor_Info();
                        InfoProveedor.IdEmpresa = RowSelected.IdEmpresa;
                        InfoProveedor.IdProveedor = RowSelected.IdProveedor;
                        InfoProveedor.IdTipoCta_acreditacion_cat = RowSelected.IdTipoCuenta;
                        InfoProveedor.num_cta_acreditacion = RowSelected.NumeroCuenta;
                        InfoProveedor.IdBanco_acreditacion = RowSelected.IdBanco;
                        InfoProveedor.IdTipoDocumento_acreditacion = RowSelected.IdTipoIdentificacion;
                        InfoProveedor.cedulaRuc_acreditacion = RowSelected.Identificacion;
                        InfoProveedor.beneficiario_acreditacion = RowSelected.Beneficiario;
                        InfoProveedor.correo_acreditacion = RowSelected.Correo;

                        BusProveedor.ActualizarDatosBancarios(InfoProveedor);
                    }
        ...
        private void gridViewDatosBancarios_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            if(e.Column == colPredeterminada)
            {
                ...
                {
                    gridViewDatosBancarios.SetFocusedRowCellValue(colPredeterminada, true);
                }
            }

        }

        private bool SaveRow(cp_proveedor_datos_acreditacion_Info RowSelected)
        {
            if (RowSelected.IdEmpresa == 0 && RowSelected.IdProveedor == 0 && RowSelected.Secuencia == 0)
            {
                RowSelected.IdEmpresa = param.IdEmpresa;
                RowSelected.IdProveedor = Convert.ToDecimal(txt_IdProveedor.Text);

                BusProveedorDatosAcreditacion.GrabarBD(RowSelected);
            }
            else
            {
                BusProveedorDatosAcreditacion.ModificarBD(RowSelected);
            }

            return true;
        }

        private void gridViewDatosBancarios_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            //GridView view = sender as GridView;

            //if (view != null && e.RowHandle != DevExpress.XtraGrid.GridControl.NewItemRowHandle)
            //{
            //    string cellValue = e.Value.ToString();

            //    // Verifica si la celda afectada es la que quieres validar
            //    if (e.Column == colNumIdentifi)
            //    {
            //        string TipoIdentificacion = gridViewDatosBancarios.GetFocusedRowCellValue(colTipoIdentifi).ToString();

            //        int cantidadDigitosPermitidos = 10;

            //        if (TipoIdentificacion == "RUC")
            //        {
            //            cantidadDigitosPermitidos = 13;
            //        }

            //        // Verifica la cantidad de dígitos
            //        if (cellValue.Length != cantidadDigitosPermitidos)
            //        {
            //            // Muestra un mensaje de error o toma alguna acción
            //            //MessageBox.Show($"La longitud de la entrada debe ser de {cantidadDigitosPermitidos} dígitos.", "Error de validación", MessageBoxButtons.OK, MessageBoxIcon.Error);

            //            // Revierte el cambio cancelando la edición
        ...
        private void gridViewDatosBancarios_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            GridView view = sender as GridView;
            ...
            if (view != null)
            {
                ...
                {
                    string NumeroCuenta = Convert.ToString(gridViewDatosBancarios.GetFocusedRowCellValue(colNumCuenta));
                    if (columnName == "NumeroCuenta")
                    {
                        // Verifica si la entrada contiene solo caracteres numéricos
                        if (!EsNumero(e.Value.ToString()))
                        {
                            e.ErrorText = "Solo se permiten caracteres numéricos en esta columna.";
                            e.Valid = false;
                        }
                    }
                }
                ...
                {
                    string Correo = Convert.ToString(gridViewDatosBancarios.GetFocusedRowCellValue(colCorreo));
                    // Verifica si el correo electrónico es válido utilizando una expresión regular
                    if (!EsCorreoElectronicoValido(e.Value.ToString()))
                    {
                        e.ErrorText = "El formato del correo electrónico no es válido.";
                        e.Valid = false;
                    }
                }
                ...
                {
                    string TipoIdentificacion = Convert.ToString(gridViewDatosBancarios.GetFocusedRowCellValue(colTipoIdentifi));
                    ...
                    // Verifica la cantidad de dígitos
                    if (e.Value != null && e.Value.ToString().Length != cantidadDigitosPermitidos)
                    {
                        e.ErrorText = $"La longitud de la entrada debe ser de {cantidadDigitosPermitidos} dígitos.";
                        e.Valid = false;
                    }
                }
            }
        ...
        // Método para verificar si una cadena contiene solo caracteres numéricos
        ...
        private bool EsCorreoElectronicoValido(string input)
        {
