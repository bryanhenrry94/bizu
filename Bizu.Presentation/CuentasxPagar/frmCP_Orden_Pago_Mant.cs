...
{
    public partial class frmCP_Orden_Pago_Mant : DevExpress.XtraEditors.XtraForm
    {
        ...
        cp_orden_pago_x_empleado_x_nomina_x_periodo_Bus bus_cp_orden_pago_x_empleado_x_nomina_x_periodo = new cp_orden_pago_x_empleado_x_nomina_x_periodo_Bus();
        ...
        cp_orden_pago_x_empleado_x_nomina_x_periodo_Info Info_cp_orden_pago_x_empleado_x_nomina_x_periodo = new cp_orden_pago_x_empleado_x_nomina_x_periodo_Info();
        ...
        void ModificarAsientoContable()
        {
            try
            {
                ...
                if (tipPago.GeneraDiario == "S")
                {
                    ...
                    ct_Cbtecble_det_Info info;

                    // VALIDACION PARA SETEAR LA CUENTA CONTABLE DEL BANCO SELECCIONADO
                    ...
                    //    info.IdCtaCble = banco_info.IdCtaCble;
        ...
        public frmCP_Orden_Pago_Mant(Cl_Enumeradores.eTipo_action Numerador)
            : this()
        {
            try
            {
        ...
        private void Set_Info_en_Controls()
        {
            try
            {
                ...
                {
                    cp_orden_pago_tipo_Info tipPago = list_OrdenTipPago.Find(q => q.IdTipo_op == Info_Orden_Pago.IdTipo_op);

                    if (tipPago != null)
                    {
                        if (tipPago.GeneraDiario == "S")
                        {
                            if (Convert.ToInt32(infoDetPago.IdTipoCbte_cxp) != 0 && Convert.ToDecimal(infoDetPago.IdCbteCble_cxp) != 0)
                            {
            ...
            try
            {
                ...
                foreach (var item in DataSource)
                {
                    ...
                    if (item.check == true || colcheck.Visible == false)
                    {
                        ...
                        {
                            info_det.Valor_a_pagar = item.Total_a_pagar_OP;
                        }
                        else
                        {
        ...
        public Boolean Insertar_OrdenPago()
        {
            try
            {
                ...
                if (res)
                {
                    ...
                    string smensaje = string.Format("{0} # {1} se guardó con éxito.", "Orden de Pago ", Id);
                    ...
                    //GRABAR cp_orden_pago_x_empleado_x_nomina_x_periodo
                    Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdOrdenPago = Id;


                    //se valida porque cuando ingresa una orden de pago manual se está callendo
                    if (Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdEmpresa > 0)
                    {
                        if (Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdNomina_Tipo > 0 && Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdNomina_TipoLiqui > 0 && Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdPeriodo > 0)
                        {
                            bool res2 = bus_cp_orden_pago_x_empleado_x_nomina_x_periodo.GuardarDB(Info_cp_orden_pago_x_empleado_x_nomina_x_periodo, ref mensaje);
                            if (res2 == false)
                            {
            ...
            try
            {
                if (Info_Orden_Pago != null)
                {
                    ...
                    if (MessageBox.Show("¿Está seguro que desea anular la orden de pago #: " + Info_Orden_Pago.IdOrdenPago + " ?", "Anulación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        Info_Orden_Pago.Fecha_UltAnu = fechaAnulacion;
        ...
        Boolean Actualizar()
        {
            try
            {
                ...
                if (Bus_OrdenPago.ModificarDB(Cab, ref mensaje))
                {
                    ...
                    if (bus_cp_orden_pago_x_empleado_x_nomina_x_periodo.ModificarDB(Info_cp_orden_pago_x_empleado_x_nomina_x_periodo, ref mensaje))
                    {
                        res = true;
                    }
                    else
                    {
                        MessageBox.Show("No se pudo Asociar la orden de pago a la Nomina Seleccionada para el Periodo#: " + Info_cp_orden_pago_x_empleado_x_nomina_x_periodo.IdPeriodo, "Anulación", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        private void cmbBanco_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (lista_Banco.Count > 0)
                    banco_info = lista_Banco.FirstOrDefault(q => q.IdEmpresa == param.IdEmpresa && q.IdBanco == Convert.ToInt32(cmbBanco.EditValue));
            }
            catch (Exception ex)
            {
