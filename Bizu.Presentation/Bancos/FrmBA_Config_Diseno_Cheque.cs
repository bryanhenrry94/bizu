...
{
    public partial class FrmBA_Config_Diseno_Cheque : DevExpress.XtraEditors.XtraForm
    {
        ...
        public ba_Config_Diseno_Cheque_Info Get_Info() 
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(param.Get_Mensaje_sys(enum_Mensajes_sys.Error_comunicarse_con_sistemas) + ex.Message + " ", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Log_Error_bus.Log_Error(NameMetodo + " - " + ex.ToString());
                DiseñoI = new Bizu.Domain.Bancos.ba_Config_Diseno_Cheque_Info();
                return DiseñoI;
            }
       ...
       //*************************************************************//
        ...
        private void cmb_CtaBanco_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IdBanco != 0 && IdBanco != null)
                { 
                    ba_Banco_Cuenta_Info rr = new Bizu.Domain.Bancos.ba_Banco_Cuenta_Info();
