...
{
    public partial class FrmBA_Catalogo_Mant : DevExpress.XtraEditors.XtraForm
    {
                ...
                else
                {
                    if (MessageBox.Show("¿Está seguro que desea Anular ?", "Anulación  ", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        if (baCatalogoBus.AnularDB(ref _Info))
                        {
                            ...
                            string smensaje = string.Format("{0} # {1} se anuló con éxito.", "El catálogo", _Info.IdCatalogo);
            ...
            try
            {
                ...
                if (baCatalogoBus.ValidarCodigoExiste(_Info.IdCatalogo) == false)
                {
                    if (baCatalogoBus.ValidarIdTipoCatalogo_Descripcion(_Info.IdTipoCatalogo, _Info.ca_descripcion)==false)
                    {
                        ...
                        {
                            string smensaje = string.Format("{0} # {1} se guardó con éxito.", "El Catálogo", _Info.IdCatalogo);
                            MessageBox.Show(smensaje, param.Nombre_sistema);                             
                            bloquear_Datos();
                        }
                        else
        ...
        void Actualizar()
        {
            ...
            try
            {
                ...
                {
                    textID.Text = _Info.IdCatalogo.ToString();
                    string smensaje = string.Format("{0} # {1} se modificó con éxito.", "El Catálogo", _Info.IdCatalogo);
                    MessageBox.Show(smensaje, param.Nombre_sistema);    
                    bloquear_Datos();
