    ...
    {                
        ...
        private void set_Info_CbteBan()
        {
            try
            {
                ...
                // LLena campos de la pestana Datos Entrega Cheque
                ba_Cbte_Ban_Datos_Entrega_cheq_Bus BusCbteBanDatosEntrega = new ba_Cbte_Ban_Datos_Entrega_cheq_Bus();
                ba_Cbte_Ban_Datos_Entrega_cheq_Info InfoCbteBanDatosEntrega = new ba_Cbte_Ban_Datos_Entrega_cheq_Info();

                InfoCbteBanDatosEntrega = BusCbteBanDatosEntrega.GetInfo(Info_CbteBan.IdEmpresa, Info_CbteBan.IdCbteCble, Info_CbteBan.IdTipocbte);

                if (InfoCbteBanDatosEntrega != null)
                {
                    dtpFechaEntrega.EditValue = InfoCbteBanDatosEntrega.FechaEntrega;
                    txtCedula.EditValue = InfoCbteBanDatosEntrega.Cedula;
                    txtNombres.EditValue = InfoCbteBanDatosEntrega.Nombres;
                    txtApellidos.EditValue = InfoCbteBanDatosEntrega.Apellidos;
                    txtNotaAdicional.EditValue = InfoCbteBanDatosEntrega.NotaEntrega;

                    if (InfoCbteBanDatosEntrega.Foto != null)
                    {
                        this.pic_foto.Image = Funciones.ArrayAImage(InfoCbteBanDatosEntrega.Foto);
                    }
                }
            }
            catch (Exception ex)
                ...
                if (_Accion == Cl_Enumeradores.eTipo_action.grabar)
                {
                    ...
                    foreach (var item in Obj_DetalleAprob)
                    {
                        if (item.Check == true)
                        {
                            ...
                            info_ordenCan.FechaCancelacion = Convert.ToDateTime(dt_fechaCbte.Text);
        ...
        public ba_Cbte_Ban_Info get_CbteBan()
        {
            try
            {
                ...
                Info_CbteBan.IdProveedor = ucGe_Beneficiario.Get_Info_Proveedor().IdProveedor;
            ...
            try
            {
                ...
                foreach (var item in Obj_DetalleAprob)
                {
                    if (item.Check == true)
                    {
                        ...
                        {
                            InfoDebe.IdCtaCble = Info_Cta_Acreedora.IdCtaCble_Acreedora;
                        }
