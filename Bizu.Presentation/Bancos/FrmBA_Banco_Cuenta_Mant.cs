...
{
    public partial class FrmBA_Banco_Cuenta_Mant : DevExpress.XtraEditors.XtraForm
    {
            ...
            try
            {
                ...
                if (_Accion == Cl_Enumeradores.eTipo_action.grabar)
                {
                    if (BancoCuentB.Get_Cuenta_Existe(BancoCuentI.IdEmpresa, BancoCuentI.IdCtaCble, ref mensaje) == true)
                    {
                        ...
                        {
                            string smensaje = string.Format("{0} # {1} se guardó con éxito.", "La Cuenta ", BancoCuentI.IdBanco);
                ...
                if (_Accion == Cl_Enumeradores.eTipo_action.actualizar)
                {
                    ...
                    {
                        string smensaje = string.Format("{0} # {1} se modificó con éxito.", "La Cuenta ", BancoCuentI.IdBanco);
                        MessageBox.Show(smensaje, param.Nombre_sistema);
                        valor = true;
                    }
                    else
        ...
        private Boolean Anular()
        {
            try
            {
                ...
                if (MessageBox.Show("¿Está seguro que desea anular la Cuenta Banco ?", "Anulación de Cuenta Banco  " + param.Nombre_sistema, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        string smensaje = string.Format("{0} # {1} se anuló con éxito.", "La Cuenta ", BancoCuentI.IdBanco);
                        MessageBox.Show(smensaje, param.Nombre_sistema); 
                        lbl_Estado.Visible = true;
                        resultB = true;
                    }
