...
{
    public partial class FrmCa_Caja : DevExpress.XtraEditors.XtraForm
    {
               ...
               if (validaColumna())
               {
                   ...
                   if (_Accion == Cl_Enumeradores.eTipo_action.grabar)
                   {
                       if(caja_B.GrabarDB(caja_I,ref idCaja, ref  MensajeError))
                       {
                           ...
                           string smensaje = string.Format("{0} # {1} se guardó con éxito.", "Caja ", txt_idCaja.Text);
                   ...
                   if (_Accion == Cl_Enumeradores.eTipo_action.actualizar)
                   {
                       ...
                       {
                           string smensaje = string.Format("{0} # {1} se modificó con éxito.", "Caja ", txt_idCaja.Text);
                           MessageBox.Show(smensaje, param.Nombre_sistema);
                           //ucGe_Menu.Visible_bntGuardar_y_Salir = DevExpress.XtraBars.BarItemVisibility.Never;
                           //ucGe_Menu.Visible_btnGuardar = DevExpress.XtraBars.BarItemVisibility.Never;
                           bolResult = true;
                   ...
                   if (_Accion == Cl_Enumeradores.eTipo_action.Anular)
                   {
                       if (caja_I.Estado != "I")
                       {
                           if (MessageBox.Show("¿Está seguro que desea anular la Caja #: " + txt_idCaja.Text  + " ?", "Anulación de Caja", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                           {
                               ...
                               {
                                   string smensaje = string.Format("{0} # {1} se anuló con éxito.", "Caja ", txt_idCaja.Text);
                                   MessageBox.Show(smensaje, param.Nombre_sistema);                                   
