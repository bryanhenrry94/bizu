...
{
    //Ult.Mod=DEREK MEJIA 06022014
    //                   FrmCa_Caja_MovimientoConsulta_Ingreso
    public partial class FrmCa_Caja_MovimientoConsulta_Ingreso : DevExpress.XtraEditors.XtraForm
    {
        #region Declaración de Variables
        seg_Menu_bus Busseg = new seg_Menu_bus();
        List<seg_Menu_info> Infoseg = new List<seg_Menu_info>();
        tb_sis_Log_Error_Vzen_Bus Log_Error_bus = new tb_sis_Log_Error_Vzen_Bus();
        FrmCa_Caja_Movimiento_Ingreso formaCajaMan = new FrmCa_Caja_Movimiento_Ingreso();
        ...
        void formaCajaMan_event_FrmCa_Caja_Movimiento_FormClosing(object sender, System.Windows.Forms.FormClosingEventArgs e)
        {
            try
            {
                 load_data();
        ...
        private void load_data()
        {
            try
            {
                ...
                this.gridControl_MoviCaja.DataSource = cajaMovi_B.Get_list_Ingreso (param.IdEmpresa,FechaIni,FechaFin, ref  MensajeError);
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        private void FrmCa_Caja_MovimientoConsulta_Load(object sender, EventArgs e)
        {
            ...
            {
                //Control de Usuario
                Infoseg = Busseg.Get_List_Menu_x_Empresa_x_Usuario_Control(param.IdEmpresa, param.IdUsuario, "Caja.FrmCa_Caja_MovimientoConsulta_Ingreso");
