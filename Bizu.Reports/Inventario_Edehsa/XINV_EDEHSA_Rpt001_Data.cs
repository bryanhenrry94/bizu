    ...
    public class XINV_EDEHSA_Rpt001_Data
    {
        public List<XINV_EDEHSA_Rpt001_Info> Get_Kardes_Movimiento(int IdEmpresa, int IdSucursalIni, int IdSucursalFin, int IdBodegaIni, int IdBodegaFin, decimal IdProductoIni, decimal IdProductoFin, string IdCentroCosto, string IdSubCentroCosto, int IdMov_inven_tipoIni, int IdMov_inven_tipoFin, string TipoMov, DateTime FechaIni, DateTime FechaFin)
        {
            try
            {
                ...
                using (Entities_Inventario_General conexion = new Entities_Inventario_General())
                {
                    ...
                    foreach (var item in querry)
                    {
                        ...
                        XINV_EDEHSA_Rpt001_Info itemInfo = new XINV_EDEHSA_Rpt001_Info();
                        ...
                        itemInfo.nom_centro_costo = (item.IdCentro_costo != null) ? "[" + item.IdCentro_costo + "] - " + item.nom_centro_costo : "";
            ...
            catch (Exception)
            {
                ...
                return new List<XINV_EDEHSA_Rpt001_Info>();
            }
        }
    }
}