...
namespace Bizu.Reports.Contabilidad
{
    public partial class XCONTA_Rpt006_rpt : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public bool bMostrarCentroCosto;
        ...
        private void XCONTA_Rpt006_rpt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                ...
                FechaIni = Convert.ToDateTime(P_FechaIni.Value).Date;
                FechaFin = Convert.ToDateTime(P_FechaFin.Value).Date;
                Mostrar_Asiento_cierre = Convert.ToBoolean(P_Mostrar_Asiento_cierre.Value);
                                
                if (P_ListIdCtasCbles == null)
                {
                    lista = BusReporte.Get_Data_Reporte(IdEmpresa, IdCtaCble,IdCentro_Costo, FechaIni, FechaFin, IdPuntoCargo_Grupo, IdPuntoCargo,
                        Mostrar_Asiento_cierre, ref msg);                            
                }
                else
                {
                    if (P_ListIdCtasCbles.Count > 0)
                    {
                        foreach (var item in P_ListIdCtasCbles)
                        {
                            ...
                            lista_Aux = BusReporte.Get_Data_Reporte(IdEmpresa, item,IdCentro_Costo, FechaIni, FechaFin, IdPuntoCargo_Grupo,
                                IdPuntoCargo,Mostrar_Asiento_cierre, ref msg);
