    ...
    public partial class XCONTA_Rpt012_frm : Form
    {
        ...
        private void cargar_combos()
        {
            try
            {
                ...
                if (param.IdCliente_Ven_x_Default == Domain.General.Cl_Enumeradores.eCliente_Vzen.EDEHSA)
                {
                    foreach (var itemPeriodo in ListPeriodo)
                    {
                        chkList_Periodos.Items.Add(itemPeriodo, itemPeriodo.nom_periodo + " (" + itemPeriodo.pe_FechaIni.Date + "-" + itemPeriodo.pe_FechaFin.Date + ")", CheckState.Unchecked, true);
                    }
                }else{
                    foreach (var itemPeriodo in ListPeriodo)
                    {
        ...
        private void Llamar_rpt_balance_general()
        {
            try
            {
                ...
                Reporte.PConsiderar_Asiento_cierre_anual.Value = chk_cierreAnual.Checked;
