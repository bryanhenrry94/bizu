...
namespace Bizu.Reports.Contabilidad
{
    public class XCONTA_Rpt012_Data
    {
           ...
           , bool MostrarCC, string IdUsuario, ref String MensajeError)
        {
            try
            {
                ...
                using (EntitiesContabilidadRptGeneral Estado_Resultado = new EntitiesContabilidadRptGeneral())
                {
                    ...
                    IdCentroCosto, IdPunto_cargo_grupo, IdPunto_cargo, Mostrar_reg_en_cero, MostrarCC, false, IdUsuario).ToList();

                    // QUIERO TRAER TODAS LAS CUENTAS CONTABLES DEL PLAN DE CUENTAS
                    var Querry_x_ER = from C in listBalance
                                      where 
                                      //C.gc_estado_financiero == "BG" && 
                    ...
                    foreach (var item in Querry_x_ER)
                    {
                        ...
                        itemInfo.orden_fila = item.orden_fila;
                        itemInfo.Reg_x_CC = item.Reg_x_CC;
        ...
        public List<XCONTA_Rpt002_Info> Get_data_Mayorizado_x_RangoFecha(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, string IdCentroCosto, int IdNivel_a_mostrar
             , int IdPunto_cargo_grupo
           , int IdPunto_cargo
            , bool Mostrar_reg_en_cero
           , bool MostrarCC,
            bool Considerar_Asiento_cierre_anual,
            string IdUsuario, ref String MensajeError)
        {
            try
            {
                List<XCONTA_Rpt002_Info> listadedatos = new List<XCONTA_Rpt002_Info>();
