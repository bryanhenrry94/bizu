    ...
    public class XCXP_Rpt017_Data
    {
        ...
        public List<XCXP_Rpt017_Info> get_Reporte_Estado_Cuenta_Proveedor_con_Dias_Vencidos(int IdEmpresa, DateTime FechaCorte, decimal IdProveedorIni, decimal IdProveedorFin)
        {
            try
            {
                ...
                {
                    listado.SetCommandTimeOut(30000);
                    ...
                    foreach (var item in select)
                    {
                        ...
                        infoRpt.IdClaseProveedor = item.IdClaseProveedor;
                        infoRpt.Total_Compra = item.Total_Compra;
                        ...
                        infoRpt.IdCentroCosto = item.IdCentroCosto;
                        lstRpt.Add(infoRpt);
                    }
                }
        ...
        public List<XCXP_Rpt017_Info> get_Reporte_Estado_Cuenta_Proveedor_con_Dias_Vencidos_XClase_Prov(int IdEmpresa, DateTime FechaCorte, decimal IdProveedorIni, decimal IdProveedorFin, int claseProv)
        {
            try
            {
                FechaCorte = FechaCorte.Date;
                ...
                using (EntitiesCXP_General listado = new EntitiesCXP_General())
                {
                    ...
                    foreach (var item in select)
                    {
                        if (claseProv > 0)
                        {
                            if (item.IdClaseProveedor == claseProv)
                            {
                                XCXP_Rpt017_Info infoRpt = new XCXP_Rpt017_Info();
                                infoRpt.IdRow = item.IdRow;
                                infoRpt.IdEmpresa = item.IdEmpresa;
