...
namespace Bizu.Reports.Inventario
{
    public class XINV_Rpt009_Data
    {
        ...
        public List<XINV_Rpt009_Info> Get_data(int IdEmpresa, int IdSucursal, int IdBodega, string IdCategoria, int IdLinea,
            Boolean Registro_Cero, DateTime Fecha_corte, decimal IdProducto, ref String MensajeError)
        {
            try
            {
                int IdSucursalIni = 0;
                ...
                using (Entities_Inventario_General BalanceGeneral = new Entities_Inventario_General())
                {
                    ...
                    {
                        if (IdCategoria != "")
                            select = select.Where(v => Math.Round(Convert.ToDouble(v.costo_total), 2) != 0 && v.IdCategoria == IdCategoria);
                        else
                            select = select.Where(v => Math.Round(Convert.ToDouble(v.costo_total), 2) != 0);
                    }
                    else
                        if (IdCategoria != "")
        ...
        public List<XINV_Rpt009_Info> Get_data_Edehsa(int IdEmpresa, int IdSucursal, List<int> List_IdBodega, string IdCategoria, List<int> List_Linea,
            Boolean Registro_Cero, DateTime Fecha_corte, decimal IdProducto, string IdUsuario, ref String MensajeError)
        {
            try
            {
                decimal IdProducto_ini = 0;
                ...
                using (Entities_Inventario_General BalanceGeneral = new Entities_Inventario_General())
                {
                    ...
                    if (Registro_Cero == false)
                        select = select.Where(v => Math.Round(Convert.ToDouble(v.Stock), 2) != 0);

                    if (IdCategoria != "")
                        select = select.Where(v => v.IdCategoria == IdCategoria);

                    if (List_Linea.Count() > 0)
                        select = select.Where(v => List_Linea.Contains(v.IdLinea));
                    ...
                    foreach (var item in select)
                    {
                        ...
                        itemInfo.nom_sucursal = item.nom_sucursal;
                        itemInfo.Stock = item.Stock;
                        itemInfo.Stock_conversion = item.Stock_conversion;
                        itemInfo.costo = Convert.ToDouble(item.mv_costo);
                        itemInfo.costo_total = item.costo_total;
                        itemInfo.IdCategoria = item.IdCategoria;
                        itemInfo.ca_Categoria = item.ca_Categoria;
