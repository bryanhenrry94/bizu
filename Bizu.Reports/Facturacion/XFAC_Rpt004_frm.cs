...
{
    public partial class XFAC_Rpt004_frm : Form
    {
        tb_sis_Log_Error_Vzen_Bus Log_Error_bus = new tb_sis_Log_Error_Vzen_Bus();
        cl_parametrosGenerales_Bus param = cl_parametrosGenerales_Bus.Instance;  

        public XFAC_Rpt004_frm()
        {
            InitializeComponent();
        ...
        void ConsultarDatos()
        {
            try
            {
                XFAC_Rpt004_rpt Reporte = new XFAC_Rpt004_rpt();
                string tipoPago = "";
                ...
                Reporte.PTipoNota.Value = ucFa_Menu.cmb_TipoNota.EditValue;
                Reporte.Set_lista_motivo(ucFa_Menu.Get_lst_chk_Tipo_nota());
        ...
        private void XFAC_Rpt004_frm_Load(object sender, EventArgs e)
        {
            try
            {
                ucFa_Menu.cargarTipoDocumento_NC_ND();
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
