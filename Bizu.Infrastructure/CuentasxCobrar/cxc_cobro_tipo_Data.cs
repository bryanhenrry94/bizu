    ...
    public class cxc_cobro_tipo_Data
    {
        ...
        public List<cxc_cobro_tipo_Info> Get_List_Cobro_Tipo_FUEIVA()
        {
            try
            {
                ...
                foreach (var info in select)
                {
                    if (info.ESRetenFTE == "S" || info.ESRetenIVA == "S")
                    {
        ...
        public cxc_cobro_tipo_Info Get_Info_Cobro_Tipo(string IdCobro_tipo)
        {
            try
            {
                ...
                {                    
                    ...
                    Info.tc_EsCheque = item.tc_EsCheque;
                    Info.tc_Afecha = item.tc_Afecha;
                    Info.tc_interno = item.tc_interno;
                    Info.Estado = item.Estado;
                    Info.tc_generaNCAuto = item.tc_generaNCAuto;
                    Info.tc_abreviatura = item.tc_abreviatura;
                    Info.tc_docXCobrar = item.tc_docXCobrar;
                    Info.tc_Orden = Convert.ToInt32(item.tc_Orden);
                    Info.IdUsuario = item.IdUsuario;
                    Info.IdEstadoCobro_Inicial = item.IdEstadoCobro_Inicial;
                    Info.tc_Que_Tipo_Registro_Genera = item.tc_Que_Tipo_Registro_Genera;
                    Info.tc_SePuede_Depositar = item.tc_SePuede_Depositar;
                    Info.ESRetenIVA = item.ESRetenIVA;
                    Info.ESRetenFTE = item.ESRetenFTE;
                    Info.PorcentajeRet = Convert.ToDouble(item.PorcentajeRet);
                    Info.IdEstadoCobro_Inicial = item.IdEstadoCobro_Inicial;
                    Info.tc_Tomar_Cta_Cble_De = item.tc_Tomar_Cta_Cble_De;
                    Info.IdMotivo_tipo_cobro = item.IdMotivo_tipo_cobro;
