    ...
    public class cp_Aprobacion_Ing_Bod_x_OC_det_Data
    {
        ...
        public List<cp_Aprobacion_Ing_Bod_x_OC_det_Info> Get_List_Aprobacion_Ing_Bod_x_OC_det(int IdEmpresa, decimal IdAprobacion)
        {
            ...
            try
            {
                ...
                foreach (var item in Query)
                {
                    ...
                    Obj.Precio = (item.SubTotal + item.Descuento) / item.Cantidad;
                    Lst.Add(Obj);
                }
        ...
        public Boolean EliminarDB(int IdEmpresa, decimal IdAprobacion, ref string msg)
        {
            try
            {
                ...
                {
                    int numeroElimindo = Entity.Database.ExecuteSqlCommand("delete from cp_Aprobacion_Ing_Bod_x_OC_det where IdEmpresa = {0} and IdAprobacion = {1};", IdEmpresa, IdAprobacion);
                }
        ...
        {
            cp_Aprobacion_Ing_Bod_x_OC_det_Info2 Obj = new cp_Aprobacion_Ing_Bod_x_OC_det_Info2();

            try
            {

                EntitiesInventario OECxP = new EntitiesInventario();
                var Query = from q in OECxP.vwin_Ing_Egr_Inven_det_x_com_ordencompra_local_det
                            where q.IdEmpresa_inv == IdEmpresa
                            && q.IdSucursal_inv == Idsucursal
                            && q.IdNumMovi == IdNumMovi
                            && q.IdMovi_inven_tipo_inv == IdNumMoviTipo
