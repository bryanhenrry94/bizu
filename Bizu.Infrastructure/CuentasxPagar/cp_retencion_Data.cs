...
namespace Bizu.Infrastructure.CuentasxPagar
{
    ...
    {
        string mensaje = "";
        string campoAdicionalMail = null;
        ...
        public List<cp_retencion_Info> Get_List_retencion(int IdEmpresa, DateTime Fechaini, DateTime FechaFin)
        {
            try
            {
                ...
                foreach (var item in consulta)
                {
                    ...
                    //info.EsDocumentoElectronico = Convert.ToBoolean(item.EsDocumentoElectronico);
            ...
            try
            {
                ...
                IQueryable<vwcp_Retencion_sri_Info> consulta = from selec in ECXP.vwcp_Retencion_sri
                                                               ...
                                                               select new vwcp_Retencion_sri_Info
                                                               {
                                                                   ...
                                                                   IdRetencion = selec.IdRetencion,
                                                                   serie = selec.serie,
                                                                   NumRetencion = selec.NumRetencion,
                                                                   fecha = selec.fecha,
                                                                   ...
                                                                   co_FechaFactura = selec.co_FechaFactura,
                                                                   pe_direccion = selec.pe_direccion,
                                                                   pe_telfono_Contacto = selec.pe_telfono_Contacto,
                                                                   co_serie = selec.co_serie,
                                                                   co_factura = selec.co_factura,
                                                                   IdOrden_giro_Tipo = selec.IdOrden_giro_Tipo,
                                                                   RazonSocial = selec.RazonSocial,
                                                                   NombreComercial = selec.NombreComercial,
                                                                   ContribuyenteEspecial = selec.ContribuyenteEspecial,
                                                                   ObligadoAllevarConta = selec.ObligadoAllevarConta,
                                                                   em_ruc = selec.em_ruc,
                                                                   em_direccion = selec.em_direccion,
                                                                   IdTipoDocumento = selec.IdTipoDocumento,
                                                                   IdSucursal = selec.IdSucursal,
                                                                   Su_Descripcion = selec.Su_Descripcion,
                                                                   Su_Direccion = selec.Su_Direccion,
                                                                   Estado = selec.Estado, 
                                                                   CorreoPrincipal = selec.CorreoPrincipal,
                                                                   CorreoAlterno = selec.CorreoAlterno,
                                                                   CorreoSecundario = selec.CorreoSecundario
                ...
                foreach (var item in consulta)
                {
                    ...
                    myObjectRete.impuestos = new List<Domain.class_sri.Retencion.impuesto>();
                    ...
                    if (listaDetReten.Count != 0)
                    {
                        ...
                        {
                            Domain.class_sri.Retencion.impuesto imp = new Domain.class_sri.Retencion.impuesto();
                            ...
                            imp.baseImponible = Convert.ToDecimal(Math.Round(itemDET.re_baseRetencion, 2, MidpointRounding.AwayFromZero));
                            imp.valorRetenido = Convert.ToDecimal(Math.Round(Convert.ToDouble(itemDET.re_valor_retencion), 2, MidpointRounding.AwayFromZero));
                    ...
                    if (!String.IsNullOrEmpty(item.CorreoPrincipal.Trim()))
                    {
                        campoAdicionalMail = item.CorreoPrincipal.Trim();
                        // campos adicionales               
                        ...
                        //if (datosAdc.email_bien_escrito(item.CorreoPrincipal.Trim()) == true)
                        //{
                        myObjectRete.infoAdicional = new List<comprobanteRetencionCampoAdicional>();
                        if (!String.IsNullOrEmpty(campoAdicionalMail))
                        {
                            //if (datosAdc.email_bien_escrito(campoAdicionalMail) == true)
                            //{
                            compoadicional.nombre = "MAIL";
                            compoadicional.Value = campoAdicionalMail;
                    ...
                    direccion = item.pe_direccion.Trim();
                    if (!String.IsNullOrEmpty(direccion))
                    {
                        comprobanteRetencionCampoAdicional compoadicional = new comprobanteRetencionCampoAdicional();
                        compoadicional.nombre = "DIRECCION_CLIENTE";
                        compoadicional.Value = direccion;
                        myObjectRete.infoAdicional.Add(compoadicional);
                    }
