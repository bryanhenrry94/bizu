    ...
    public class fa_pedido_Data
    {
        ...
        public fa_pedido_Info Get_Pedido_Venta(int IdEmpresa, int idsucursal, int idbodega, decimal IdPedido)
        {
            try
            {
                ...
                var cabeceraPedido = from ped in OEPPedido.fa_pedido
                                     where ped.IdEmpresa == IdEmpresa
                                     && ped.IdSucursal == idsucursal
                                     && ped.IdBodega == idbodega
                ...
                foreach (var item in cabeceraPedido)
                {
                    ...
                    info.cp_tipopago = item.cp_tipopago.Trim();
                    info.Trasnporte = item.transporte;
                    info.Interes = item.interes;
                    info.Otro1 = item.otro1;
                    info.Otro2 = item.otro2;
                    info.IdEstadoProduccion = item.IdEstadoProduccion;

                    info.cp_observacion2 = "[" + item.IdPedido + "] " + item.cp_observacion;
        ...
        public List<fa_pedido_Info> Get_List_pedido2(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, int idsucursal, int idbodega)
        {
            try
            {
                ...
                var cabeceraPedido = from ped in OEPPedido.vwfa_pedido_detalle
                                     where ped.IdEmpresa == IdEmpresa && ped.IdSucursal == idsucursal && ped.IdBodega >= 1
                ...
                foreach (var item in cabeceraPedido.OrderByDescending(a => a))
                {
                    fa_pedido_Info info = new fa_pedido_Info();
        ...
        public List<fa_pedido_Info> Get_List_pedido(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, int idsucursal, int idbodega)
        {
            try
            {
                ...
                var cabeceraPedido = from ped in OEPPedido.vwfa_pedido_detalle
                                     where ped.IdEmpresa == IdEmpresa && ped.IdSucursal == idsucursal && ped.IdBodega == idbodega//>= 1
                              && ped.cp_fecha >= FechaIni && ped.cp_fecha <= FechaFin && ped.Estado == "A"
                                     group ped by new { ped.IdEstadoProduccion, ped.Entregado, ped.CodPedido, ped.cp_fecha, ped.cp_fechaVencimiento, ped.cp_diasPlazo, ped.interes, ped.transporte, ped.otro1, ped.otro2, ped.IdEmpresa, ped.IdSucursal, ped.IdBodega, ped.IdPedido, ped.IdVendedor, ped.IdCliente, ped.pe_apellido, ped.pe_nombre, ped.pe_nombreCompleto, ped.bo_Descripcion, ped.Su_Descripcion, ped.Ve_Vendedor, ped.cp_observacion, ped.cp_tipopago, ped.Estado, ped.IdEstadoAprobacion, ped.Descripcion }
                              into grouping
                                     let count = grouping.Count()
                                     select new { grouping.Key, subtotal = grouping.Sum(p => p.dp_subtotal), total = grouping.Sum(p => p.dp_total) };
        ...
        public List<fa_pedido_Info> Get_List_pedido_estado(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, int idsucursal, string idEstadoApro)
        {
            try
            {
                List<fa_pedido_Info> lM = new List<fa_pedido_Info>();
                ...
                var cabeceraPedido = from ped in OEPPedido.vwfa_pedido_detalle
                                     where ped.IdEmpresa == IdEmpresa && ped.IdSucursal == idsucursal && ped.IdBodega >= 1
                                     && ped.cp_fecha >= FechaIni && ped.cp_fecha <= FechaFin
                                     && ped.IdEstadoAprobacion == idEstadoApro
                                     && ped.Estado == "A"
                                     group ped by new { ped.IdEstadoProduccion, ped.Entregado, ped.CodPedido, ped.cp_fecha, ped.cp_fechaVencimiento, ped.cp_diasPlazo, ped.interes, ped.transporte, ped.otro1, ped.otro2, ped.IdEmpresa, ped.IdSucursal, ped.IdBodega, ped.IdPedido, ped.IdVendedor, ped.IdCliente, ped.pe_apellido, ped.pe_nombre, ped.pe_nombreCompleto, ped.bo_Descripcion, ped.Su_Descripcion, ped.Ve_Vendedor, ped.cp_observacion, ped.cp_tipopago, ped.Estado, ped.IdEstadoAprobacion, ped.Descripcion }
                                         into grouping
                                     let count = grouping.Count()
        ...
        public List<fa_pedido_Info> Get_List_pedido_Venta(int IdEmpresa)
        {
            try
            {
                List<fa_pedido_Info> lM = new List<fa_pedido_Info>();

                //Bizu.Infrastructure.Facturacion_Poligrup.EntitiesFacturacionPoligrup OEPPedido = new Bizu.Infrastructure.Facturacion_Poligrup.EntitiesFacturacionPoligrup();

                //var consulta = from q in OEPPedido.SPFa_Solicitud_Ventas()
                //               where q.IdEmpresa == IdEmpresa
                //               && q.Estado == "A"
                //               && q.IdEstadoAprobacion == "A"
                //               select q;
        ...
        public Boolean GrabarDB(fa_pedido_Info info, ref decimal id, ref string msg)
        {
            try
            {
                using (EntitiesFacturacion context = new EntitiesFacturacion())
                {
                    ...
                    //Falta modificar la cantidad del producto ojo.
                    ...
                    foreach (var item in info.lista_detalle)
                    {
                        ...
                        {
                            i = i + 1;
                            fa_pedido_det_Info pedido_info = new Domain.Facturacion.fa_pedido_det_Info();
                            ...
                            pedido_info.IdCod_Impuesto_Iva = item.IdCod_Impuesto_Iva;
                            ...
                            pedido_info.Estado = item.Estado;
                            pedido_info.idPunto_cargo = item.idPunto_cargo;
                            pedido_info.idPunto_cargo_grupo = item.idPunto_cargo_grupo;
                            pedido_info.dp_por_iva = item.dp_por_iva;
                            pedido_info.idCod_Impuesto_Ice = item.idCod_Impuesto_Ice;
