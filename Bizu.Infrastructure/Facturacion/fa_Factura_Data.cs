...
namespace Bizu.Infrastructure.Facturacion
{
    public class fa_Factura_Data
    {
        ...
        public List<Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas> Get_List_VentasParaATS(int IdEmpresa, int anio, int mes)
        {
            try
            {
                List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas> Lista = new List<Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas>();
                ...
                using (EntitiesFacturacion OEFAC = new EntitiesFacturacion())
                {
                    ...
                    foreach (var item in SelectFactura)
                    {
                        Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas Info = new Domain.CuentasxPagar.xmlATS_V_1_1_4.detalleVentas();
                        ...
                        {
                            Info.parteRelVtas = Domain.CuentasxPagar.xmlATS_V_1_1_4.parteRelType.SI;
                        }
                        else
                        {
                            Info.parteRelVtas = Domain.CuentasxPagar.xmlATS_V_1_1_4.parteRelType.NO;
                        }
                        ...
                        Info.numeroComprobantes = Convert.ToString(item.numeroComprobantes);
                        ...
                        Info.tipoEmision = Domain.CuentasxPagar.xmlATS_V_1_1_4.tipoEmisionType.F;
        ...
        {
            List<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.ventaEst> list;

            try
            {
                using (EntitiesFacturacion facturacion = new EntitiesFacturacion())
                {
                    string sql = ((((" select T1.vt_serie1 as codEstab,  ROUND(sum(case when RTRIM(LTRIM(T1.vt_tipoDoc))='C' then T1.total*-1 else  T1.total end), 2) as ventasEstab   " + " from (select vt_tipoDoc,vt_serie1,ROUND(sum(baseNoGraIva) + Sum(baseImpGrav), 2) as total  from vwFa_Documento_DeclaracionSRI   ") + " where idempresa= " + IdEmpresa.ToString()) + " and Year(vt_fecha)= " + anio.ToString()) + " and month(vt_fecha)=" + mes.ToString()) + " group by  vt_tipoDoc,vt_serie1)T1  group by T1.vt_serie1 ";
                    list = facturacion.Database.SqlQuery<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.ventaEst>(sql, new object[0]).ToList<Bizu.Domain.CuentasxPagar.xmlATS_V_1_1_4.ventaEst>();
                }
            }
            catch (Exception ex)
            {
