...
namespace Bizu.Infrastructure.Bancos
{
    public class ba_Archivo_Transferencia_Det_Data
    {
        ...
        public List<ba_Archivo_Transferencia_Det_Info> Get_List_Archivo_transferencia_Det(int idEmpresa, decimal idArchivo)
        {
            try
            {
                ...
                using (EntitiesBanco Conexion = new EntitiesBanco())
                {
                    ...
                    {
                        ba_Archivo_Transferencia_Det_Info item = new ba_Archivo_Transferencia_Det_Info();
                        item.IdEmpresa = det.IdEmpresa;
                        item.IdArchivo = det.IdArchivo;
                        item.Secuencia = det.Secuencia;
                        item.IdProceso_bancario = det.IdProceso_bancario;
                        item.pe_nombreCompleto = det.pe_nombreCompleto;
                        item.pe_cedulaRuc = det.pe_cedulaRuc;
                        item.IdBanco_acreditacion = det.IdBanco_acreditacion;
                        item.IdTipoCta_acreditacion_cat = det.IdTipoCta_acreditacion_cat;
                        item.num_cta_acreditacion = det.num_cta_acreditacion;
                        item.IdTipoDocumento_acreditacion = det.IdTipoDocumento_acreditacion;
                        item.cedulaRuc_acreditacion = det.cedulaRuc_acreditacion;
                        item.beneficiario_acreditacion = det.beneficiario_acreditacion;
                        item.correo_acreditacion = det.correo_acreditacion;                        
                        item.IdEmpresa_OP = det.IdEmpresa_OP;
                        item.IdOrdenPago = det.IdOrdenPago;
                        item.Secuencia_OP = det.Secuencia_OP;
                        item.Referencia = det.Referencia;
                        item.Valor = det.Valor;
                        item.Valor_cobrado = det.Valor_cobrado;
                        decimal saldo = det.Saldo;
                        item.Saldo = Convert.ToDecimal(det.Saldo);
                        item.IdEstadoRegistro_cat = det.IdEstadoRegistro_cat;
                        item.nom_EstadoRegistro = det.nom_EstadoRegistro_cat;
                        item.Secuencial_reg_x_proceso = det.Secuencial_reg_x_proceso;
                        item.Id_Item = det.Id_Item;
                        item.IdOrden_Bancaria = det.IdOrden_Bancaria;
                        item.Fecha = det.Fecha;
                        item.IdEmpresa_pago = det.IdEmpresa_pago;
                        item.IdTipoCbte_pago = det.IdTipoCbte_pago;
                        item.IdCbteCble_pago = det.IdCbteCble_pago;
                        item.cb_Estado = det.cb_Estado;
                        item.CodPersona = det.CodPersona;
                        item.IdInstitucion_contra = det.IdInstitucion_contrato;
                        item.IdContrato = det.idContrato;
                        item.IdEmpresaNomina = det.IdEmpresaNomina;
                        item.IdNominaTipo = det.IdNominaTipo;
                        item.IdNominaTipoLiqui = det.IdNominaTipoLiqui;
                        item.IdPeriodo = det.IdPeriodo;
                        item.IdRubro = det.IdRubro;
                        item.IdEmpleado = det.IdEmpleado;
                        item.IdBanco = det.IdBanco;                        
            ...
            try
            {
                ...
                using (EntitiesBanco Conexion = new EntitiesBanco())
                {
                    ...
                    foreach (var item in lst)
                    {
                        ...
                        Info.correo_acreditacion = item.correo_acreditacion;
        ...
        public decimal Get_Secuencial_registro(int IdEmpresa, string IdProceso_bancario)
        {
            try
            {
                try
        ...
        public decimal Get_secuencial_registro_modificar(int IdEmpresa, string IdProceso_bancario, decimal IdArchivo)
        {
            try
            {
                try
