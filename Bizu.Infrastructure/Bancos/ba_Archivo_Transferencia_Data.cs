    ...
    public class ba_Archivo_Transferencia_Data
    {
        ...
        public string GetId_codigoArchivo_guayaquil(int IdEmpresa, int IdBanco, DateTime fecha)
        {
            ...
            try
            {
                ...
                if (q.ToList().Count == 0)
                {
                    Id = fecha.Year.ToString().PadLeft(4, '0') + fecha.Month.ToString().PadLeft(2, '0') + fecha.Day.ToString().PadLeft(2, '0') + "_01";
                }
                else
                {
                    var select_ = (from Query in context.ba_Archivo_Transferencia
                    ...
                    int secuencia = Convert.ToInt32(select_.Substring(9, len - 9));
                    secuencia = secuencia + 1;
                    Id = fecha.Year.ToString().PadLeft(4, '0') + fecha.Month.ToString().PadLeft(2, '0') + fecha.Day.ToString().PadLeft(2, '0') + "_" + Convert.ToString(secuencia).PadLeft(2, '0');
                }
                return Id;
            }
            catch (Exception ex)
