...
namespace Bizu.Infrastructure.Bancos
{
    public class ba_Cbte_Ban_Data
    {
            ...
            try
            {
                ...
                var select_ = from T in b.vwba_MoviCaja_Depositados
                              ...
                              group T by new
                              {
                                  ...
                                  T.tm_Signo,
                                  T.IdUsuario,
                                  T.IdCbteCble_MoviCaja,
                                  T.IdTipocbte_MoviCaja,
                              }
                              into grouping
                ...
                foreach (var item in select_)
                {
                    ...
                    dat.IdEmpresa = Convert.ToInt32((item.Key.IdEmpresa == null) ? 0 : item.Key.IdEmpresa);
                    dat.IdSucursal = Convert.ToInt32((item.Key.IdSucursal == null) ? 0 : item.Key.IdSucursal);
                    dat.IdCobro = Convert.ToDecimal((item.Key.IdCobro == null) ? 0 : item.Key.IdCobro);
                    dat.IdCobro_tipo = item.Key.IdCobro_tipo;
                    dat.IdCliente = Convert.ToDecimal((item.Key.IdCliente == null) ? 0 : item.Key.IdCliente);
                    dat.cr_TotalCobro = Convert.ToDouble((item.cr_TotalCobro == null) ? 0 : item.cr_TotalCobro);
                    dat.cr_fecha = Convert.ToDateTime((item.Key.cr_fecha == null) ? DateTime.Now : item.Key.cr_fecha);
        ...
        public List<ba_Cbte_Ban_Info> Get_List_Cbte_Ban(int IdEmpres, int IdTipocbteBan, DateTime F_inicio, DateTime F_fin, string IdEstado_Preaviso_ch_cat, ref string MensajeError)
        {
        ...
        public List<vwba_Banco_Movimiento_det_cancelado_Info> Get_List_Cancelada(int IdEmpresa, decimal IdCbteCble, int IdTipocbte, ref string MensajeError)
        {
            try
            {
                ...
                try
                {
                    ...
                    var select_ = from T in db.vwba_Banco_Movimiento_det_cancelado_lite
        ...
        public bool ActualizarEstadoCheque(int IdEmpresa, int IdTipoCbte, decimal IdCbteCble, eEstado_Cheque Estado)
        {
            try
            {
                ...
                var InfoCbteBa = dbContext.ba_Cbte_Ban.Where(q => q.IdEmpresa == IdEmpresa && q.IdTipocbte == IdTipoCbte && q.IdCbteCble == IdCbteCble).First();
                
                if(InfoCbteBa != null)
                {
                    InfoCbteBa.IdEstado_cheque_cat = Estado.ToString();
