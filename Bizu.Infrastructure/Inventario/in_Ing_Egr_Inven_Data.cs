    ...
    public class in_Ing_Egr_Inven_Data
    {
            ...
            try
            {
                try
                {
                    using (EntitiesInventario Context = new EntitiesInventario())
                    {
                        ...
                        Address.EsDocumentoAutorizado = info.EsDocumentoAutorizado;
            ...
            try
            {
                using (EntitiesInventario context = new EntitiesInventario())
                {
                    ...
                    if (contact != null)
                    {
                        ...
                        contact.EsDocumentoAutorizado = info.EsDocumentoAutorizado;
        ...
        public Boolean Existe_NumGuia(int IdEmpresa, string numGuia, ref string msgs)
        {
            try
            {
                ...
                var query = from q in Context.in_Ing_Egr_Inven
                            where q.IdEmpresa == IdEmpresa
                            && q.numGuia == numGuia
                            && q.Estado == "A"
                            select q;
                ...
                if (query.Count() > 0)
                {
                    mensaje = "número de documento ya se encuentra registrado en la base de datos";
                    return true;
                }
                else
                    return false;
             ...
             int IdBodegaIni, int IdBodegaFin, DateTime FechaIni, DateTime FechaFin, int IdMovi_inven_tipo)
        {
            ...
            try
            {
                ...
                foreach (var item in Query)
                {
                    ...
                    Obj.EsDocumentoAutorizado = item.EsDocumentoAutorizado;
                    Obj.numDocumento = item.numDocumento;
                    Obj.Num_Requerimiento = item.Num_Requerimiento;
        ...
        public Boolean EliminarDB(in_Ing_Egr_Inven_Info info, ref string mensaje)
        {
            try
            {
                try
                {
                    ...
                    {
                        in_Ing_Egr_Inven_det_Data odata = new in_Ing_Egr_Inven_det_Data();
                        odata.EliminarDB(info.IdEmpresa, info.IdSucursal, 0, info.IdNumMovi, info.IdMovi_inven_tipo, ref mensaje);
                    }

                    using (EntitiesInventario Entity = new EntitiesInventario())
                    {
                        int numeroElimindo = Entity.Database.ExecuteSqlCommand("delete from in_Ing_Egr_Inven where IdEmpresa="
                            + info.IdEmpresa + " and IdSucursal=" + info.IdSucursal
                            + " and IdNumMovi= " + info.IdNumMovi
                            + " and IdMovi_inven_tipo= " + info.IdMovi_inven_tipo);
                    }
        ...
        public double Obtener_Stock_x_FechaMovimiento(int IdEmpresa, int IdSucursal, int IdBodega, decimal IdProducto, DateTime FechaMovimiento)
        {
            ...
            try
            {
                EntitiesInventario Inventario_Entity = new EntitiesInventario();

                DateTime FechaCorte = FechaMovimiento.Date;

                var query = from q in Inventario_Entity.spin_Producto_Stock_x_Fecha(IdEmpresa, IdSucursal, IdBodega, IdProducto, FechaCorte) select q;
