   ...
   public class in_movi_inven_tipo_x_tb_bodega_Data
    {
       ...
       public Boolean ModificarDB(List<in_movi_inven_tipo_x_tb_bodega_Info> lst, int idEmpresa, in_movi_inven_tipo_Info moviI, ref string mensaje)
       {
           try
           {
               eliminarregistrotabla(idEmpresa, moviI.IdMovi_inven_tipo, ref mensaje);
       ...
       //Nuevo no está la opcion de eliminar J.A.
       public Boolean eliminarregistrotabla(int idEmpresa, int moviI, ref string mensaje)
       {
           try
           {
               EntitiesInventario OEInvet = new EntitiesInventario();
               ...
               if (c.Count() > 0)
               {
                   foreach (var item in c)
                   {
                       ...
                       {
                           var contact = context.in_movi_inven_tipo_x_tb_bodega.FirstOrDefault(obj => obj.IdEmpresa == idEmpresa && obj.IdMovi_inven_tipo == moviI);
                           if (contact != null)
                           {
                               context.in_movi_inven_tipo_x_tb_bodega.Remove(contact);
                               context.SaveChanges();
                               context.Dispose();
                           }
                       }
