    ...
    public class Af_Depreciacion_Data
    {
        ...
        public Boolean VerificarFechaUltDepre(int IdEmpresa, ref DateTime FechaUlt, decimal TipoDepreciacion)
        {
            try
            {
                ...
                var select = (from q in ocxc.Af_Depreciacion
                              where q.IdEmpresa == IdEmpresa
                              && q.IdTipoDepreciacion == TipoDepreciacion && q.Estado == "A"
                              select q.IdDepreciacion).Max();

                idDep = Convert.ToInt32(select.ToString());

                EntitiesActivoFijo entDep = new EntitiesActivoFijo();
                var selec = from q in entDep.Af_Depreciacion
                            where q.IdEmpresa == IdEmpresa && q.IdDepreciacion == idDep
                             && q.IdTipoDepreciacion == TipoDepreciacion && q.Estado == "A"
                            select q;
                foreach(var item in selec)
                {
                    FechaUlt = item.Fecha_Depreciacion;
                }
