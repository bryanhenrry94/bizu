...
namespace Core.Erp.Business.Compras
{
    public class com_solicitud_compra_det_Bus
    {
        ...
        public List<com_solicitud_compra_det_Info> GetList_SolicitudPendiente(int IdEmpresa)
        {
            ...
            {
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "GetList_SolicitudPendiente", ex.Message), ex) { EntityType = typeof(com_solicitud_compra_det_Bus) };
            }
        }
