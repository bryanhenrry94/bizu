...
namespace Core.Erp.Business.ActivoFijo
{
    public class vwAf_ActivoFijo_Bus
    {
        ...
        public List<vwAf_ActivoFijo_Info> Get_List_ActivoFijo_Depre_Mes_Contable(int IdEmpresa, DateTime FechaIni, DateTime FechaFin, string IdUsuario)
        {
            ...
            {
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Get_List_ActivoFijo_Depre_Mes_Contable", ex.Message), ex) { EntityType = typeof(vwAf_ActivoFijo_Bus) };
            }
        }
