...
namespace Core.Erp.Business.CuentasxPagar.Archivos_Bancos
{
    public class cp_Archivo_Banco_Produbanco_pago_proveedores_Bus
    {
        ...
        public Boolean Generar_archivo_pago_proveedores_BP(Archivo_Banco_Produbanco_pago_proveedores_Info info, string patch, string carSeparador, string nombre_File)
        {
            ...
            {
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Generar_archivo_pago_proveedores_BP", ex.Message), ex) { EntityType = typeof(cp_Archivo_Banco_Bolivariano_pago_proveedores_Bus) };
            }
        }
    }
}