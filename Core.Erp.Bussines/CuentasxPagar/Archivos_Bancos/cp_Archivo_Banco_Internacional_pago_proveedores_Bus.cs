...
{
    public class cp_Archivo_Banco_Internacional_pago_proveedores_Bus
    {
        public Boolean Guardar_Archivo_Banco_Internacional(Archivo_Banco_Internacional_pago_proveedores_Info info, tb_banco_procesos_bancarios_x_empresa_Info Info_proceso, string patch, string nombre_file)
        {
            try
            {
                if (Validar_Linea_archivo_BI(info))
                {
                    switch (Info_proceso.cod_Proceso)
                    {
                        case ebanco_procesos_bancarios_tipo.PAGO_PROVEEDORES_BANC_INT:// pago a proveedores banco bolivariano
                            Generar_archivo_pago_proveedores_BI(info, patch, "\t", nombre_file);
                            break;
                        default:
                            break;
                    }
        ...
        public bool Validar_Linea_archivo_BI(Archivo_Banco_Internacional_pago_proveedores_Info info_)
        {
            try
            {
        ...
        public Boolean Generar_archivo_pago_proveedores_BI(Archivo_Banco_Internacional_pago_proveedores_Info info, string patch, string carSeparador, string nombre_File)
        {
            try
            {
                string linea = "";
                ...
                linea += info.PagTer_referenciaAdicional; //OPCIONAL
            ...
            {
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Generar_archivo_pago_proveedores_BI", ex.Message), ex) { EntityType = typeof(cp_Archivo_Banco_Bolivariano_pago_proveedores_Bus) };
            }
        }
    }
}